...
CHOOSECROWN="false"
LIMITEDTRIES="false"
FORCEMODDEDWEAPON="true"
FORCEMODDEDCROWN="true"
FORCEMODDEDRACE="true"
MOD=("Egg" "Blaac's Weapon Mutations" "Crypt Of The Necrodancer")
DESC="Hop like a bunny to the music to get more eggs!"
...
do
	for line in $(< ${MOD[$COUNT]});do
		...
		if [ "${line:0:1}" = '/' ]
		then
			if [ "$IFCHARACTER" = "0" -a "$FORCEMODDEDRACE" = "true" ]
			then
				CHARACTERS=()
				IFCHARACTER="1"
			fi
		...
		if [ "${line:0:1}" = '"' ]
		then
			if [ "$IFCROWN" = "0" -a "$FORCEMODDEDCROWN" = "true" ]
			then
				CROWNS=()
				IFCROWN="1"
			fi
		...
		if [ "${line:0:1}" = '*' ]
		then
			if [ "$IFWEAPON" = "0" -a "$FORCEMODDEDWEAPON" = "true" ]
			then
				WEAPONS=()
				IFWEAPON="1"
			fi
		...
		if [ "${line:0:1}" = '?' ]
		then
			COMMENTS+=("${MOD[$COUNT]}[${line:1:${#line}-1}]")
		fi
	done
	COUNT=$(($COUNT+1))
done
...
for i in ${MODS[@]}
do
	DWL+="wait mod_load(string_trim("'"'"data/Event.mod/"$i'"'"));"$'\n'
done
...
#WEAPONS=("slot machine")
#CROWNS=("slots")
CHR=$[$RANDOM % ${#CHARACTERS[@]}]
CRW=$[$RANDOM % ${#CROWNS[@]}]
WEP=$[$RANDOM % ${#WEAPONS[@]}]
echo -e "
/*@pDesc@w=$DESC#@rMods@w=$(printf "#  %s" "${MOD[@]}")#@bCharacter@w=#  ${CHARACTERS[$CHR]}#@yCrown@w=#  ${CROWNS[$CRW]}#@gWeapon@w=#  ${WEAPONS[$WEP]}#@pSeed@w=$SEED#Limited Tries:${LIMITEDTRIES}*/
/*$(printf "|%s" "${COMMENTS[@]}")*/
#define init
...
trace("'"'"Current Event:"'"'");
trace("'"'"Mods=${MOD[0]}$(printf ",%s" "${MOD[@]:1}")"'"'");
...
if(global.start){
	global.alias = "'"'""'"'";
	with(Player){
		give_wep(string_trim("'"'"${WEAPONS[$WEP]}"'"'"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("'"'"${CHARACTERS[$CHR]}"'"'")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("'"'"${CHARACTERS[$CHR]}"'"'"))){
					instance_destroy();
				}
				exit;
			}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "'"'""'"'";
	with(GameCont){
		score = global.alias + "'"'": Area "'"'" + string(area) + "'"'"-"'"'" + string(subarea) + "'"'" L"'"'" + string(loops) + "'"'" Kills: "'"'" + string(kills) + "'"'" Character: "'"'" + (ChooseCharacter ? global.Race : "'"'""'"'") + "'"'" Mods:${MOD[0]}$(printf ",%s" "${MOD[@]:1}")"'"'" + "'"'" Character:${CHARACTERS[$CHR]}"'"'" + "'"'" Crown:${CROWNS[$CRW]}"'"'" + "'"'" Weapon:${WEAPONS[$WEP]}"'"'" + "'"'" Seed:$SEED"'"'";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + "'"'" $DATE Event.txt"'"'")){
		prevScores = string_load(global.alias + "'"'" $DATE Event.txt"'"'");
		while(!file_loaded(global.alias + "'"'" $DATE Event.txt"'"'")){wait 1;}
		string_save(prevScores, global.alias + "'"'" $DATE Event.txt"'"'");
	}else{
		string_save(score, global.alias + "'"'" $DATE Event.txt"'"'");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "'"'"404"'"'"){
			...
			"'"'"content"'"'":"'"'"'+base64("'"'"Mods:${MOD[0]}$(printf ",%s" "${MOD[@]:1}")"'"'" + "'"'" Character:${CHARACTERS[$CHR]}"'"'" + "'"'" Crown:${CROWNS[$CRW]}"'"'" + "'"'" Weapon:${WEAPONS[$WEP]}"'"'" + "'"'" Seed:$SEED"'"'" + "'"'"|"'"'" + string_split(score, "'"'"Mods:"'"'")[0])+'"'"'"
			}'
			, "'"'"out.txt"'"'");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),"'"'""'"'"),chr(13),"'"'""'"'"), string_replace_all(string_replace_all("'"'":${MOD[0]}$(printf ",%s" "${MOD[@]:1}")"'"'" + "'"'" Character:${CHARACTERS[$CHR]}"'"'" + "'"'" Crown:${CROWNS[$CRW]}"'"'" + "'"'" Weapon:${WEAPONS[$WEP]}"'"'" + "'"'" Seed:$SEED"'"'",chr(10),"'"'""'"'"),chr(13),"'"'""'"'"))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("'"'"${CHARACTERS[$CHR]}"'"'");}else{global.Race = race;}}
...
" | tee Event.mod.gml PreviousWeeklies/${DATE}-Event.mod.gml
FILE="leaderboards/Event.txt"
SCORE_DATE=$(echo $(cat "$FILE")| cut -d';' -f 2)
cp leaderboards/Event.txt leaderboards/${SCORE_DATE}-Event.txt
git add Event.mod.gml
git add leaderboards/${SCORE_DATE}-Event.txt
git rm leaderboards/Event.txt
echo -e "Mods:${MOD[0]}$(printf ",%s" "${MOD[@]:1}") Character:${CHARACTERS[$CHR]} Crown:${CROWNS[$CRW]} Weapon:${WEAPONS[$WEP]} Seed:$SEED Date:;$DATE;" | tee leaderboards/Event.txt
git add leaderboards/Event.txt
git add PreviousWeeklies/${DATE}-Event.mod.gml
git commit -am "Automatically Updated The Event on "$DATE
#git push