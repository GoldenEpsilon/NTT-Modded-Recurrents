...
/*@rMods@w=#  Brimthrone#  Disk Marrow#  Carmageddon#  Cool Flame#  Dasher#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  brimthrone#@pSeed@w=13544#Limited Tries:false*/
/*|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Carmageddon[WARNING: May contain drifting]|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Brimthrone,Disk Marrow,Carmageddon,Cool Flame,Dasher");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Brimthrone,Disk Marrow,Carmageddon,Cool Flame,Dasher" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:brimthrone" + " Seed:13544";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Brimthrone,Disk Marrow,Carmageddon,Cool Flame,Dasher" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:brimthrone" + " Seed:13544" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Brimthrone,Disk Marrow,Carmageddon,Cool Flame,Dasher" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:brimthrone" + " Seed:13544",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
