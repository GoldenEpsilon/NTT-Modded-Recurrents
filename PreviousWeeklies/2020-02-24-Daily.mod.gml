...
/*@rMods@w=#  BadEnd#  Carmageddon#  bossbar#  B#  Team Attack#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  super bazooka#@pSeed@w=14814#Limited Tries:false*/
/*|BadEnd[For when you don't want a good end.#(sit after loop 3)]|Carmageddon[WARNING: May contain drifting]|bossbar[For when you want to know#just how close you were to looping]|B[B]|Team Attack[Team Age Mutant#Ninja Mutants Go!]*/
#define init
...
trace("Current Daily:");
trace("Mods=BadEnd,Carmageddon,bossbar,B,Team Attack");
trace("Character:Choose");
trace("Crown:crown of hatred");
trace("Weapon:super bazooka");
trace("Seed:14814");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("super bazooka"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:BadEnd,Carmageddon,bossbar,B,Team Attack" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:super bazooka" + " Seed:14814";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-24 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-24 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-24 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-24 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-24 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:BadEnd,Carmageddon,bossbar,B,Team Attack" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:super bazooka" + " Seed:14814" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":BadEnd,Carmageddon,bossbar,B,Team Attack" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:super bazooka" + " Seed:14814",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
