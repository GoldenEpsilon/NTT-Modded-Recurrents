...
/*@rMods@w=#  Bullet Time#  Chicken Head#  Chaos Guns#  Adjustable Ammo#  Confuse#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  Blunderbuss#@pSeed@w=24667#Limited Tries:false*/
/*|Bullet Time[The closer you are to bullets#the slower time moves]|Chicken Head[Chicken can now carry her head.#Gross.]|Chaos Guns[For when you need a bit#more chaos in your life.]|Adjustable Ammo[Push the limits#of ammo storage!]|Confuse[I guess you had a bit#too much to drink last night, eh?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bullet Time,Chicken Head,Chaos Guns,Adjustable Ammo,Confuse");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bullet Time,Chicken Head,Chaos Guns,Adjustable Ammo,Confuse" + " Character:Choose" + " Crown:none" + " Weapon:Blunderbuss" + " Seed:24667";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-10 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-10 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-10 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-10 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-10 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bullet Time,Chicken Head,Chaos Guns,Adjustable Ammo,Confuse" + " Character:Choose" + " Crown:none" + " Weapon:Blunderbuss" + " Seed:24667" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bullet Time,Chicken Head,Chaos Guns,Adjustable Ammo,Confuse" + " Character:Choose" + " Crown:none" + " Weapon:Blunderbuss" + " Seed:24667",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
