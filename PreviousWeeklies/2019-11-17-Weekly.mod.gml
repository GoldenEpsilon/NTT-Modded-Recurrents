...
/*@rMods@w=#  Cosmi#  Brimthrone#  Babylon Shotgun#  Hard Mode#  Confuse#@bCharacter@w=#  cosmi#@yCrown@w=#  none#@gWeapon@w=#  venuz shotgun#@pSeed@w=31982#Limited Tries:false*/
/*|Cosmi[*insert *insert *insert self insert joke here* joke here* joke here*]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Babylon Shotgun[So much destruction your enemies will babble-on]|Hard Mode[Blaac's Hard Mode.#Good luck.#@r@q@qYou'll need it.]|Confuse[I guess you had a bit#too much to drink last night, eh?]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/confuse.mod.gml"));
...
trace("Mods=Cosmi,Brimthrone,Babylon Shotgun,Hard Mode,Confuse");
trace("Character:cosmi");
trace("Crown:none");
trace("Weapon:venuz shotgun");
trace("Seed:31982");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("venuz shotgun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("cosmi")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("cosmi"))){
					instance_destroy();
				}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cosmi,Brimthrone,Babylon Shotgun,Hard Mode,Confuse" + " Character:cosmi" + " Crown:none" + " Weapon:venuz shotgun" + " Seed:31982";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-17 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-11-17 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-11-17 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-17 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-11-17 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cosmi,Brimthrone,Babylon Shotgun,Hard Mode,Confuse" + " Character:cosmi" + " Crown:none" + " Weapon:venuz shotgun" + " Seed:31982" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cosmi,Brimthrone,Babylon Shotgun,Hard Mode,Confuse" + " Character:cosmi" + " Crown:none" + " Weapon:venuz shotgun" + " Seed:31982",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("cosmi");}else{global.Race = race;}}
