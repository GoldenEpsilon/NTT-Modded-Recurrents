...
/*@rMods@w=#  Chips#  Derpweps#  Bigger Bandit#  Cool Flame#  Crypt Of The Necrodancer#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  thebolt#@pSeed@w=13776#Limited Tries:false*/
/*|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Derpweps[Derrrrrrrrrrrp]|Bigger Bandit[Ever gotten bored of Big Dog?]|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Crypt Of The Necrodancer[Move to the beat!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chips,Derpweps,Bigger Bandit,Cool Flame,Crypt Of The Necrodancer");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chips,Derpweps,Bigger Bandit,Cool Flame,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:thebolt" + " Seed:13776";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-20 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chips,Derpweps,Bigger Bandit,Cool Flame,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:thebolt" + " Seed:13776" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chips,Derpweps,Bigger Bandit,Cool Flame,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:thebolt" + " Seed:13776",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
