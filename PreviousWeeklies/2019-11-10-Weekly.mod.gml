...
/*@rMods@w=#  Charge Template 2#  Ascension 15#  Babylon Shotgun#  Dummy#  Cool Dark#@bCharacter@w=#  dummy#@yCrown@w=#  crown of death#@gWeapon@w=#  Babylon Shotgun#@pSeed@w=3951#Limited Tries:false*/
/*|Charge Template 2[Why aren't people more descriptive, like#"a shotgun that uses a pistol sprite that shoots#more bullets the more you charge that#happens to be a charge template"?]|Ascension 15[@qASCENSION LEVEL#@q15/15]|Babylon Shotgun[So much destruction your enemies will babble-on]|Dummy[No strings on me!]|Cool Dark[spoopy time]*/
#define init
...
trace("Mods=Charge Template 2,Ascension 15,Babylon Shotgun,Dummy,Cool Dark");
trace("Character:dummy");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Charge Template 2,Ascension 15,Babylon Shotgun,Dummy,Cool Dark" + " Character:dummy" + " Crown:crown of death" + " Weapon:Babylon Shotgun" + " Seed:3951";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-10 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-11-10 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-11-10 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-10 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-11-10 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Charge Template 2,Ascension 15,Babylon Shotgun,Dummy,Cool Dark" + " Character:dummy" + " Crown:crown of death" + " Weapon:Babylon Shotgun" + " Seed:3951" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Charge Template 2,Ascension 15,Babylon Shotgun,Dummy,Cool Dark" + " Character:dummy" + " Crown:crown of death" + " Weapon:Babylon Shotgun" + " Seed:3951",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("dummy");}else{global.Race = race;}}
