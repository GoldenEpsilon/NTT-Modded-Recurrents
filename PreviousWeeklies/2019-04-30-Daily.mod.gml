...
/*@rMods@w=#  Auto Bullet Shotgun#  Blood#  Big Bandit#  6 Weapons#  Elite Turrets 2#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=5936#Limited Tries:false*/
/*|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Blood[If Nuclear Throne had an M rating...]|Big Bandit['Nuff said.]|6 Weapons[Gives you one weapon slot#for each ammo type like DOOM]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Auto Bullet Shotgun,Blood,Big Bandit,6 Weapons,Elite Turrets 2");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Bullet Shotgun,Blood,Big Bandit,6 Weapons,Elite Turrets 2" + " Character:Choose" + " Crown:none" + " Weapon:Auto Bullet Shotgun" + " Seed:5936";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Bullet Shotgun,Blood,Big Bandit,6 Weapons,Elite Turrets 2" + " Character:Choose" + " Crown:none" + " Weapon:Auto Bullet Shotgun" + " Seed:5936" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Bullet Shotgun,Blood,Big Bandit,6 Weapons,Elite Turrets 2" + " Character:Choose" + " Crown:none" + " Weapon:Auto Bullet Shotgun" + " Seed:5936",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
