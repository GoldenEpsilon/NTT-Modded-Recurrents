...
/*@rMods@w=#  Bomber#  Airhorn#  Some Crowns#  bow#  !ARMAGGEDNN#@bCharacter@w=#  Choose#@yCrown@w=#  plague#@gWeapon@w=#  ded#@pSeed@w=21564#Limited Tries:false*/
/*|Bomber[Bombs?]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Some Crowns[Adds... Some Crowns.#dunno what you expected]|bow[It's a bow.]|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bomber,Airhorn,Some Crowns,bow,!ARMAGGEDNN");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bomber,Airhorn,Some Crowns,bow,!ARMAGGEDNN" + " Character:Choose" + " Crown:plague" + " Weapon:ded" + " Seed:21564";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-04 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bomber,Airhorn,Some Crowns,bow,!ARMAGGEDNN" + " Character:Choose" + " Crown:plague" + " Weapon:ded" + " Seed:21564" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bomber,Airhorn,Some Crowns,bow,!ARMAGGEDNN" + " Character:Choose" + " Crown:plague" + " Weapon:ded" + " Seed:21564",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
