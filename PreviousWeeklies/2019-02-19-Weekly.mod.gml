...
trace("Mods=Level Balls, MS Paint Shader, Freak Arena, Bloom Shader, Bandit Cannon");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Level Balls, MS Paint Shader, Freak Arena, Bloom Shader, Bandit Cannon" + " Character=char_rebel" + " Crown=crwn_none" + " Weapon=wep_golden_slugger" + " Seed=4562";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 4) == "404:"){
			...
			"content":"'+base64("Mods:Level Balls, MS Paint Shader, Freak Arena, Bloom Shader, Bandit Cannon" + " Character=char_rebel" + " Crown=crwn_none" + " Weapon=wep_golden_slugger" + " Seed=4562" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else{
			file_delete("sha.txt");
