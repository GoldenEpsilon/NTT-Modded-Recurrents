...
trace("Mods=Level Balls, Fuzzy Shader, Bloom Shader, Paper Mario, MS Paint Shader");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Level Balls, Fuzzy Shader, Bloom Shader, Paper Mario, MS Paint Shader" + " Character=char_chicken" + " Crown=crwn_guns" + " Weapon=wep_plasma_rifle" + " Seed=16871";
	}
	trace(score);
	...
	if(global.qualified){
		...
		file_download("https://raw.githubusercontent.com/GoldenEpsilon/NTT-Modded-Recurrents/master/leaderboards/weekly.txt", "dl.txt");
		...
		if(string_copy(leaderboard, 0, 4) == "404:"){
			...
			"content":"'+base64("Mods:Level Balls, Fuzzy Shader, Bloom Shader, Paper Mario, MS Paint Shader" + " Character=char_chicken" + " Crown=crwn_guns" + " Weapon=wep_plasma_rifle" + " Seed=16871" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else{
			file_delete("sha.txt");
