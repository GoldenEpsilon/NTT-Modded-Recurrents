...
trace("Mods=Lots O' Muts, Bloom Shader, MS Paint Shader, Level Balls, NTT X10 Ver 1.0.1");
trace("Character=char_eyes");
trace("Crown=crwn_curses");
trace("Weapon=wep_ultra_shovel");
trace("Seed=472");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Lots O' Muts, Bloom Shader, MS Paint Shader, Level Balls, NTT X10 Ver 1.0.1" + " Character=char_eyes" + " Crown=crwn_curses" + " Weapon=wep_ultra_shovel" + " Seed=472";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 4) == "404:"){
			...
			"content":"'+base64("Mods:Lots O' Muts, Bloom Shader, MS Paint Shader, Level Balls, NTT X10 Ver 1.0.1" + " Character=char_eyes" + " Crown=crwn_curses" + " Weapon=wep_ultra_shovel" + " Seed=472" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else{
			file_delete("sha.txt");
