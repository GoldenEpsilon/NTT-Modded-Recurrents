...
/*@rMods@w=#  Bandit#  Assault Flak Cannon#  Drama Kill#  Chips#  Cosmi Gun Enemies#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  Assault Flak Cannon#@pSeed@w=15998#Limited Tries:false*/
/*|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]|Drama Kill[For when you want#to feel really good#for defeating Big Bandit]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bandit,Assault Flak Cannon,Drama Kill,Chips,Cosmi Gun Enemies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bandit,Assault Flak Cannon,Drama Kill,Chips,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Assault Flak Cannon" + " Seed:15998";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-16 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bandit,Assault Flak Cannon,Drama Kill,Chips,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Assault Flak Cannon" + " Seed:15998" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bandit,Assault Flak Cannon,Drama Kill,Chips,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Assault Flak Cannon" + " Seed:15998",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
