...
/*@rMods@w=#  Dummy#  Ascension 13#  !ARMAGGEDNN#  basicgunallies#  Doctor#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  M16-Rifle#@pSeed@w=31876#Limited Tries:false*/
/*|Dummy[No strings on me!]|Ascension 13[@qASCENSION LEVEL#15/15]|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dummy,Ascension 13,!ARMAGGEDNN,basicgunallies,Doctor");
trace("Character:Choose");
trace("Crown:crown of life");
trace("Weapon:M16-Rifle");
trace("Seed:31876");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("M16-Rifle"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dummy,Ascension 13,!ARMAGGEDNN,basicgunallies,Doctor" + " Character:Choose" + " Crown:crown of life" + " Weapon:M16-Rifle" + " Seed:31876";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-12 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-12 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-12 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-12 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-12 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dummy,Ascension 13,!ARMAGGEDNN,basicgunallies,Doctor" + " Character:Choose" + " Crown:crown of life" + " Weapon:M16-Rifle" + " Seed:31876" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dummy,Ascension 13,!ARMAGGEDNN,basicgunallies,Doctor" + " Character:Choose" + " Crown:crown of life" + " Weapon:M16-Rifle" + " Seed:31876",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
