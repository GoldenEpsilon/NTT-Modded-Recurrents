...
/*@rMods@w=#  Colored Palace#  Charge Template#  Car#  xefsstuff#  banditstack#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  thunder cloud minigun#@pSeed@w=8837#Limited Tries:false*/
/*|Colored Palace[Gives a palace a#well-deserved paint job.]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]|xefsstuff[This has a bit of everything.#A bunch of weapons, a few characters,#a butler robot#buddy thingamijig that kills guys...#@qDON'T GIVE HIM A GRENADE LAUNCHER OH N-]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]*/
#define init
...
trace("Current Daily:");
trace("Mods=Colored Palace,Charge Template,Car,xefsstuff,banditstack");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Colored Palace,Charge Template,Car,xefsstuff,banditstack" + " Character:Choose" + " Crown:crown of risk" + " Weapon:thunder cloud minigun" + " Seed:8837";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-27 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Colored Palace,Charge Template,Car,xefsstuff,banditstack" + " Character:Choose" + " Crown:crown of risk" + " Weapon:thunder cloud minigun" + " Seed:8837" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Colored Palace,Charge Template,Car,xefsstuff,banditstack" + " Character:Choose" + " Crown:crown of risk" + " Weapon:thunder cloud minigun" + " Seed:8837",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
