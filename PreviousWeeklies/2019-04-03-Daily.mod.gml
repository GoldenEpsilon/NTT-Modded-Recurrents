...
/*@rMods@w=#  brad#  Dogfly#  DIO#  Dasher x10#  Area Health#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  nowep#@pSeed@w=6124#Limited Tries:false*/
/*|brad[Adds Brad from LISA: the Painful RPG]|Dogfly[The dog guardians#learned to jump good.]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Dasher x10[@qEXTREME DASHING]|Area Health[A nice mod that shows how many enemies are left.]*/
#define init
...
trace("Current Daily:");
trace("Mods=brad,Dogfly,DIO,Dasher x10,Area Health");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:brad,Dogfly,DIO,Dasher x10,Area Health" + " Character:Choose" + " Crown:crown of haste" + " Weapon:nowep" + " Seed:6124";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:brad,Dogfly,DIO,Dasher x10,Area Health" + " Character:Choose" + " Crown:crown of haste" + " Weapon:nowep" + " Seed:6124" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":brad,Dogfly,DIO,Dasher x10,Area Health" + " Character:Choose" + " Crown:crown of haste" + " Weapon:nowep" + " Seed:6124",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
