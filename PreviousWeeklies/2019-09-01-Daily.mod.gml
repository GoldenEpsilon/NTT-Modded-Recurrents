...
/*@rMods@w=#  brad#  Colored Palace#  B#  Defect#  CoD Proper#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  nowep#@pSeed@w=5728#Limited Tries:false*/
/*|brad[Adds Brad from LISA: the Painful RPG]|Colored Palace[Gives a palace a#well-deserved paint job.]|B[B]|Defect[BEEP BOOP I AM ROBOT]*/
#define init
...
trace("Current Daily:");
trace("Mods=brad,Colored Palace,B,Defect,CoD Proper");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:brad,Colored Palace,B,Defect,CoD Proper" + " Character:Choose" + " Crown:crown of curses" + " Weapon:nowep" + " Seed:5728";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:brad,Colored Palace,B,Defect,CoD Proper" + " Character:Choose" + " Crown:crown of curses" + " Weapon:nowep" + " Seed:5728" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":brad,Colored Palace,B,Defect,CoD Proper" + " Character:Choose" + " Crown:crown of curses" + " Weapon:nowep" + " Seed:5728",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
