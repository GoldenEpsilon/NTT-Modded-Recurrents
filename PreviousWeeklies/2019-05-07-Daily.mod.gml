...
/*@rMods@w=#  Drama Kill#  Druid#  bosshudredux#  Double Maxigun#  Babylon Shotgun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  Double Maxigun#@pSeed@w=9108#Limited Tries:false*/
/*|Drama Kill[For when you want#to feel really good#for defeating Big Bandit]|Druid[Not Getafix,#but he'll do.]|bosshudredux[For knowing how bad your weapons are,#every once in a while]|Double Maxigun[Because miniguns#aren't cool enough]|Babylon Shotgun[So much destruction your enemies will babble-on]*/
#define init
...
trace("Current Daily:");
trace("Mods=Drama Kill,Druid,bosshudredux,Double Maxigun,Babylon Shotgun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Drama Kill,Druid,bosshudredux,Double Maxigun,Babylon Shotgun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Double Maxigun" + " Seed:9108";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-07 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-07 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-07 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-07 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-07 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Drama Kill,Druid,bosshudredux,Double Maxigun,Babylon Shotgun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Double Maxigun" + " Seed:9108" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Drama Kill,Druid,bosshudredux,Double Maxigun,Babylon Shotgun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Double Maxigun" + " Seed:9108",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
