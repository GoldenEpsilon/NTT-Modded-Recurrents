...
/*@rMods@w=#  Champions#  Beamer#  bouncybolts#  Dasher#  Doctor#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  Beamer#@pSeed@w=14754#Limited Tries:false*/
/*|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Beamer[BZAAAAAP]|bouncybolts[Upgrade to Bolt Marrow]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Champions,Beamer,bouncybolts,Dasher,Doctor");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Champions,Beamer,bouncybolts,Dasher,Doctor" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Beamer" + " Seed:14754";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-11 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Champions,Beamer,bouncybolts,Dasher,Doctor" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Beamer" + " Seed:14754" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Champions,Beamer,bouncybolts,Dasher,Doctor" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Beamer" + " Seed:14754",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
