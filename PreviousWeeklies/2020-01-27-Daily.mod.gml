...
/*@rMods@w=#  Cool Dark#  Crazy Allies#  Director#  Dogfly#  Drop Me Gun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  hyper rifle#@pSeed@w=12565#Limited Tries:false*/
/*|Cool Dark[spoopy time]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Director[He will direct your enemies#to the nearest exit of life]|Dogfly[The dog guardians#learned to jump good.]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Dark,Crazy Allies,Director,Dogfly,Drop Me Gun");
trace("Character:Choose");
trace("Crown:crown of luck");
trace("Weapon:hyper rifle");
trace("Seed:12565");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("hyper rifle"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Dark,Crazy Allies,Director,Dogfly,Drop Me Gun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyper rifle" + " Seed:12565";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-27 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Dark,Crazy Allies,Director,Dogfly,Drop Me Gun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyper rifle" + " Seed:12565" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Dark,Crazy Allies,Director,Dogfly,Drop Me Gun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyper rifle" + " Seed:12565",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
