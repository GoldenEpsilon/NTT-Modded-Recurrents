...
/*@rMods@w=#  Bloodking#  Bandit#  blstorm#  Colored Palace#  Car#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  blstorm#@pSeed@w=15426#Limited Tries:false*/
/*|Bloodking[Blood, pain, and a half-finished character.]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|blstorm[BLEED YOURSELF DRY]|Colored Palace[Gives a palace a#well-deserved paint job.]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bloodking,Bandit,blstorm,Colored Palace,Car");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bloodking,Bandit,blstorm,Colored Palace,Car" + " Character:Choose" + " Crown:crown of haste" + " Weapon:blstorm" + " Seed:15426";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-04 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bloodking,Bandit,blstorm,Colored Palace,Car" + " Character:Choose" + " Crown:crown of haste" + " Weapon:blstorm" + " Seed:15426" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bloodking,Bandit,blstorm,Colored Palace,Car" + " Character:Choose" + " Crown:crown of haste" + " Weapon:blstorm" + " Seed:15426",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
