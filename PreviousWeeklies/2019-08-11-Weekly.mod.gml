...
/*@rMods@w=#  bossbar#  Cool Shadows#  Cosmi Gun Enemies#  Crazy Traps#  Armageddon Cannon#@bCharacter@w=#  frog#@yCrown@w=#  crown of guns#@gWeapon@w=#  Armageddon Cannon#@pSeed@w=29014#Limited Tries:false*/
/*|bossbar[For when you want to know#just how close you were to looping]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|Armageddon Cannon[Some people just want to watch the world burn...]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/CosmiGunEnemies.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/crazytraps.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Armageddon_Cannon.wep.gml"));
...
trace("Mods=bossbar,Cool Shadows,Cosmi Gun Enemies,Crazy Traps,Armageddon Cannon");
trace("Character:frog");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Armageddon Cannon"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("frog")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("frog"))){
					instance_destroy();
				}
				exit;
			}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bossbar,Cool Shadows,Cosmi Gun Enemies,Crazy Traps,Armageddon Cannon" + " Character:frog" + " Crown:crown of guns" + " Weapon:Armageddon Cannon" + " Seed:29014";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-11 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-08-11 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-08-11 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-11 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-08-11 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bossbar,Cool Shadows,Cosmi Gun Enemies,Crazy Traps,Armageddon Cannon" + " Character:frog" + " Crown:crown of guns" + " Weapon:Armageddon Cannon" + " Seed:29014" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bossbar,Cool Shadows,Cosmi Gun Enemies,Crazy Traps,Armageddon Cannon" + " Character:frog" + " Crown:crown of guns" + " Weapon:Armageddon Cannon" + " Seed:29014",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("frog");}else{global.Race = race;}}
