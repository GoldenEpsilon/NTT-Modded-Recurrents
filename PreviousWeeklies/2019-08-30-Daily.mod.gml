...
/*@rMods@w=#  Cosmi#  Chips#  Defect#  Cool Shadows#  Ancient Hatred#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  flame shotgun#@pSeed@w=9359#Limited Tries:false*/
/*|Cosmi[*insert *insert *insert self insert joke here* joke here* joke here*]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Defect[BEEP BOOP I AM ROBOT]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Ancient Hatred[Crown that trades health for rads over time]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cosmi,Chips,Defect,Cool Shadows,Ancient Hatred");
trace("Character:Choose");
trace("Crown:crown of ancient hatred");
trace("Weapon:flame shotgun");
trace("Seed:9359");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("flame shotgun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cosmi,Chips,Defect,Cool Shadows,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:flame shotgun" + " Seed:9359";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cosmi,Chips,Defect,Cool Shadows,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:flame shotgun" + " Seed:9359" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cosmi,Chips,Defect,Cool Shadows,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:flame shotgun" + " Seed:9359",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
