...
/*@rMods@w=#  Carbine#  Double Plasma Minigun#  Crazy Traps#  Derpweps#  Carmageddon#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  doubleplasma#@pSeed@w=25899#Limited Tries:false*/
/*|Carbine[Pew Pew.]|Double Plasma Minigun[For when you want to#go through even more ammo]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|Derpweps[Derrrrrrrrrrrp]|Carmageddon[WARNING: May contain drifting]*/
#define init
...
trace("Current Daily:");
trace("Mods=Carbine,Double Plasma Minigun,Crazy Traps,Derpweps,Carmageddon");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carbine,Double Plasma Minigun,Crazy Traps,Derpweps,Carmageddon" + " Character:Choose" + " Crown:crown of blood" + " Weapon:doubleplasma" + " Seed:25899";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-03 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carbine,Double Plasma Minigun,Crazy Traps,Derpweps,Carmageddon" + " Character:Choose" + " Crown:crown of blood" + " Weapon:doubleplasma" + " Seed:25899" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carbine,Double Plasma Minigun,Crazy Traps,Derpweps,Carmageddon" + " Character:Choose" + " Crown:crown of blood" + " Weapon:doubleplasma" + " Seed:25899",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
