...
/*@rMods@w=#  Bigger Bandit#  Director#  Discs#  Ancient Hatred#  Cycle#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  Mini Disc Gun#@pSeed@w=31528#Limited Tries:false*/
/*|Bigger Bandit[Ever gotten bored of Big Dog?]|Director[He will direct your enemies#to the nearest exit of life]|Discs[Because, you know, we need#MORE discs killing us.]|Ancient Hatred[Crown that trades health for rads over time]|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bigger Bandit,Director,Discs,Ancient Hatred,Cycle");
trace("Character:Choose");
trace("Crown:crown of ancient hatred");
trace("Weapon:Mini Disc Gun");
trace("Seed:31528");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Mini Disc Gun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bigger Bandit,Director,Discs,Ancient Hatred,Cycle" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Mini Disc Gun" + " Seed:31528";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-23 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-23 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-23 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-23 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-23 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bigger Bandit,Director,Discs,Ancient Hatred,Cycle" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Mini Disc Gun" + " Seed:31528" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bigger Bandit,Director,Discs,Ancient Hatred,Cycle" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Mini Disc Gun" + " Seed:31528",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
