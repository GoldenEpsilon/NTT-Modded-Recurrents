...
/*@rMods@w=#  Cannoner#  Elite Turrets#  Dasher#  Auto Flame Crossbow#  Carpocalypse#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  cannoner#@pSeed@w=13495#Limited Tries:false*/
/*|Cannoner[EIGHT RADICAL SHOTS]|Elite Turrets[V.2.0.maybe-we-#got-all-the-bugs-#this-time]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Auto Flame Crossbow[If flames were good this would be better than just looking cool...]|Carpocalypse[Beep Beep, motherf***er]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cannoner,Elite Turrets,Dasher,Auto Flame Crossbow,Carpocalypse");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cannoner,Elite Turrets,Dasher,Auto Flame Crossbow,Carpocalypse" + " Character:Choose" + " Crown:crown of blood" + " Weapon:cannoner" + " Seed:13495";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-31 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-31 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-31 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-31 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-31 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cannoner,Elite Turrets,Dasher,Auto Flame Crossbow,Carpocalypse" + " Character:Choose" + " Crown:crown of blood" + " Weapon:cannoner" + " Seed:13495" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cannoner,Elite Turrets,Dasher,Auto Flame Crossbow,Carpocalypse" + " Character:Choose" + " Crown:crown of blood" + " Weapon:cannoner" + " Seed:13495",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
