...
/*@rMods@w=#  Egg#  Dasher x10#  Ascension 5#  Ascension 7#  Adjustable Ammo#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  black sword#@pSeed@w=11482#Limited Tries:false*/
/*|Egg[You will GIVE#me an @qEGG]|Dasher x10[@qEXTREME DASHING]|Ascension 5[ASCENSION LEVEL#5/15]|Ascension 7[ASCENSION LEVEL#7/15]|Adjustable Ammo[Push the limits#of ammo storage!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Egg,Dasher x10,Ascension 5,Ascension 7,Adjustable Ammo");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Egg,Dasher x10,Ascension 5,Ascension 7,Adjustable Ammo" + " Character:Choose" + " Crown:crown of risk" + " Weapon:black sword" + " Seed:11482";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-12 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-12 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-12 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-12 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-12 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Egg,Dasher x10,Ascension 5,Ascension 7,Adjustable Ammo" + " Character:Choose" + " Crown:crown of risk" + " Weapon:black sword" + " Seed:11482" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Egg,Dasher x10,Ascension 5,Ascension 7,Adjustable Ammo" + " Character:Choose" + " Crown:crown of risk" + " Weapon:black sword" + " Seed:11482",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
