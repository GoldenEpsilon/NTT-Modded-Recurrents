...
/*@rMods@w=#  B#  Cool Trails#  Confuse#  Egg#  Fire Riders#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  hyper rifle#@pSeed@w=29088#Limited Tries:false*/
/*|B[B]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Confuse[I guess you had a bit#too much to drink last night, eh?]|Egg[You will GIVE#me an @qEGG]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]*/
#define init
...
trace("Current Daily:");
trace("Mods=B,Cool Trails,Confuse,Egg,Fire Riders");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:B,Cool Trails,Confuse,Egg,Fire Riders" + " Character:Choose" + " Crown:crown of risk" + " Weapon:hyper rifle" + " Seed:29088";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-25 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:B,Cool Trails,Confuse,Egg,Fire Riders" + " Character:Choose" + " Crown:crown of risk" + " Weapon:hyper rifle" + " Seed:29088" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":B,Cool Trails,Confuse,Egg,Fire Riders" + " Character:Choose" + " Crown:crown of risk" + " Weapon:hyper rifle" + " Seed:29088",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
