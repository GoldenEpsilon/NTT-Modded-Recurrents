...
/*@rMods@w=#  Chaos Guns#  Confuse#  Carbine#  Damage Numbers#  Boltshard#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Blunderbuss#@pSeed@w=21548#Limited Tries:false*/
/*|Chaos Guns[For when you need a bit#more chaos in your life.]|Confuse[I guess you had a bit#too much to drink last night, eh?]|Carbine[Pew Pew.]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chaos Guns,Confuse,Carbine,Damage Numbers,Boltshard");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chaos Guns,Confuse,Carbine,Damage Numbers,Boltshard" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Blunderbuss" + " Seed:21548";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-04 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chaos Guns,Confuse,Carbine,Damage Numbers,Boltshard" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Blunderbuss" + " Seed:21548" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chaos Guns,Confuse,Carbine,Damage Numbers,Boltshard" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Blunderbuss" + " Seed:21548",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
