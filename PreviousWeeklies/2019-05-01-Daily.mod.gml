...
/*@rMods@w=#  Blaac's Weapon Mutations#  Cool Trails#  Crazy Bandits#  Doctor#  Elite Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  elitemini#@pSeed@w=21311#Limited Tries:false*/
/*|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Crazy Bandits[THEY HAVE GUNS]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]|Elite Minigun[Man, it feels nice#using their own guns on them.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Blaac's Weapon Mutations,Cool Trails,Crazy Bandits,Doctor,Elite Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Blaac's Weapon Mutations,Cool Trails,Crazy Bandits,Doctor,Elite Minigun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:elitemini" + " Seed:21311";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Blaac's Weapon Mutations,Cool Trails,Crazy Bandits,Doctor,Elite Minigun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:elitemini" + " Seed:21311" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Blaac's Weapon Mutations,Cool Trails,Crazy Bandits,Doctor,Elite Minigun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:elitemini" + " Seed:21311",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
