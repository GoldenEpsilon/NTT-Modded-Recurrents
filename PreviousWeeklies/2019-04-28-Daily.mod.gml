...
/*@rMods@w=#  Cool Flame#  Fire Riders#  Cooler Hud#  Disk Marrow#  BAR#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  Big Ass Revolver#@pSeed@w=32309#Limited Tries:false*/
/*|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Cooler Hud[Symphony of the Throne]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|BAR[A Big Ass Revolver.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Flame,Fire Riders,Cooler Hud,Disk Marrow,BAR");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Flame,Fire Riders,Cooler Hud,Disk Marrow,BAR" + " Character:Choose" + " Crown:crown of life" + " Weapon:Big Ass Revolver" + " Seed:32309";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-28 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Flame,Fire Riders,Cooler Hud,Disk Marrow,BAR" + " Character:Choose" + " Crown:crown of life" + " Weapon:Big Ass Revolver" + " Seed:32309" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Flame,Fire Riders,Cooler Hud,Disk Marrow,BAR" + " Character:Choose" + " Crown:crown of life" + " Weapon:Big Ass Revolver" + " Seed:32309",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
