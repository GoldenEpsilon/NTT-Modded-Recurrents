...
/*@rMods@w=#  Cannondemnation#  Some Crowns#  Chaos Guns#  xefsstuff#  Blood#@bCharacter@w=#  Choose#@yCrown@w=#  torment#@gWeapon@w=#  cloud pistol#@pSeed@w=20490#Limited Tries:false*/
/*|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Some Crowns[Adds... Some Crowns.#dunno what you expected]|Chaos Guns[For when you need a bit#more chaos in your life.]|xefsstuff[This has a bit of everything.#A bunch of weapons, a few characters,#a butler robot#buddy thingamijig that kills guys...#@qDON'T GIVE HIM A GRENADE LAUNCHER OH N-]|Blood[If Nuclear Throne had an M rating...]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cannondemnation,Some Crowns,Chaos Guns,xefsstuff,Blood");
trace("Character:Choose");
trace("Crown:torment");
trace("Weapon:cloud pistol");
trace("Seed:20490");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("cloud pistol"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cannondemnation,Some Crowns,Chaos Guns,xefsstuff,Blood" + " Character:Choose" + " Crown:torment" + " Weapon:cloud pistol" + " Seed:20490";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-27 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cannondemnation,Some Crowns,Chaos Guns,xefsstuff,Blood" + " Character:Choose" + " Crown:torment" + " Weapon:cloud pistol" + " Seed:20490" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cannondemnation,Some Crowns,Chaos Guns,xefsstuff,Blood" + " Character:Choose" + " Crown:torment" + " Weapon:cloud pistol" + " Seed:20490",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
