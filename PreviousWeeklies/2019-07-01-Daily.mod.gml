...
/*@rMods@w=#  Brimthrone#  Chips#  Cool Shadows#  Charge Template#  brad#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  brimthrone#@pSeed@w=8401#Limited Tries:false*/
/*|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|brad[Adds Brad from LISA: the Painful RPG]*/
#define init
...
trace("Current Daily:");
trace("Mods=Brimthrone,Chips,Cool Shadows,Charge Template,brad");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Brimthrone,Chips,Cool Shadows,Charge Template,brad" + " Character:Choose" + " Crown:crown of guns" + " Weapon:brimthrone" + " Seed:8401";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Brimthrone,Chips,Cool Shadows,Charge Template,brad" + " Character:Choose" + " Crown:crown of guns" + " Weapon:brimthrone" + " Seed:8401" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Brimthrone,Chips,Cool Shadows,Charge Template,brad" + " Character:Choose" + " Crown:crown of guns" + " Weapon:brimthrone" + " Seed:8401",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
