...
/*@rMods@w=#  blackholes#  Beamer#  Chips#  6 Weapons#  Doctor#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  Beamer#@pSeed@w=10983#Limited Tries:false*/
/*|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Beamer[BZAAAAAP]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|6 Weapons[Gives you one weapon slot#for each ammo type like DOOM]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]*/
#define init
...
trace("Current Daily:");
trace("Mods=blackholes,Beamer,Chips,6 Weapons,Doctor");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:blackholes,Beamer,Chips,6 Weapons,Doctor" + " Character:Choose" + " Crown:crown of love" + " Weapon:Beamer" + " Seed:10983";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:blackholes,Beamer,Chips,6 Weapons,Doctor" + " Character:Choose" + " Crown:crown of love" + " Weapon:Beamer" + " Seed:10983" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":blackholes,Beamer,Chips,6 Weapons,Doctor" + " Character:Choose" + " Crown:crown of love" + " Weapon:Beamer" + " Seed:10983",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
