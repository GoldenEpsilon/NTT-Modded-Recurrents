...
/*@rMods@w=#  basicgunallies#  Double Maxigun#  Bioluminary Oscillator#  Cool Dark#  Minecraft Area#@bCharacter@w=#  fish#@yCrown@w=#  crown of haste#@gWeapon@w=#  Double Maxigun#@pSeed@w=16455#Limited Tries:false*/
/*|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|Double Maxigun[Because miniguns#aren't cool enough]|Bioluminary Oscillator[TARGET AQUIRED]|Cool Dark[spoopy time]|Minecraft Area[Who Love Minecraf]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/double maxigun/Double Maxigun.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/Bioluminary_Oscillator.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/cooldark.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/2 hour area/2hours.area.gml"));
...
trace("Mods=basicgunallies,Double Maxigun,Bioluminary Oscillator,Cool Dark,Minecraft Area");
trace("Character:fish");
trace("Crown:crown of haste");
trace("Weapon:Double Maxigun");
trace("Seed:16455");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Double Maxigun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("fish")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("fish"))){
					instance_destroy();
				}
				exit;
			}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:basicgunallies,Double Maxigun,Bioluminary Oscillator,Cool Dark,Minecraft Area" + " Character:fish" + " Crown:crown of haste" + " Weapon:Double Maxigun" + " Seed:16455";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-07 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-07-07 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-07-07 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-07 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-07-07 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:basicgunallies,Double Maxigun,Bioluminary Oscillator,Cool Dark,Minecraft Area" + " Character:fish" + " Crown:crown of haste" + " Weapon:Double Maxigun" + " Seed:16455" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":basicgunallies,Double Maxigun,Bioluminary Oscillator,Cool Dark,Minecraft Area" + " Character:fish" + " Crown:crown of haste" + " Weapon:Double Maxigun" + " Seed:16455",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("fish");}else{global.Race = race;}}
