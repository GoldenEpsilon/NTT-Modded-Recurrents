...
/*@rMods@w=#  Dasher#  Carmageddon#  Cursed Hyper Crystal#  Carbine#  Car#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  carbine#@pSeed@w=5112#Limited Tries:false*/
/*|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Carmageddon[WARNING: May contain drifting]|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]|Carbine[Pew Pew.]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dasher,Carmageddon,Cursed Hyper Crystal,Carbine,Car");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dasher,Carmageddon,Cursed Hyper Crystal,Carbine,Car" + " Character:Choose" + " Crown:crown of haste" + " Weapon:carbine" + " Seed:5112";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-14 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dasher,Carmageddon,Cursed Hyper Crystal,Carbine,Car" + " Character:Choose" + " Crown:crown of haste" + " Weapon:carbine" + " Seed:5112" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dasher,Carmageddon,Cursed Hyper Crystal,Carbine,Car" + " Character:Choose" + " Crown:crown of haste" + " Weapon:carbine" + " Seed:5112",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
