...
/*@rMods@w=#  boys#  Colored Palace#  Auto Bullet Shotgun#  Death Pit#  Double Time#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=28706#Limited Tries:false*/
/*|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Colored Palace[Gives a palace a#well-deserved paint job.]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Double Time[GOTTAGOFAST]*/
#define init
...
trace("Current Daily:");
trace("Mods=boys,Colored Palace,Auto Bullet Shotgun,Death Pit,Double Time");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:boys,Colored Palace,Auto Bullet Shotgun,Death Pit,Double Time" + " Character:Choose" + " Crown:deathpit" + " Weapon:Auto Bullet Shotgun" + " Seed:28706";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-26 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:boys,Colored Palace,Auto Bullet Shotgun,Death Pit,Double Time" + " Character:Choose" + " Crown:deathpit" + " Weapon:Auto Bullet Shotgun" + " Seed:28706" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":boys,Colored Palace,Auto Bullet Shotgun,Death Pit,Double Time" + " Character:Choose" + " Crown:deathpit" + " Weapon:Auto Bullet Shotgun" + " Seed:28706",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
