...
/*@rMods@w=#  !ARMAGGEDNN#  B#  bossbar#  blackholes#  Area Health#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Shielder Heavy Gun#@pSeed@w=387#Limited Tries:false*/
/*|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]|B[B]|bossbar[For when you want to know#just how close you were to looping]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Area Health[A nice mod that shows how many enemies are left.]*/
#define init
...
trace("Current Daily:");
trace("Mods=!ARMAGGEDNN,B,bossbar,blackholes,Area Health");
trace("Character:Choose");
trace("Crown:crown of death");
trace("Weapon:Shielder Heavy Gun");
trace("Seed:387");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Shielder Heavy Gun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:!ARMAGGEDNN,B,bossbar,blackholes,Area Health" + " Character:Choose" + " Crown:crown of death" + " Weapon:Shielder Heavy Gun" + " Seed:387";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-31 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-31 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-31 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-31 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-31 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:!ARMAGGEDNN,B,bossbar,blackholes,Area Health" + " Character:Choose" + " Crown:crown of death" + " Weapon:Shielder Heavy Gun" + " Seed:387" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":!ARMAGGEDNN,B,bossbar,blackholes,Area Health" + " Character:Choose" + " Crown:crown of death" + " Weapon:Shielder Heavy Gun" + " Seed:387",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
