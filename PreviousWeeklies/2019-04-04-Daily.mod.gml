...
/*@rMods@w=#  Auto Bullet Shotgun#  Detailer#  Crazy Allies#  Baldi#  BadEnd#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  Baldi/ultraruler#@pSeed@w=11577#Limited Tries:false*/
/*|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Detailer[Adds pretty patches of things]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]|BadEnd[For when you don't want a good end.#(sit after loop 3)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Auto Bullet Shotgun,Detailer,Crazy Allies,Baldi,BadEnd");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Bullet Shotgun,Detailer,Crazy Allies,Baldi,BadEnd" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Baldi/ultraruler" + " Seed:11577";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-04 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Bullet Shotgun,Detailer,Crazy Allies,Baldi,BadEnd" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Baldi/ultraruler" + " Seed:11577" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Bullet Shotgun,Detailer,Crazy Allies,Baldi,BadEnd" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Baldi/ultraruler" + " Seed:11577",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
