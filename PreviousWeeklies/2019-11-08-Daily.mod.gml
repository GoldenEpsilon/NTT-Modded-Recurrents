...
/*@rMods@w=#  Detonator#  Cannoner#  Croc Man#  Babylon Shotgun#  Baller#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  cannoner#@pSeed@w=24235#Limited Tries:false*/
/*|Detonator[Active has a radius#where when you release#it blows up your bullets]|Cannoner[EIGHT RADICAL SHOTS]|Croc Man[From what I can tell you either#don't take damage from anything or#you die instantly because of that passive.]|Babylon Shotgun[So much destruction your enemies will babble-on]|Baller[Fire burns...#me like fire...]*/
#define init
...
trace("Current Daily:");
trace("Mods=Detonator,Cannoner,Croc Man,Babylon Shotgun,Baller");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Detonator,Cannoner,Croc Man,Babylon Shotgun,Baller" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:24235";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-08 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Detonator,Cannoner,Croc Man,Babylon Shotgun,Baller" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:24235" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Detonator,Cannoner,Croc Man,Babylon Shotgun,Baller" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:24235",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
