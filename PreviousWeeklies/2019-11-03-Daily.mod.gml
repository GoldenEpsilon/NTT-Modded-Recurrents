...
/*@rMods@w=#  Dasher#  Crazy Traps#  Big Dog Snipers#  Carmageddon#  Auto Bullet Shotgun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=4574#Limited Tries:false*/
/*|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|Big Dog Snipers[The actual boss]|Carmageddon[WARNING: May contain drifting]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dasher,Crazy Traps,Big Dog Snipers,Carmageddon,Auto Bullet Shotgun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dasher,Crazy Traps,Big Dog Snipers,Carmageddon,Auto Bullet Shotgun" + " Character:Choose" + " Crown:crown of life" + " Weapon:Auto Bullet Shotgun" + " Seed:4574";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dasher,Crazy Traps,Big Dog Snipers,Carmageddon,Auto Bullet Shotgun" + " Character:Choose" + " Crown:crown of life" + " Weapon:Auto Bullet Shotgun" + " Seed:4574" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dasher,Crazy Traps,Big Dog Snipers,Carmageddon,Auto Bullet Shotgun" + " Character:Choose" + " Crown:crown of life" + " Weapon:Auto Bullet Shotgun" + " Seed:4574",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
