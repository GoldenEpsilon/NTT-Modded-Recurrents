...
/*@rMods@w=#  basicgunallies#  Armageddon Cannon#  Grappler#  Adjustable Ammo#  Elite Gamers#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  Armageddon Cannon#@pSeed@w=22079#Limited Tries:false*/
/*|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|Armageddon Cannon[Some people just want to watch the world burn...]|Grappler[For when you really want to throw someone at someone else.]|Adjustable Ammo[Push the limits#of ammo storage!]|Elite Gamers[The bad guys got#aiming classes]*/
#define init
...
trace("Current Daily:");
trace("Mods=basicgunallies,Armageddon Cannon,Grappler,Adjustable Ammo,Elite Gamers");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:basicgunallies,Armageddon Cannon,Grappler,Adjustable Ammo,Elite Gamers" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Armageddon Cannon" + " Seed:22079";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-10 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-10 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-10 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-10 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-10 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:basicgunallies,Armageddon Cannon,Grappler,Adjustable Ammo,Elite Gamers" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Armageddon Cannon" + " Seed:22079" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":basicgunallies,Armageddon Cannon,Grappler,Adjustable Ammo,Elite Gamers" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Armageddon Cannon" + " Seed:22079",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
