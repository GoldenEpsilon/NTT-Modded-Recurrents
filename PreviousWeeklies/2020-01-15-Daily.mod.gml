...
/*@rMods@w=#  Double Pop Gun#  Elite Turrets 2#  Doctor#  Beamer#  Druid#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  double_pop_gun#@pSeed@w=30187#Limited Tries:false*/
/*|Double Pop Gun[Now you can tickle enemies#twice as fast!]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]|Beamer[BZAAAAAP]|Druid[Not Getafix,#but he'll do.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Pop Gun,Elite Turrets 2,Doctor,Beamer,Druid");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Pop Gun,Elite Turrets 2,Doctor,Beamer,Druid" + " Character:Choose" + " Crown:crown of haste" + " Weapon:double_pop_gun" + " Seed:30187";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-15 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Pop Gun,Elite Turrets 2,Doctor,Beamer,Druid" + " Character:Choose" + " Crown:crown of haste" + " Weapon:double_pop_gun" + " Seed:30187" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Pop Gun,Elite Turrets 2,Doctor,Beamer,Druid" + " Character:Choose" + " Crown:crown of haste" + " Weapon:double_pop_gun" + " Seed:30187",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
