...
/*@rMods@w=#  Dummy#  Belly#  Elite Turrets 2#  Cooler Hud#  Double Plasma Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  doubleplasma#@pSeed@w=4747#Limited Tries:false*/
/*|Dummy[No strings on me!]|Belly[The only survivor of the#meme that is melee actives]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Cooler Hud[Symphony of the Throne]|Double Plasma Minigun[For when you want to#go through even more ammo]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dummy,Belly,Elite Turrets 2,Cooler Hud,Double Plasma Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dummy,Belly,Elite Turrets 2,Cooler Hud,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:doubleplasma" + " Seed:4747";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-07 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-07 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-07 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-07 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-07 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dummy,Belly,Elite Turrets 2,Cooler Hud,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:doubleplasma" + " Seed:4747" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dummy,Belly,Elite Turrets 2,Cooler Hud,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:doubleplasma" + " Seed:4747",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
