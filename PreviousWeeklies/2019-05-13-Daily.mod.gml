...
/*@rMods@w=#  Babylon Shotgun#  bouncybolts#  Blood#  Area Health#  Blindness#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  Babylon Shotgun#@pSeed@w=2639#Limited Tries:false*/
/*|Babylon Shotgun[So much destruction your enemies will babble-on]|bouncybolts[Upgrade to Bolt Marrow]|Blood[If Nuclear Throne had an M rating...]|Area Health[A nice mod that shows how many enemies are left.]|Blindness[MY GLASSES! I BROKE MY GLASSES!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Babylon Shotgun,bouncybolts,Blood,Area Health,Blindness");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Babylon Shotgun,bouncybolts,Blood,Area Health,Blindness" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Babylon Shotgun" + " Seed:2639";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-13 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Babylon Shotgun,bouncybolts,Blood,Area Health,Blindness" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Babylon Shotgun" + " Seed:2639" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Babylon Shotgun,bouncybolts,Blood,Area Health,Blindness" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Babylon Shotgun" + " Seed:2639",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
