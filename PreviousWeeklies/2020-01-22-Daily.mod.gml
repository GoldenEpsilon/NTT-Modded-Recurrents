...
/*@rMods@w=#  Cannondemnation#  Brimthrone#  Cursed Hyper Crystal#  blackholes#  Egg#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  cannondemnation#@pSeed@w=31464#Limited Tries:false*/
/*|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Egg[You will GIVE#me an @qEGG]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cannondemnation,Brimthrone,Cursed Hyper Crystal,blackholes,Egg");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cannondemnation,Brimthrone,Cursed Hyper Crystal,blackholes,Egg" + " Character:Choose" + " Crown:crown of guns" + " Weapon:cannondemnation" + " Seed:31464";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-22 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cannondemnation,Brimthrone,Cursed Hyper Crystal,blackholes,Egg" + " Character:Choose" + " Crown:crown of guns" + " Weapon:cannondemnation" + " Seed:31464" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cannondemnation,Brimthrone,Cursed Hyper Crystal,blackholes,Egg" + " Character:Choose" + " Crown:crown of guns" + " Weapon:cannondemnation" + " Seed:31464",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
