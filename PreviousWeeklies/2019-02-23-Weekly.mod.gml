...
//@rMods@w=#  BadEnd#  6 Weapons#  Assault Flak Cannon#  Baller#  !ARMAGGEDNN#@bCharacter@w=#  Baller#@yCrown@w=#  crwn_haste#@gWeapon@w=#  wep_eraser#@pSeed@w=24833
#define init
...
mod_load(string_trim("data/Weekly.mod/baller.race.gml"));
...
trace("Mods=BadEnd, 6 Weapons, Assault Flak Cannon, Baller, !ARMAGGEDNN");
trace("Character=Baller");
trace("Crown=crwn_haste");
trace("Weapon=wep_eraser");
trace("Seed=24833");
...
else{with(instances_matching(CustomObject, "name", mod_current)){instance_change(CharSelect, 0);}with(CharSelect){race=Baller;visible=true;}}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:BadEnd, 6 Weapons, Assault Flak Cannon, Baller, !ARMAGGEDNN" + " Character=Baller" + " Crown=crwn_haste" + " Weapon=wep_eraser" + " Seed=24833";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:BadEnd, 6 Weapons, Assault Flak Cannon, Baller, !ARMAGGEDNN" + " Character=Baller" + " Crown=crwn_haste" + " Weapon=wep_eraser" + " Seed=24833" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(leaderboard, "Mods:BadEnd, 6 Weapons, Assault Flak Cannon, Baller, !ARMAGGEDNN" + " Character=Baller" + " Crown=crwn_haste")) > 1){
			file_delete("sha.txt");
