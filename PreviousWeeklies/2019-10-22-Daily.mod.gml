...
/*@rMods@w=#  Ancient Hatred#  Cool Hud#  Bitpack#  Cooler Hud#  bossbar#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  industrialshankgun#@pSeed@w=30779#Limited Tries:false*/
/*|Ancient Hatred[Crown that trades health for rads over time]|Cool Hud[*C418 starts playing*]|Bitpack[idk, bitflux made this]|Cooler Hud[Symphony of the Throne]|bossbar[For when you want to know#just how close you were to looping]*/
#define init
...
downloadmod("ancienthatred.crown.gml");
...
wait mod_load(string_trim("data/Daily.mod/bossbar.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Ancient Hatred,Cool Hud,Bitpack,Cooler Hud,bossbar");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ancient Hatred,Cool Hud,Bitpack,Cooler Hud,bossbar" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:industrialshankgun" + " Seed:30779";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-22 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ancient Hatred,Cool Hud,Bitpack,Cooler Hud,bossbar" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:industrialshankgun" + " Seed:30779" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ancient Hatred,Cool Hud,Bitpack,Cooler Hud,bossbar" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:industrialshankgun" + " Seed:30779",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
