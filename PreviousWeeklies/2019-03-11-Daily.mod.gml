...
/*@rMods@w=#  Auto Flame Crossbow#  Chest Bandits#  Assault Flak Cannon#  Brimthrone#  Car#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  brimthrone#@pSeed@w=11950#Limited Tries:false*/
/*|Auto Flame Crossbow[If flames were good this would be better than just looking cool...]|Chest Bandits[Gets rid of bandits in random areas]|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/chestbandits.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Auto Flame Crossbow,Chest Bandits,Assault Flak Cannon,Brimthrone,Car");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Flame Crossbow,Chest Bandits,Assault Flak Cannon,Brimthrone,Car" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:brimthrone" + " Seed:11950";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Flame Crossbow,Chest Bandits,Assault Flak Cannon,Brimthrone,Car" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:brimthrone" + " Seed:11950" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Flame Crossbow,Chest Bandits,Assault Flak Cannon,Brimthrone,Car" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:brimthrone" + " Seed:11950",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
