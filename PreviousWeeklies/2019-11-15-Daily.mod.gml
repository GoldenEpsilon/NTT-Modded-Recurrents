...
/*@rMods@w=#  AllSkills#  Druid#  Balls#  Auto Flame Crossbow#  Armageddon Cannon#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  laserwrench#@pSeed@w=30084#Limited Tries:false*/
/*|AllSkills[Congratulations!#You just got the perfect#mutation set.]|Druid[Not Getafix,#but he'll do.]|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]|Auto Flame Crossbow[If flames were good this would be better than just looking cool...]|Armageddon Cannon[Some people just want to watch the world burn...]*/
#define init
...
trace("Current Daily:");
trace("Mods=AllSkills,Druid,Balls,Auto Flame Crossbow,Armageddon Cannon");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:AllSkills,Druid,Balls,Auto Flame Crossbow,Armageddon Cannon" + " Character:Choose" + " Crown:crown of guns" + " Weapon:laserwrench" + " Seed:30084";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:AllSkills,Druid,Balls,Auto Flame Crossbow,Armageddon Cannon" + " Character:Choose" + " Crown:crown of guns" + " Weapon:laserwrench" + " Seed:30084" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":AllSkills,Druid,Balls,Auto Flame Crossbow,Armageddon Cannon" + " Character:Choose" + " Crown:crown of guns" + " Weapon:laserwrench" + " Seed:30084",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
