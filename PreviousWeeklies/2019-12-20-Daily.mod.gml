...
/*@rMods@w=#  Chrononaut#  Belly#  Ascension 13#  bow#  Buff#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  idpdslugpistol#@pSeed@w=538#Limited Tries:false*/
/*|Chrononaut[Wibbley Wobbley#Timey Wimey]|Belly[The only survivor of the#meme that is melee actives]|Ascension 13[@qASCENSION LEVEL#15/15]|bow[It's a bow.]|Buff[Buffs to various weapons,#ultras, mutations,#and more]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chrononaut,Belly,Ascension 13,bow,Buff");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chrononaut,Belly,Ascension 13,bow,Buff" + " Character:Choose" + " Crown:crown of love" + " Weapon:idpdslugpistol" + " Seed:538";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-20 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chrononaut,Belly,Ascension 13,bow,Buff" + " Character:Choose" + " Crown:crown of love" + " Weapon:idpdslugpistol" + " Seed:538" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chrononaut,Belly,Ascension 13,bow,Buff" + " Character:Choose" + " Crown:crown of love" + " Weapon:idpdslugpistol" + " Seed:538",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
