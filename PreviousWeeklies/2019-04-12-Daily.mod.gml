...
/*@rMods@w=#  Armageddon Cannon#  bow#  boys#  Ancient Hatred#  Elite Turrets 2#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  bow#@pSeed@w=28126#Limited Tries:false*/
/*|Armageddon Cannon[Some people just want to watch the world burn...]|bow[It's a bow.]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Ancient Hatred[Crown that trades health for rads over time]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Armageddon Cannon,bow,boys,Ancient Hatred,Elite Turrets 2");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Armageddon Cannon,bow,boys,Ancient Hatred,Elite Turrets 2" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:bow" + " Seed:28126";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-12 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-12 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-12 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-12 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-12 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Armageddon Cannon,bow,boys,Ancient Hatred,Elite Turrets 2" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:bow" + " Seed:28126" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Armageddon Cannon,bow,boys,Ancient Hatred,Elite Turrets 2" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:bow" + " Seed:28126",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
