...
/*@rMods@w=#  Fire Riders#  Minecraft Area#  Cool Trails#  Corsair#  Double Plasma Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Corsair#@pSeed@w=11882#Limited Tries:false*/
/*|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Minecraft Area[Who Love Minecraf]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Corsair[The Corsair from EtG#Plot A Course]|Double Plasma Minigun[For when you want to#go through even more ammo]*/
#define init
...
trace("Current Daily:");
trace("Mods=Fire Riders,Minecraft Area,Cool Trails,Corsair,Double Plasma Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Fire Riders,Minecraft Area,Cool Trails,Corsair,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:Corsair" + " Seed:11882";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Fire Riders,Minecraft Area,Cool Trails,Corsair,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:Corsair" + " Seed:11882" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Fire Riders,Minecraft Area,Cool Trails,Corsair,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:Corsair" + " Seed:11882",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
