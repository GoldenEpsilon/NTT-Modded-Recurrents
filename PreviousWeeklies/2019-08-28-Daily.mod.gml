...
/*@rMods@w=#  Elite Dasher#  Dasher#  Area Health#  Double Pop Gun#  Elite Gamers#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  double_pop_gun#@pSeed@w=11695#Limited Tries:false*/
/*|Elite Dasher[@qPICK UP ALL THE THINGS]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Area Health[A nice mod that shows how many enemies are left.]|Double Pop Gun[Now you can tickle enemies#twice as fast!]|Elite Gamers[The bad guys got#aiming classes]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Dasher,Dasher,Area Health,Double Pop Gun,Elite Gamers");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Dasher,Dasher,Area Health,Double Pop Gun,Elite Gamers" + " Character:Choose" + " Crown:none" + " Weapon:double_pop_gun" + " Seed:11695";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-28 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Dasher,Dasher,Area Health,Double Pop Gun,Elite Gamers" + " Character:Choose" + " Crown:none" + " Weapon:double_pop_gun" + " Seed:11695" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Dasher,Dasher,Area Health,Double Pop Gun,Elite Gamers" + " Character:Choose" + " Crown:none" + " Weapon:double_pop_gun" + " Seed:11695",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
