...
/*@rMods@w=#  Chaos Guns#  BAR#  Grappler#  Elite Turrets 2#  bouncybolts#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  Chaos Minigun#@pSeed@w=12622#Limited Tries:false*/
/*|Chaos Guns[For when you need a bit#more chaos in your life.]|BAR[A Big Ass Revolver.]|Grappler[For when you really want to throw someone at someone else.]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|bouncybolts[Upgrade to Bolt Marrow]*/
#define init
...
while (!file_loaded(string_trim(data+"Chaos Guns/Blunderbuss.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"Chaos Guns/Super Blunderbuss.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"Chaos Guns/Chaos Minigun.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"Chaos Guns/Super Chaos Minigun.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"Chaos Guns/Hyper Chaos Minigun.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"BAR.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"grappler.race.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"EliteTurret2.mod.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"bouncybolts.mod.gml"))) {wait 1;}
...
trace("Current Daily:");
trace("Mods=Chaos Guns,BAR,Grappler,Elite Turrets 2,bouncybolts");
trace("Character:Choose");
trace("Crown:crown of guns");
trace("Weapon:Chaos Minigun");
trace("Seed:12622");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Chaos Minigun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chaos Guns,BAR,Grappler,Elite Turrets 2,bouncybolts" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Chaos Minigun" + " Seed:12622";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-22 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chaos Guns,BAR,Grappler,Elite Turrets 2,bouncybolts" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Chaos Minigun" + " Seed:12622" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chaos Guns,BAR,Grappler,Elite Turrets 2,bouncybolts" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Chaos Minigun" + " Seed:12622",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
