...
/*@rMods@w=#  Cool Flame#  Charge Template#  Damage Numbers#  Bullet Time#  Cannoner#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  cannoner#@pSeed@w=22670#Limited Tries:false*/
/*|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Bullet Time[The closer you are to bullets#the slower time moves]|Cannoner[EIGHT RADICAL SHOTS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Flame,Charge Template,Damage Numbers,Bullet Time,Cannoner");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Flame,Charge Template,Damage Numbers,Bullet Time,Cannoner" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:22670";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-05 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Flame,Charge Template,Damage Numbers,Bullet Time,Cannoner" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:22670" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Flame,Charge Template,Damage Numbers,Bullet Time,Cannoner" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:22670",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
