...
/*@rMods@w=#  Double Plasma Minigun#  Belly#  Crazy Bandits#  boys#  Cool Dark#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  doubleplasma#@pSeed@w=3554#Limited Tries:false*/
/*|Double Plasma Minigun[For when you want to#go through even more ammo]|Belly[The only survivor of the#meme that is melee actives]|Crazy Bandits[THEY HAVE GUNS]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Cool Dark[spoopy time]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/cooldark.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Double Plasma Minigun,Belly,Crazy Bandits,boys,Cool Dark");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Plasma Minigun,Belly,Crazy Bandits,boys,Cool Dark" + " Character:Choose" + " Crown:crown of death" + " Weapon:doubleplasma" + " Seed:3554";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-26 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Plasma Minigun,Belly,Crazy Bandits,boys,Cool Dark" + " Character:Choose" + " Crown:crown of death" + " Weapon:doubleplasma" + " Seed:3554" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Plasma Minigun,Belly,Crazy Bandits,boys,Cool Dark" + " Character:Choose" + " Crown:crown of death" + " Weapon:doubleplasma" + " Seed:3554",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
