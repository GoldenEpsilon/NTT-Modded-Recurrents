...
/*@rMods@w=#  CoDeath#  Camper Boys#  Bioluminary Oscillator#  Double Plasma Minigun#  Cursed Hyper Crystal#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  doubleplasma#@pSeed@w=1396#Limited Tries:false*/
/*|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]|Bioluminary Oscillator[TARGET AQUIRED]|Double Plasma Minigun[For when you want to#go through even more ammo]|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]*/
#define init
...
trace("Current Daily:");
trace("Mods=CoDeath,Camper Boys,Bioluminary Oscillator,Double Plasma Minigun,Cursed Hyper Crystal");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:CoDeath,Camper Boys,Bioluminary Oscillator,Double Plasma Minigun,Cursed Hyper Crystal" + " Character:Choose" + " Crown:CoDeath" + " Weapon:doubleplasma" + " Seed:1396";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-16 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:CoDeath,Camper Boys,Bioluminary Oscillator,Double Plasma Minigun,Cursed Hyper Crystal" + " Character:Choose" + " Crown:CoDeath" + " Weapon:doubleplasma" + " Seed:1396" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":CoDeath,Camper Boys,Bioluminary Oscillator,Double Plasma Minigun,Cursed Hyper Crystal" + " Character:Choose" + " Crown:CoDeath" + " Weapon:doubleplasma" + " Seed:1396",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
