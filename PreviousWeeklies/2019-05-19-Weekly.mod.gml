...
/*@rMods@w=#  Elite Rifle#  Big Bandit#  bow#  Cool Trails#  Brimthrone#@bCharacter@w=#  BigBandit#@yCrown@w=#  crown of death#@gWeapon@w=#  brimthrone#@pSeed@w=350#Limited Tries:false*/
/*|Elite Rifle[I'm just gonna grab that,#thank you very much.]|Big Bandit['Nuff said.]|bow[It's a bow.]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/Big Bandit/bigbandit.race.gml"));
wait mod_load(string_trim("data/Weekly.mod/Big Bandit/bigrifle.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/bow.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/cooltrails.mod.gml"));
...
trace("Mods=Elite Rifle,Big Bandit,bow,Cool Trails,Brimthrone");
trace("Character:BigBandit");
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("BigBandit")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("BigBandit"))){
					instance_destroy();
				}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Rifle,Big Bandit,bow,Cool Trails,Brimthrone" + " Character:BigBandit" + " Crown:crown of death" + " Weapon:brimthrone" + " Seed:350";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-19 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-05-19 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-05-19 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-19 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-05-19 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Rifle,Big Bandit,bow,Cool Trails,Brimthrone" + " Character:BigBandit" + " Crown:crown of death" + " Weapon:brimthrone" + " Seed:350" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Rifle,Big Bandit,bow,Cool Trails,Brimthrone" + " Character:BigBandit" + " Crown:crown of death" + " Weapon:brimthrone" + " Seed:350",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race="BigBandit";}else{global.Race = race;}}
