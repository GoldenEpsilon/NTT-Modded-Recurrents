...
/*@rMods@w=#  B#  Elite Minigun#  C Ultras#  Dasher#  Ascension 10#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  elitemini#@pSeed@w=6271#Limited Tries:false*/
/*|B[B]|Elite Minigun[Man, it feels nice#using their own guns on them.]|C Ultras[Adds a C ultra to all#the vanilla characters]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Ascension 10[ASCENSION LEVEL#10/15]*/
#define init
...
trace("Current Daily:");
trace("Mods=B,Elite Minigun,C Ultras,Dasher,Ascension 10");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:B,Elite Minigun,C Ultras,Dasher,Ascension 10" + " Character:Choose" + " Crown:crown of risk" + " Weapon:elitemini" + " Seed:6271";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-31 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-31 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-31 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-31 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-31 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:B,Elite Minigun,C Ultras,Dasher,Ascension 10" + " Character:Choose" + " Crown:crown of risk" + " Weapon:elitemini" + " Seed:6271" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":B,Elite Minigun,C Ultras,Dasher,Ascension 10" + " Character:Choose" + " Crown:crown of risk" + " Weapon:elitemini" + " Seed:6271",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
