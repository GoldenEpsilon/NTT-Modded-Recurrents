...
/*@rMods@w=#  Bomber#  bouncybolts#  Babylon Shotgun#  Double Maxigun#  Confuse#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  Babylon Shotgun#@pSeed@w=26604#Limited Tries:false*/
/*|Bomber[Bombs?]|bouncybolts[Upgrade to Bolt Marrow]|Babylon Shotgun[So much destruction your enemies will babble-on]|Double Maxigun[Because miniguns#aren't cool enough]|Confuse[I guess you had a bit#too much to drink last night, eh?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bomber,bouncybolts,Babylon Shotgun,Double Maxigun,Confuse");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bomber,bouncybolts,Babylon Shotgun,Double Maxigun,Confuse" + " Character:Choose" + " Crown:crown of love" + " Weapon:Babylon Shotgun" + " Seed:26604";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-21 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bomber,bouncybolts,Babylon Shotgun,Double Maxigun,Confuse" + " Character:Choose" + " Crown:crown of love" + " Weapon:Babylon Shotgun" + " Seed:26604" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bomber,bouncybolts,Babylon Shotgun,Double Maxigun,Confuse" + " Character:Choose" + " Crown:crown of love" + " Weapon:Babylon Shotgun" + " Seed:26604",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
