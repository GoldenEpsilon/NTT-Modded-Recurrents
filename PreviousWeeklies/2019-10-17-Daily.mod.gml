...
/*@rMods@w=#  Ash#  Bandit Brute#  Blood#  CoDeath#  Cool Hud#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  sentry gun#@pSeed@w=27103#Limited Tries:false*/
/*|Ash[Guy from The End Is Nigh.]|Bandit Brute[Randomly replaces bandits with Bandit Brutes]|Blood[If Nuclear Throne had an M rating...]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Cool Hud[*C418 starts playing*]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ash,Bandit Brute,Blood,CoDeath,Cool Hud");
trace("Character:Choose");
trace("Crown:CoDeath");
trace("Weapon:sentry gun");
trace("Seed:27103");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("sentry gun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ash,Bandit Brute,Blood,CoDeath,Cool Hud" + " Character:Choose" + " Crown:CoDeath" + " Weapon:sentry gun" + " Seed:27103";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-17 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ash,Bandit Brute,Blood,CoDeath,Cool Hud" + " Character:Choose" + " Crown:CoDeath" + " Weapon:sentry gun" + " Seed:27103" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ash,Bandit Brute,Blood,CoDeath,Cool Hud" + " Character:Choose" + " Crown:CoDeath" + " Weapon:sentry gun" + " Seed:27103",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
