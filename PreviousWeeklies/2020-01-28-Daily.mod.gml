...
/*@rMods@w=#  Cosmi#  Chips#  banditstack#  CoD Proper#  Big Dog Snipers#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  hyper launcher#@pSeed@w=9320#Limited Tries:false*/
/*|Cosmi[*insert *insert *insert self insert joke here* joke here* joke here*]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]|Big Dog Snipers[The actual boss]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cosmi,Chips,banditstack,CoD Proper,Big Dog Snipers");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cosmi,Chips,banditstack,CoD Proper,Big Dog Snipers" + " Character:Choose" + " Crown:crown of blood" + " Weapon:hyper launcher" + " Seed:9320";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-28 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cosmi,Chips,banditstack,CoD Proper,Big Dog Snipers" + " Character:Choose" + " Crown:crown of blood" + " Weapon:hyper launcher" + " Seed:9320" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cosmi,Chips,banditstack,CoD Proper,Big Dog Snipers" + " Character:Choose" + " Crown:crown of blood" + " Weapon:hyper launcher" + " Seed:9320",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
