...
/*@rMods@w=#  Elite Gamers#  Elite Dasher#  Cool Flame#  Buff#  Director#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  smartgun#@pSeed@w=412#Limited Tries:false*/
/*|Elite Gamers[The bad guys got#aiming classes]|Elite Dasher[@qPICK UP ALL THE THINGS]|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Buff[Buffs to various weapons,#ultras, mutations,#and more]|Director[He will direct your enemies#to the nearest exit of life]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Gamers,Elite Dasher,Cool Flame,Buff,Director");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Gamers,Elite Dasher,Cool Flame,Buff,Director" + " Character:Choose" + " Crown:crown of blood" + " Weapon:smartgun" + " Seed:412";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Gamers,Elite Dasher,Cool Flame,Buff,Director" + " Character:Choose" + " Crown:crown of blood" + " Weapon:smartgun" + " Seed:412" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Gamers,Elite Dasher,Cool Flame,Buff,Director" + " Character:Choose" + " Crown:crown of blood" + " Weapon:smartgun" + " Seed:412",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
