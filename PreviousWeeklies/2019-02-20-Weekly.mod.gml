...
//@rMods@w=#  Bloom Shader#  Lots O' Muts#  Level Balls#  Nuke Launcher#  Bandit Cannon#@bCharacter@w=#  char_robot#@yCrown@w=#  crwn_none#@gWeapon@w=#  wep_heavy_grenade_launcher#@pSeed@w=5618
#define init
...
trace("Mods=Bloom Shader, Lots O' Muts, Level Balls, Nuke Launcher, Bandit Cannon");
...
if(!global.canStart){with(CharSelect){instance_change(CustomObject, 0);}}
else{with(CustomObject){instance_change(CharSelect, 0);}with(CharSelect){race=char_robot;visible=true;}}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Bloom Shader, Lots O' Muts, Level Balls, Nuke Launcher, Bandit Cannon" + " Character=char_robot" + " Crown=crwn_none" + " Weapon=wep_heavy_grenade_launcher" + " Seed=5618";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bloom Shader, Lots O' Muts, Level Balls, Nuke Launcher, Bandit Cannon" + " Character=char_robot" + " Crown=crwn_none" + " Weapon=wep_heavy_grenade_launcher" + " Seed=5618" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else{
			file_delete("sha.txt");
