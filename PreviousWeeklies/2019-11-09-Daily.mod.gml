...
/*@rMods@w=#  Drop Me Gun#  Grappler#  Cannoner#  Big Bandit#  Double Plasma Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  cannoner#@pSeed@w=23#Limited Tries:false*/
/*|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Grappler[For when you really want to throw someone at someone else.]|Cannoner[EIGHT RADICAL SHOTS]|Big Bandit['Nuff said.]|Double Plasma Minigun[For when you want to#go through even more ammo]*/
#define init
...
trace("Current Daily:");
trace("Mods=Drop Me Gun,Grappler,Cannoner,Big Bandit,Double Plasma Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Drop Me Gun,Grappler,Cannoner,Big Bandit,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of love" + " Weapon:cannoner" + " Seed:23";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-09 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Drop Me Gun,Grappler,Cannoner,Big Bandit,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of love" + " Weapon:cannoner" + " Seed:23" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Drop Me Gun,Grappler,Cannoner,Big Bandit,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of love" + " Weapon:cannoner" + " Seed:23",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
