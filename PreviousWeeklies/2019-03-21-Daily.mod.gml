...
/*@rMods@w=#  Cosmi Gun Enemies#  Charge Template#  Big Dog#  Challenge Test#  Crazy Bandits#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  ct#@pSeed@w=12691#Limited Tries:false*/
/*|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Big Dog[HE IS AWAKENED]|Challenge Test[GOTTA GO FAST]|Crazy Bandits[THEY HAVE GUNS]*/
#define init
...
downloadmod("challenge_test.mod.gml");
downloadmod("crazybandits.mod.gml");
while (!file_loaded(string_trim(data+"CosmiGunEnemies.mod.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"ct.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"bigdog.mod.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"challenge_test.mod.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"crazybandits.mod.gml"))) {wait 1;}
...
trace("Current Daily:");
trace("Mods=Cosmi Gun Enemies,Charge Template,Big Dog,Challenge Test,Crazy Bandits");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cosmi Gun Enemies,Charge Template,Big Dog,Challenge Test,Crazy Bandits" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:ct" + " Seed:12691";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-21 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cosmi Gun Enemies,Charge Template,Big Dog,Challenge Test,Crazy Bandits" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:ct" + " Seed:12691" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cosmi Gun Enemies,Charge Template,Big Dog,Challenge Test,Crazy Bandits" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:ct" + " Seed:12691",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
