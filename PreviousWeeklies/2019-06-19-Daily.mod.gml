...
/*@rMods@w=#  Carmageddon#  Detailer#  Cooler Hud#  blackholes#  Big Dog Snipers#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  triple machinegun#@pSeed@w=21255#Limited Tries:false*/
/*|Carmageddon[WARNING: May contain drifting]|Detailer[Adds pretty patches of things]|Cooler Hud[Symphony of the Throne]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Big Dog Snipers[The actual boss]*/
#define init
...
trace("Current Daily:");
trace("Mods=Carmageddon,Detailer,Cooler Hud,blackholes,Big Dog Snipers");
trace("Character:Choose");
trace("Crown:crown of haste");
trace("Weapon:triple machinegun");
trace("Seed:21255");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("triple machinegun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carmageddon,Detailer,Cooler Hud,blackholes,Big Dog Snipers" + " Character:Choose" + " Crown:crown of haste" + " Weapon:triple machinegun" + " Seed:21255";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-19 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-19 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-19 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-19 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-19 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carmageddon,Detailer,Cooler Hud,blackholes,Big Dog Snipers" + " Character:Choose" + " Crown:crown of haste" + " Weapon:triple machinegun" + " Seed:21255" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carmageddon,Detailer,Cooler Hud,blackholes,Big Dog Snipers" + " Character:Choose" + " Crown:crown of haste" + " Weapon:triple machinegun" + " Seed:21255",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
