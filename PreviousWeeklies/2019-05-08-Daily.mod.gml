...
/*@rMods@w=#  Airhorn#  Freak Arena#  Chaos Guns#  Blood#  Dasher x10#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  Chaos Minigun#@pSeed@w=5749#Limited Tries:false*/
/*|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Freak Arena[Do you think you can survive#in the @qFREAK ARENA???]|Chaos Guns[For when you need a bit#more chaos in your life.]|Blood[If Nuclear Throne had an M rating...]|Dasher x10[@qEXTREME DASHING]*/
#define init
...
trace("Current Daily:");
trace("Mods=Airhorn,Freak Arena,Chaos Guns,Blood,Dasher x10");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Airhorn,Freak Arena,Chaos Guns,Blood,Dasher x10" + " Character:Choose" + " Crown:crown of life" + " Weapon:Chaos Minigun" + " Seed:5749";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-08 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Airhorn,Freak Arena,Chaos Guns,Blood,Dasher x10" + " Character:Choose" + " Crown:crown of life" + " Weapon:Chaos Minigun" + " Seed:5749" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Airhorn,Freak Arena,Chaos Guns,Blood,Dasher x10" + " Character:Choose" + " Crown:crown of life" + " Weapon:Chaos Minigun" + " Seed:5749",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
