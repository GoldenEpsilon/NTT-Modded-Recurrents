...
/*@rMods@w=#  Bandit Brute#  Drop Me Gun#  Devastator#  Freak Arena#  Druid#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  grenade launcher#@pSeed@w=946#Limited Tries:false*/
/*|Bandit Brute[Randomly replaces bandits with Bandit Brutes]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Devastator[Changes the devastator#to make it more interesting]|Freak Arena[Do you think you can survive#in the @qFREAK ARENA???]|Druid[Not Getafix,#but he'll do.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bandit Brute,Drop Me Gun,Devastator,Freak Arena,Druid");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bandit Brute,Drop Me Gun,Devastator,Freak Arena,Druid" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:grenade launcher" + " Seed:946";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-13 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bandit Brute,Drop Me Gun,Devastator,Freak Arena,Druid" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:grenade launcher" + " Seed:946" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bandit Brute,Drop Me Gun,Devastator,Freak Arena,Druid" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:grenade launcher" + " Seed:946",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
