...
/*@rMods@w=#  Elite Minigun#  Drama Kill#  Bitpack#  Double Maxigun#  Double Pop Gun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  double_pop_gun#@pSeed@w=5050#Limited Tries:false*/
/*|Elite Minigun[Man, it feels nice#using their own guns on them.]|Drama Kill[For when you want#to feel really good#for defeating Big Bandit]|Bitpack[idk, bitflux made this]|Double Maxigun[Because miniguns#aren't cool enough]|Double Pop Gun[Now you can tickle enemies#twice as fast!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Minigun,Drama Kill,Bitpack,Double Maxigun,Double Pop Gun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Minigun,Drama Kill,Bitpack,Double Maxigun,Double Pop Gun" + " Character:Choose" + " Crown:crown of life" + " Weapon:double_pop_gun" + " Seed:5050";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-08 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Minigun,Drama Kill,Bitpack,Double Maxigun,Double Pop Gun" + " Character:Choose" + " Crown:crown of life" + " Weapon:double_pop_gun" + " Seed:5050" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Minigun,Drama Kill,Bitpack,Double Maxigun,Double Pop Gun" + " Character:Choose" + " Crown:crown of life" + " Weapon:double_pop_gun" + " Seed:5050",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
