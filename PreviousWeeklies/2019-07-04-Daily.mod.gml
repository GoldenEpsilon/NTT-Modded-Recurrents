...
/*@rMods@w=#  Detonator#  Director#  B#  Elite Rifle#  CoDeath#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  eliterifle#@pSeed@w=12738#Limited Tries:false*/
/*|Detonator[Active has a radius#where when you release#it blows up your bullets]|Director[He will direct your enemies#to the nearest exit of life]|B[B]|Elite Rifle[I'm just gonna grab that,#thank you very much.]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]*/
#define init
...
trace("Current Daily:");
trace("Mods=Detonator,Director,B,Elite Rifle,CoDeath");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Detonator,Director,B,Elite Rifle,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:eliterifle" + " Seed:12738";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-04 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Detonator,Director,B,Elite Rifle,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:eliterifle" + " Seed:12738" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Detonator,Director,B,Elite Rifle,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:eliterifle" + " Seed:12738",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
