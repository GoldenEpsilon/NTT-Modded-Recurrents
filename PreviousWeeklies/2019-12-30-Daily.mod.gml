...
/*@rMods@w=#  Drop Me Gun#  Blindness#  Cool Flame#  Ascension 5#  Colored Projectiles#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  rogue rifle#@pSeed@w=26979#Limited Tries:false*/
/*|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Blindness[MY GLASSES! I BROKE MY GLASSES!]|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Ascension 5[ASCENSION LEVEL#5/15]|Colored Projectiles[Colors projectiles based on team color]*/
#define init
...
trace("Current Daily:");
trace("Mods=Drop Me Gun,Blindness,Cool Flame,Ascension 5,Colored Projectiles");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Drop Me Gun,Blindness,Cool Flame,Ascension 5,Colored Projectiles" + " Character:Choose" + " Crown:crown of haste" + " Weapon:rogue rifle" + " Seed:26979";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Drop Me Gun,Blindness,Cool Flame,Ascension 5,Colored Projectiles" + " Character:Choose" + " Crown:crown of haste" + " Weapon:rogue rifle" + " Seed:26979" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Drop Me Gun,Blindness,Cool Flame,Ascension 5,Colored Projectiles" + " Character:Choose" + " Crown:crown of haste" + " Weapon:rogue rifle" + " Seed:26979",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
