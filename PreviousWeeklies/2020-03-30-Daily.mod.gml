...
/*@rMods@w=#  Airhorn#  Damage Numbers#  Double Pop Gun#  Ascension 15#  Bioluminary Oscillator#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  double_pop_gun#@pSeed@w=6362#Limited Tries:false*/
/*|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Double Pop Gun[Now you can tickle enemies#twice as fast!]|Ascension 15[@qASCENSION LEVEL#@q15/15]|Bioluminary Oscillator[TARGET AQUIRED]*/
#define init
...
trace("Current Daily:");
trace("Mods=Airhorn,Damage Numbers,Double Pop Gun,Ascension 15,Bioluminary Oscillator");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Airhorn,Damage Numbers,Double Pop Gun,Ascension 15,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of curses" + " Weapon:double_pop_gun" + " Seed:6362";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-30 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Airhorn,Damage Numbers,Double Pop Gun,Ascension 15,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of curses" + " Weapon:double_pop_gun" + " Seed:6362" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Airhorn,Damage Numbers,Double Pop Gun,Ascension 15,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of curses" + " Weapon:double_pop_gun" + " Seed:6362",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
