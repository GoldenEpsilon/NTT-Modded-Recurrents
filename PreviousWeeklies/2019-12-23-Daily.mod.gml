...
/*@rMods@w=#  Defect#  AllCrowns#  Bullet Time#  Chrononaut#  Ascension 13#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  idpdslugpistol#@pSeed@w=25872#Limited Tries:false*/
/*|Defect[BEEP BOOP I AM ROBOT]|AllCrowns[Lets you choose your crown#out of all crowns#at a crown vault.]|Bullet Time[The closer you are to bullets#the slower time moves]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Ascension 13[@qASCENSION LEVEL#15/15]*/
#define init
...
trace("Current Daily:");
trace("Mods=Defect,AllCrowns,Bullet Time,Chrononaut,Ascension 13");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Defect,AllCrowns,Bullet Time,Chrononaut,Ascension 13" + " Character:Choose" + " Crown:crown of guns" + " Weapon:idpdslugpistol" + " Seed:25872";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-23 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-23 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-23 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-23 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-23 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Defect,AllCrowns,Bullet Time,Chrononaut,Ascension 13" + " Character:Choose" + " Crown:crown of guns" + " Weapon:idpdslugpistol" + " Seed:25872" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Defect,AllCrowns,Bullet Time,Chrononaut,Ascension 13" + " Character:Choose" + " Crown:crown of guns" + " Weapon:idpdslugpistol" + " Seed:25872",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
