...
/*@rMods@w=#  Baldi#  Bandit#  bow#  Defect#  Crazy Allies#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  bow#@pSeed@w=974#Limited Tries:false*/
/*|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|bow[It's a bow.]|Defect[BEEP BOOP I AM ROBOT]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]*/
#define init
...
trace("Current Daily:");
trace("Mods=Baldi,Bandit,bow,Defect,Crazy Allies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Baldi,Bandit,bow,Defect,Crazy Allies" + " Character:Choose" + " Crown:crown of luck" + " Weapon:bow" + " Seed:974";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-05 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Baldi,Bandit,bow,Defect,Crazy Allies" + " Character:Choose" + " Crown:crown of luck" + " Weapon:bow" + " Seed:974" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Baldi,Bandit,bow,Defect,Crazy Allies" + " Character:Choose" + " Crown:crown of luck" + " Weapon:bow" + " Seed:974",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
