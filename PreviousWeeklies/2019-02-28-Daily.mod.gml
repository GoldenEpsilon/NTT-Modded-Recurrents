...
/*@rMods@w=#  Carpocalypse#  Dasher x10#  Camper Boys#  Crypt Of The Necrodancer#  banditstack#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  assault slugger#@pSeed@w=11584#Limited Tries:false*/
/*|Carpocalypse[Beep Beep, motherf***er]|Dasher x10[@qEXTREME DASHING]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]|Crypt Of The Necrodancer[Move to the beat!]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]*/
#define init
...
file_delete(string_trim("dasherx10.race.gml"));while (file_exists(string_trim("dasherx10.race.gml"))) {wait 1;}wait(file_download(string_trim("https://raw.githubusercontent.com/GoldenEpsilon/NTT-Modded-Recurrents/master/WeeklyMods/dasherx10.race.gml"), string_trim("dasherx10.race.gml")));
...
while (!file_loaded(string_trim("dasherx10.race.gml"))) {wait 1;}
...
trace("Current Daily:");
trace("Mods=Carpocalypse,Dasher x10,Camper Boys,Crypt Of The Necrodancer,banditstack");
trace("Character=Choose");
trace("Crown=crown of love");
trace("Weapon=assault slugger");
trace("Seed=11584");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("assault slugger"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carpocalypse,Dasher x10,Camper Boys,Crypt Of The Necrodancer,banditstack" + " Character=Choose" + " Crown=crown of love" + " Weapon=assault slugger" + " Seed=11584";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-02-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-02-28 Daily.txt");
		while(!file_loaded(global.alias + " 2019-02-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-02-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-02-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carpocalypse,Dasher x10,Camper Boys,Crypt Of The Necrodancer,banditstack" + " Character=Choose" + " Crown=crown of love" + " Weapon=assault slugger" + " Seed=11584" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all("ods:Carpocalypse,Dasher x10,Camper Boys,Crypt Of The Necrodancer,banditstack" + " Character=Choose" + " Crown=crown of love" + " Weapon=assault slugger" + " Seed=11584",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
