...
/*@pDesc@w=A new direction for#Nuclear Throne Development#@rMods@w=#  NTD3D#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  bouncer shotgun#@pSeed@w=5044#Limited Tries:false*/
/*|*/
#define init
...
downloadmod("NTD3D-april.mod.gml");
wait mod_load(string_trim("data/Event.mod/NTD3D-april.mod.gml"));
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("bouncer shotgun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:NTD3D," + " Character:Choose" + " Crown:crown of blood" + " Weapon:bouncer shotgun" + " Seed:5044";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-01 Event.txt")){
		prevScores = string_load(global.alias + " 2019-04-01 Event.txt");
		while(!file_loaded(global.alias + " 2019-04-01 Event.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-01 Event.txt");
	}else{
		string_save(score, global.alias + " 2019-04-01 Event.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:NTD3D," + " Character:Choose" + " Crown:crown of blood" + " Weapon:bouncer shotgun" + " Seed:5044" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":NTD3D," + " Character:Choose" + " Crown:crown of blood" + " Weapon:bouncer shotgun" + " Seed:5044",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
