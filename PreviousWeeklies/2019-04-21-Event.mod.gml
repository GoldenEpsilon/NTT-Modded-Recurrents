...
trace("Current Event:");
trace("Mods=Egg,Blaac's Weapon Mutations,Crypt Of The Necrodancer");
trace("Character:Choose");
trace("Crown:crown of curses");
trace("Weapon:pop gun");
trace("Seed:18040");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Egg,Blaac's Weapon Mutations,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:crown of curses" + " Weapon:pop gun" + " Seed:18040";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-21 Event.txt")){
		prevScores = string_load(global.alias + " 2019-04-21 Event.txt");
		while(!file_loaded(global.alias + " 2019-04-21 Event.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-21 Event.txt");
	}else{
		string_save(score, global.alias + " 2019-04-21 Event.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Egg,Blaac's Weapon Mutations,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:crown of curses" + " Weapon:pop gun" + " Seed:18040" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Egg,Blaac's Weapon Mutations,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:crown of curses" + " Weapon:pop gun" + " Seed:18040",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
