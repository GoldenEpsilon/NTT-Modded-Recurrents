...
/*@rMods@w=#  Crazy Bandits#  Ancient Hatred#  6 Weapons#  brad#  Cannoner#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  cannoner#@pSeed@w=13924#Limited Tries:false*/
/*|Crazy Bandits[THEY HAVE GUNS]|Ancient Hatred[Crown that trades health for rads over time]|6 Weapons[Gives you one weapon slot#for each ammo type like DOOM]|brad[Adds Brad from LISA: the Painful RPG]|Cannoner[EIGHT RADICAL SHOTS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Crazy Bandits,Ancient Hatred,6 Weapons,brad,Cannoner");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Crazy Bandits,Ancient Hatred,6 Weapons,brad,Cannoner" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:cannoner" + " Seed:13924";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-14 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Crazy Bandits,Ancient Hatred,6 Weapons,brad,Cannoner" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:cannoner" + " Seed:13924" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Crazy Bandits,Ancient Hatred,6 Weapons,brad,Cannoner" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:cannoner" + " Seed:13924",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
