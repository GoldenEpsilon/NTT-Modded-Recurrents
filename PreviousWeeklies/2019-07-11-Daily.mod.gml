...
/*@rMods@w=#  Charge Template#  Some Crowns#  Drama Kill#  blstorm#  Balls#@bCharacter@w=#  Choose#@yCrown@w=#  dogs#@gWeapon@w=#  laserwrench#@pSeed@w=21094#Limited Tries:false*/
/*|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Some Crowns[Adds... Some Crowns.#dunno what you expected]|Drama Kill[For when you want#to feel really good#for defeating Big Bandit]|blstorm[BLEED YOURSELF DRY]|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Charge Template,Some Crowns,Drama Kill,blstorm,Balls");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Charge Template,Some Crowns,Drama Kill,blstorm,Balls" + " Character:Choose" + " Crown:dogs" + " Weapon:laserwrench" + " Seed:21094";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Charge Template,Some Crowns,Drama Kill,blstorm,Balls" + " Character:Choose" + " Crown:dogs" + " Weapon:laserwrench" + " Seed:21094" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Charge Template,Some Crowns,Drama Kill,blstorm,Balls" + " Character:Choose" + " Crown:dogs" + " Weapon:laserwrench" + " Seed:21094",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
