...
/*@rMods@w=#  Doctor#  brad#  Big Dog Snipers#  Adjustable Ammo#  Bandit Brute#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  nowep#@pSeed@w=8632#Limited Tries:false*/
/*|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]|brad[Adds Brad from LISA: the Painful RPG]|Big Dog Snipers[The actual boss]|Adjustable Ammo[Push the limits#of ammo storage!]|Bandit Brute[Randomly replaces bandits with Bandit Brutes]*/
#define init
...
trace("Current Daily:");
trace("Mods=Doctor,brad,Big Dog Snipers,Adjustable Ammo,Bandit Brute");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Doctor,brad,Big Dog Snipers,Adjustable Ammo,Bandit Brute" + " Character:Choose" + " Crown:crown of life" + " Weapon:nowep" + " Seed:8632";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-26 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Doctor,brad,Big Dog Snipers,Adjustable Ammo,Bandit Brute" + " Character:Choose" + " Crown:crown of life" + " Weapon:nowep" + " Seed:8632" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Doctor,brad,Big Dog Snipers,Adjustable Ammo,Bandit Brute" + " Character:Choose" + " Crown:crown of life" + " Weapon:nowep" + " Seed:8632",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
