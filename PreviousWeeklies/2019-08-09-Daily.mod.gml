...
/*@rMods@w=#  Chrononaut#  Crypt Of The Necrodancer#  Ash#  bossbar#  Dodge Roll#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  idpdslugpistol#@pSeed@w=29834#Limited Tries:false*/
/*|Chrononaut[Wibbley Wobbley#Timey Wimey]|Crypt Of The Necrodancer[Move to the beat!]|Ash[Guy from The End Is Nigh.]|bossbar[For when you want to know#just how close you were to looping]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chrononaut,Crypt Of The Necrodancer,Ash,bossbar,Dodge Roll");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chrononaut,Crypt Of The Necrodancer,Ash,bossbar,Dodge Roll" + " Character:Choose" + " Crown:crown of guns" + " Weapon:idpdslugpistol" + " Seed:29834";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-09 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chrononaut,Crypt Of The Necrodancer,Ash,bossbar,Dodge Roll" + " Character:Choose" + " Crown:crown of guns" + " Weapon:idpdslugpistol" + " Seed:29834" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chrononaut,Crypt Of The Necrodancer,Ash,bossbar,Dodge Roll" + " Character:Choose" + " Crown:crown of guns" + " Weapon:idpdslugpistol" + " Seed:29834",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
