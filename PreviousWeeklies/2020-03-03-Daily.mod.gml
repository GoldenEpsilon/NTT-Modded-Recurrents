...
/*@rMods@w=#  Double Laser Minigun#  Cuz#  Cooler Hud#  bossbar#  Ascension 15#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  doublelaser#@pSeed@w=6521#Limited Tries:false*/
/*|Double Laser Minigun[Great for personal rave parties!]|Cuz[Has an energy shield, for some reason.]|Cooler Hud[Symphony of the Throne]|bossbar[For when you want to know#just how close you were to looping]|Ascension 15[@qASCENSION LEVEL#@q15/15]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Laser Minigun,Cuz,Cooler Hud,bossbar,Ascension 15");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Laser Minigun,Cuz,Cooler Hud,bossbar,Ascension 15" + " Character:Choose" + " Crown:crown of curses" + " Weapon:doublelaser" + " Seed:6521";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-03 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Laser Minigun,Cuz,Cooler Hud,bossbar,Ascension 15" + " Character:Choose" + " Crown:crown of curses" + " Weapon:doublelaser" + " Seed:6521" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Laser Minigun,Cuz,Cooler Hud,bossbar,Ascension 15" + " Character:Choose" + " Crown:crown of curses" + " Weapon:doublelaser" + " Seed:6521",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
