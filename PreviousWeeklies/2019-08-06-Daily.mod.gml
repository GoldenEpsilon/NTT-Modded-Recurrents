...
/*@rMods@w=#  Bullet Time#  Cosmi Gun Enemies#  Big Bandit#  Airhorn#  Brimthrone#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  brimthrone#@pSeed@w=6682#Limited Tries:false*/
/*|Bullet Time[The closer you are to bullets#the slower time moves]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]|Big Bandit['Nuff said.]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bullet Time,Cosmi Gun Enemies,Big Bandit,Airhorn,Brimthrone");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bullet Time,Cosmi Gun Enemies,Big Bandit,Airhorn,Brimthrone" + " Character:Choose" + " Crown:crown of protection" + " Weapon:brimthrone" + " Seed:6682";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-06 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bullet Time,Cosmi Gun Enemies,Big Bandit,Airhorn,Brimthrone" + " Character:Choose" + " Crown:crown of protection" + " Weapon:brimthrone" + " Seed:6682" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bullet Time,Cosmi Gun Enemies,Big Bandit,Airhorn,Brimthrone" + " Character:Choose" + " Crown:crown of protection" + " Weapon:brimthrone" + " Seed:6682",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
