...
/*@rMods@w=#  Dasher x10#  Chips#  Blaac's Weapon Mutations#  Detonator B#  Bandit Brute#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  ultra grenade launcher#@pSeed@w=21962#Limited Tries:false*/
/*|Dasher x10[@qEXTREME DASHING]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|Bandit Brute[Randomly replaces bandits with Bandit Brutes]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dasher x10,Chips,Blaac's Weapon Mutations,Detonator B,Bandit Brute");
trace("Character:Choose");
trace("Crown:none");
trace("Weapon:ultra grenade launcher");
trace("Seed:21962");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("ultra grenade launcher"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dasher x10,Chips,Blaac's Weapon Mutations,Detonator B,Bandit Brute" + " Character:Choose" + " Crown:none" + " Weapon:ultra grenade launcher" + " Seed:21962";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-05 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dasher x10,Chips,Blaac's Weapon Mutations,Detonator B,Bandit Brute" + " Character:Choose" + " Crown:none" + " Weapon:ultra grenade launcher" + " Seed:21962" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dasher x10,Chips,Blaac's Weapon Mutations,Detonator B,Bandit Brute" + " Character:Choose" + " Crown:none" + " Weapon:ultra grenade launcher" + " Seed:21962",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
