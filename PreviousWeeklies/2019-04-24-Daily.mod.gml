...
/*@rMods@w=#  Carbine#  Drop Me Gun#  Determination#  Blaac's Weapon Mutations#  Bear#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  carbine#@pSeed@w=31569#Limited Tries:false*/
/*|Carbine[Pew Pew.]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Determination[Seeing this filler text#fills you with determination]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|Bear[4 da meeeeeeeeee#eeeeemez]*/
#define init
...
trace("Current Daily:");
trace("Mods=Carbine,Drop Me Gun,Determination,Blaac's Weapon Mutations,Bear");
trace("Character:Choose");
trace("Crown:crown of haste");
trace("Weapon:carbine");
trace("Seed:31569");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carbine,Drop Me Gun,Determination,Blaac's Weapon Mutations,Bear" + " Character:Choose" + " Crown:crown of haste" + " Weapon:carbine" + " Seed:31569";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-24 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-24 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-24 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-24 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-24 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carbine,Drop Me Gun,Determination,Blaac's Weapon Mutations,Bear" + " Character:Choose" + " Crown:crown of haste" + " Weapon:carbine" + " Seed:31569" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carbine,Drop Me Gun,Determination,Blaac's Weapon Mutations,Bear" + " Character:Choose" + " Crown:crown of haste" + " Weapon:carbine" + " Seed:31569",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
