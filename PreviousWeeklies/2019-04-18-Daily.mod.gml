...
/*@rMods@w=#  Challenge Test#  Drop Me Gun#  Auto Bullet Shotgun#  blstorm#  Elite Rifle#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  eliterifle#@pSeed@w=27876#Limited Tries:false*/
/*|Challenge Test[GOTTA GO FAST]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|blstorm[BLEED YOURSELF DRY]|Elite Rifle[I'm just gonna grab that,#thank you very much.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Challenge Test,Drop Me Gun,Auto Bullet Shotgun,blstorm,Elite Rifle");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Challenge Test,Drop Me Gun,Auto Bullet Shotgun,blstorm,Elite Rifle" + " Character:Choose" + " Crown:crown of guns" + " Weapon:eliterifle" + " Seed:27876";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Challenge Test,Drop Me Gun,Auto Bullet Shotgun,blstorm,Elite Rifle" + " Character:Choose" + " Crown:crown of guns" + " Weapon:eliterifle" + " Seed:27876" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Challenge Test,Drop Me Gun,Auto Bullet Shotgun,blstorm,Elite Rifle" + " Character:Choose" + " Crown:crown of guns" + " Weapon:eliterifle" + " Seed:27876",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
