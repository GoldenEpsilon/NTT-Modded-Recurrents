...
/*@rMods@w=#  Cool Shadows#  Director#  bossbar#  BL2#  Bomber#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  gatling slugger#@pSeed@w=30950#Limited Tries:false*/
/*|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Director[He will direct your enemies#to the nearest exit of life]|bossbar[For when you want to know#just how close you were to looping]|BL2[Borderlands 2 x Nuclear Throne]|Bomber[Bombs?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Shadows,Director,bossbar,BL2,Bomber");
trace("Character:Choose");
trace("Crown:crown of haste");
trace("Weapon:gatling slugger");
trace("Seed:30950");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("gatling slugger"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Shadows,Director,bossbar,BL2,Bomber" + " Character:Choose" + " Crown:crown of haste" + " Weapon:gatling slugger" + " Seed:30950";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-20 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Shadows,Director,bossbar,BL2,Bomber" + " Character:Choose" + " Crown:crown of haste" + " Weapon:gatling slugger" + " Seed:30950" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Shadows,Director,bossbar,BL2,Bomber" + " Character:Choose" + " Crown:crown of haste" + " Weapon:gatling slugger" + " Seed:30950",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
