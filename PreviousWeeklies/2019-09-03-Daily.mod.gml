...
/*@rMods@w=#  Brimthrone#  Elite Dasher#  Big Toothbrush#  Egg#  Auto Revolver#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  Auto Revolver#@pSeed@w=4293#Limited Tries:false*/
/*|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Elite Dasher[@qPICK UP ALL THE THINGS]|Big Toothbrush[a big toothbrush for big mouths]|Egg[You will GIVE#me an @qEGG]|Auto Revolver[A truly auto weapon.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Brimthrone,Elite Dasher,Big Toothbrush,Egg,Auto Revolver");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Brimthrone,Elite Dasher,Big Toothbrush,Egg,Auto Revolver" + " Character:Choose" + " Crown:none" + " Weapon:Auto Revolver" + " Seed:4293";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Brimthrone,Elite Dasher,Big Toothbrush,Egg,Auto Revolver" + " Character:Choose" + " Crown:none" + " Weapon:Auto Revolver" + " Seed:4293" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Brimthrone,Elite Dasher,Big Toothbrush,Egg,Auto Revolver" + " Character:Choose" + " Crown:none" + " Weapon:Auto Revolver" + " Seed:4293",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
