...
/*@rMods@w=#  bouncybolts#  Cycle#  Ash#  Grappler#  Death Pit#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  golden disc gun#@pSeed@w=18592#Limited Tries:false*/
/*|bouncybolts[Upgrade to Bolt Marrow]|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]|Ash[Guy from The End Is Nigh.]|Grappler[For when you really want to throw someone at someone else.]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]*/
#define init
...
trace("Current Daily:");
trace("Mods=bouncybolts,Cycle,Ash,Grappler,Death Pit");
trace("Character:Choose");
trace("Crown:deathpit");
trace("Weapon:golden disc gun");
trace("Seed:18592");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("golden disc gun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bouncybolts,Cycle,Ash,Grappler,Death Pit" + " Character:Choose" + " Crown:deathpit" + " Weapon:golden disc gun" + " Seed:18592";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bouncybolts,Cycle,Ash,Grappler,Death Pit" + " Character:Choose" + " Crown:deathpit" + " Weapon:golden disc gun" + " Seed:18592" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bouncybolts,Cycle,Ash,Grappler,Death Pit" + " Character:Choose" + " Crown:deathpit" + " Weapon:golden disc gun" + " Seed:18592",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
