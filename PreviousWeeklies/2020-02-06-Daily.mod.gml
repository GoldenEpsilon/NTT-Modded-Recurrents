...
/*@rMods@w=#  Champions#  CoD Proper#  Challenge Test#  Detonator B#  Colored Projectiles#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  double minigun#@pSeed@w=8849#Limited Tries:false*/
/*|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Challenge Test[GOTTA GO FAST]|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|Colored Projectiles[Colors projectiles based on team color]*/
#define init
...
trace("Current Daily:");
trace("Mods=Champions,CoD Proper,Challenge Test,Detonator B,Colored Projectiles");
trace("Character:Choose");
trace("Crown:crown of life");
trace("Weapon:double minigun");
trace("Seed:8849");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("double minigun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Champions,CoD Proper,Challenge Test,Detonator B,Colored Projectiles" + " Character:Choose" + " Crown:crown of life" + " Weapon:double minigun" + " Seed:8849";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-06 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Champions,CoD Proper,Challenge Test,Detonator B,Colored Projectiles" + " Character:Choose" + " Crown:crown of life" + " Weapon:double minigun" + " Seed:8849" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Champions,CoD Proper,Challenge Test,Detonator B,Colored Projectiles" + " Character:Choose" + " Crown:crown of life" + " Weapon:double minigun" + " Seed:8849",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
