...
/*@rMods@w=#  Bullet Time#  Big Dog Snipers#  basicgunallies#  Chips#  Cool Shadows#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  chicken sword#@pSeed@w=20772#Limited Tries:false*/
/*|Bullet Time[The closer you are to bullets#the slower time moves]|Big Dog Snipers[The actual boss]|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bullet Time,Big Dog Snipers,basicgunallies,Chips,Cool Shadows");
trace("Character:Choose");
trace("Crown:crown of blood");
trace("Weapon:chicken sword");
trace("Seed:20772");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("chicken sword"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bullet Time,Big Dog Snipers,basicgunallies,Chips,Cool Shadows" + " Character:Choose" + " Crown:crown of blood" + " Weapon:chicken sword" + " Seed:20772";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-13 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bullet Time,Big Dog Snipers,basicgunallies,Chips,Cool Shadows" + " Character:Choose" + " Crown:crown of blood" + " Weapon:chicken sword" + " Seed:20772" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bullet Time,Big Dog Snipers,basicgunallies,Chips,Cool Shadows" + " Character:Choose" + " Crown:crown of blood" + " Weapon:chicken sword" + " Seed:20772",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
