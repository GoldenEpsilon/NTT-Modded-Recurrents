...
/*@rMods@w=#  Doctor#  Ally QOL#  Detonator#  Auto Flame Crossbow#  B#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Auto Flame Crossbow#@pSeed@w=580#Limited Tries:false*/
/*|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]|Ally QOL[Makes allies look nicer.#Because this only#really affects Rebel,#rebel is more likely#for this recurrent.]|Detonator[Active has a radius#where when you release#it blows up your bullets]|Auto Flame Crossbow[If flames were good this would be better than just looking cool...]|B[B]*/
#define init
...
trace("Current Daily:");
trace("Mods=Doctor,Ally QOL,Detonator,Auto Flame Crossbow,B");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Doctor,Ally QOL,Detonator,Auto Flame Crossbow,B" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Auto Flame Crossbow" + " Seed:580";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Doctor,Ally QOL,Detonator,Auto Flame Crossbow,B" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Auto Flame Crossbow" + " Seed:580" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Doctor,Ally QOL,Detonator,Auto Flame Crossbow,B" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Auto Flame Crossbow" + " Seed:580",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
