...
/*@rMods@w=#  Fire Riders#  Detailer#  Bullet Time#  Baldi#  Drop Me Gun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Baldi/ultraruler#@pSeed@w=28466#Limited Tries:false*/
/*|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Detailer[Adds pretty patches of things]|Bullet Time[The closer you are to bullets#the slower time moves]|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Fire Riders,Detailer,Bullet Time,Baldi,Drop Me Gun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Fire Riders,Detailer,Bullet Time,Baldi,Drop Me Gun" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Baldi/ultraruler" + " Seed:28466";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Fire Riders,Detailer,Bullet Time,Baldi,Drop Me Gun" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Baldi/ultraruler" + " Seed:28466" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Fire Riders,Detailer,Bullet Time,Baldi,Drop Me Gun" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Baldi/ultraruler" + " Seed:28466",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
