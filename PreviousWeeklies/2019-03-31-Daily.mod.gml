...
/*@rMods@w=#  Defect#  Double Maxigun#  Vanthro#  bow#  Cannoner#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  bow#@pSeed@w=14434#Limited Tries:false*/
/*|Defect[BEEP BOOP I AM ROBOT]|Double Maxigun[Because miniguns#aren't cool enough]|Vanthro[...blame 4chan.]|bow[It's a bow.]|Cannoner[EIGHT RADICAL SHOTS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Defect,Double Maxigun,Vanthro,bow,Cannoner");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Defect,Double Maxigun,Vanthro,bow,Cannoner" + " Character:Choose" + " Crown:crown of risk" + " Weapon:bow" + " Seed:14434";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-31 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-31 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-31 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-31 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-31 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Defect,Double Maxigun,Vanthro,bow,Cannoner" + " Character:Choose" + " Crown:crown of risk" + " Weapon:bow" + " Seed:14434" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Defect,Double Maxigun,Vanthro,bow,Cannoner" + " Character:Choose" + " Crown:crown of risk" + " Weapon:bow" + " Seed:14434",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
