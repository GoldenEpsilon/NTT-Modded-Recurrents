...
/*@rMods@w=#  Armageddon Cannon#  Chrononaut#  Dasher#  Brimthrone#  Croc Man#@bCharacter@w=#  dasher#@yCrown@w=#  crown of life#@gWeapon@w=#  Armageddon Cannon#@pSeed@w=17995#Limited Tries:false*/
/*|Armageddon Cannon[Some people just want to watch the world burn...]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Croc Man[From what I can tell you either#don't take damage from anything or#you die instantly because of that passive.]*/
#define init
...
trace("Mods=Armageddon Cannon,Chrononaut,Dasher,Brimthrone,Croc Man");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Armageddon Cannon,Chrononaut,Dasher,Brimthrone,Croc Man" + " Character:dasher" + " Crown:crown of life" + " Weapon:Armageddon Cannon" + " Seed:17995";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-26 Weekly.txt")){
		prevScores = string_load(global.alias + " 2020-01-26 Weekly.txt");
		while(!file_loaded(global.alias + " 2020-01-26 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-26 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2020-01-26 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Armageddon Cannon,Chrononaut,Dasher,Brimthrone,Croc Man" + " Character:dasher" + " Crown:crown of life" + " Weapon:Armageddon Cannon" + " Seed:17995" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Armageddon Cannon,Chrononaut,Dasher,Brimthrone,Croc Man" + " Character:dasher" + " Crown:crown of life" + " Weapon:Armageddon Cannon" + " Seed:17995",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
