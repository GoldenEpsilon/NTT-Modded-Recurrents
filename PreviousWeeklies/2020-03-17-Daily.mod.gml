...
/*@rMods@w=#  Damage Numbers#  Cannondemnation#  Beamer#  BadEnd#  Dasher#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  Beamer#@pSeed@w=28226#Limited Tries:false*/
/*|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Beamer[BZAAAAAP]|BadEnd[For when you don't want a good end.#(sit after loop 3)]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Damage Numbers,Cannondemnation,Beamer,BadEnd,Dasher");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Damage Numbers,Cannondemnation,Beamer,BadEnd,Dasher" + " Character:Choose" + " Crown:crown of life" + " Weapon:Beamer" + " Seed:28226";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-17 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Damage Numbers,Cannondemnation,Beamer,BadEnd,Dasher" + " Character:Choose" + " Crown:crown of life" + " Weapon:Beamer" + " Seed:28226" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Damage Numbers,Cannondemnation,Beamer,BadEnd,Dasher" + " Character:Choose" + " Crown:crown of life" + " Weapon:Beamer" + " Seed:28226",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
