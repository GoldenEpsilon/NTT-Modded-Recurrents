...
/*@rMods@w=#  Ascension 7#  Hard Mode#  Detonator B#  Chrononaut#  Blaac's Weapon Mutations#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  venuz smg#@pSeed@w=10517#Limited Tries:false*/
/*|Ascension 7[ASCENSION LEVEL#7/15]|Hard Mode[Blaac's Hard Mode.#Good luck.#@r@q@qYou'll need it.]|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]*/
#define init
...
downloadmod("ascension7.mod.gml");
...
wait mod_load(string_trim("data/Daily.mod/ascension7.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Ascension 7,Hard Mode,Detonator B,Chrononaut,Blaac's Weapon Mutations");
trace("Character:Choose");
trace("Crown:crown of curses");
trace("Weapon:venuz smg");
trace("Seed:10517");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("venuz smg"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ascension 7,Hard Mode,Detonator B,Chrononaut,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of curses" + " Weapon:venuz smg" + " Seed:10517";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-27 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ascension 7,Hard Mode,Detonator B,Chrononaut,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of curses" + " Weapon:venuz smg" + " Seed:10517" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ascension 7,Hard Mode,Detonator B,Chrononaut,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of curses" + " Weapon:venuz smg" + " Seed:10517",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
