...
/*@rMods@w=#  Auto Bullet Shotgun#  bouncybolts#  Big Bandit#  Fire Riders#  Ascension 13#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=22415#Limited Tries:false*/
/*|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|bouncybolts[Upgrade to Bolt Marrow]|Big Bandit['Nuff said.]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Ascension 13[@qASCENSION LEVEL#15/15]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/ascension15.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Auto Bullet Shotgun,bouncybolts,Big Bandit,Fire Riders,Ascension 13");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Bullet Shotgun,bouncybolts,Big Bandit,Fire Riders,Ascension 13" + " Character:Choose" + " Crown:crown of love" + " Weapon:Auto Bullet Shotgun" + " Seed:22415";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-16 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Bullet Shotgun,bouncybolts,Big Bandit,Fire Riders,Ascension 13" + " Character:Choose" + " Crown:crown of love" + " Weapon:Auto Bullet Shotgun" + " Seed:22415" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Bullet Shotgun,bouncybolts,Big Bandit,Fire Riders,Ascension 13" + " Character:Choose" + " Crown:crown of love" + " Weapon:Auto Bullet Shotgun" + " Seed:22415",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
