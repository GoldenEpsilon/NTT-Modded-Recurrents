...
/*@rMods@w=#  Elite Dasher#  Cosmi Gun Enemies#  Cool Flame#  Double Pop Gun#  Cooler Hud#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  double_pop_gun#@pSeed@w=4093#Limited Tries:false*/
/*|Elite Dasher[@qPICK UP ALL THE THINGS]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Double Pop Gun[Now you can tickle enemies#twice as fast!]|Cooler Hud[Symphony of the Throne]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Dasher,Cosmi Gun Enemies,Cool Flame,Double Pop Gun,Cooler Hud");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Dasher,Cosmi Gun Enemies,Cool Flame,Double Pop Gun,Cooler Hud" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:double_pop_gun" + " Seed:4093";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-16 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Dasher,Cosmi Gun Enemies,Cool Flame,Double Pop Gun,Cooler Hud" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:double_pop_gun" + " Seed:4093" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Dasher,Cosmi Gun Enemies,Cool Flame,Double Pop Gun,Cooler Hud" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:double_pop_gun" + " Seed:4093",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
