...
/*@rMods@w=#  Crazy Traps#  Boltshard#  Detailer#  Ancient Hatred#  Cool Hud#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  Boltshard#@pSeed@w=30250#Limited Tries:false*/
/*|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|Detailer[Adds pretty patches of things]|Ancient Hatred[Crown that trades health for rads over time]|Cool Hud[*C418 starts playing*]*/
#define init
...
trace("Current Daily:");
trace("Mods=Crazy Traps,Boltshard,Detailer,Ancient Hatred,Cool Hud");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Crazy Traps,Boltshard,Detailer,Ancient Hatred,Cool Hud" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Boltshard" + " Seed:30250";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-08 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Crazy Traps,Boltshard,Detailer,Ancient Hatred,Cool Hud" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Boltshard" + " Seed:30250" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Crazy Traps,Boltshard,Detailer,Ancient Hatred,Cool Hud" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Boltshard" + " Seed:30250",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
