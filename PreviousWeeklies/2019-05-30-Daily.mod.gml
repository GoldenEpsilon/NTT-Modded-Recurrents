...
/*@rMods@w=#  Champions#  Carbine#  Fire Riders#  CoD Proper#  Crazy Traps#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  carbine#@pSeed@w=19827#Limited Tries:false*/
/*|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Carbine[Pew Pew.]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]*/
#define init
...
trace("Current Daily:");
trace("Mods=Champions,Carbine,Fire Riders,CoD Proper,Crazy Traps");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Champions,Carbine,Fire Riders,CoD Proper,Crazy Traps" + " Character:Choose" + " Crown:crown of blood" + " Weapon:carbine" + " Seed:19827";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Champions,Carbine,Fire Riders,CoD Proper,Crazy Traps" + " Character:Choose" + " Crown:crown of blood" + " Weapon:carbine" + " Seed:19827" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Champions,Carbine,Fire Riders,CoD Proper,Crazy Traps" + " Character:Choose" + " Crown:crown of blood" + " Weapon:carbine" + " Seed:19827",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
