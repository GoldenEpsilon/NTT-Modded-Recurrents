...
/*@rMods@w=#  Bloodking#  boys#  Egg#  Team Attack#  Double Maxigun#@bCharacter@w=#  bloodking#@yCrown@w=#  none#@gWeapon@w=#  Double Maxigun#@pSeed@w=10662#Limited Tries:false*/
/*|Bloodking[Blood, pain, and a half-finished character.]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Egg[You will GIVE#me an @qEGG]|Team Attack[Team Age Mutant#Ninja Mutants Go!]|Double Maxigun[Because miniguns#aren't cool enough]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/Egg.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/teamattack.mod.gml"));
...
trace("Mods=Bloodking,boys,Egg,Team Attack,Double Maxigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bloodking,boys,Egg,Team Attack,Double Maxigun" + " Character:bloodking" + " Crown:none" + " Weapon:Double Maxigun" + " Seed:10662";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-08 Weekly.txt")){
		prevScores = string_load(global.alias + " 2020-03-08 Weekly.txt");
		while(!file_loaded(global.alias + " 2020-03-08 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-08 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2020-03-08 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bloodking,boys,Egg,Team Attack,Double Maxigun" + " Character:bloodking" + " Crown:none" + " Weapon:Double Maxigun" + " Seed:10662" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bloodking,boys,Egg,Team Attack,Double Maxigun" + " Character:bloodking" + " Crown:none" + " Weapon:Double Maxigun" + " Seed:10662",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("bloodking");}else{global.Race = race;}}
