...
/*@rMods@w=#  Freak Arena#  6 Weapons#  Cuz#  Carmageddon#  Damage Numbers#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  flare gun#@pSeed@w=25776#Limited Tries:false*/
/*|Freak Arena[Do you think you can survive#in the @qFREAK ARENA???]|6 Weapons[Gives you one weapon slot#for each ammo type like DOOM]|Cuz[Has an energy shield, for some reason.]|Carmageddon[WARNING: May contain drifting]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Freak Arena,6 Weapons,Cuz,Carmageddon,Damage Numbers");
trace("Character:Choose");
trace("Crown:crown of love");
trace("Weapon:flare gun");
trace("Seed:25776");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("flare gun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Freak Arena,6 Weapons,Cuz,Carmageddon,Damage Numbers" + " Character:Choose" + " Crown:crown of love" + " Weapon:flare gun" + " Seed:25776";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-08 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Freak Arena,6 Weapons,Cuz,Carmageddon,Damage Numbers" + " Character:Choose" + " Crown:crown of love" + " Weapon:flare gun" + " Seed:25776" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Freak Arena,6 Weapons,Cuz,Carmageddon,Damage Numbers" + " Character:Choose" + " Crown:crown of love" + " Weapon:flare gun" + " Seed:25776",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
