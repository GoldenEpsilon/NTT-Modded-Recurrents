...
/*@rMods@w=#  Double Pop Gun#  Brimthrone#  Dummy#  Chips#  Bigger Bandit#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  double_pop_gun#@pSeed@w=5247#Limited Tries:false*/
/*|Double Pop Gun[Now you can tickle enemies#twice as fast!]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Dummy[No strings on me!]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Bigger Bandit[Ever gotten bored of Big Dog?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Pop Gun,Brimthrone,Dummy,Chips,Bigger Bandit");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Pop Gun,Brimthrone,Dummy,Chips,Bigger Bandit" + " Character:Choose" + " Crown:crown of curses" + " Weapon:double_pop_gun" + " Seed:5247";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Pop Gun,Brimthrone,Dummy,Chips,Bigger Bandit" + " Character:Choose" + " Crown:crown of curses" + " Weapon:double_pop_gun" + " Seed:5247" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Pop Gun,Brimthrone,Dummy,Chips,Bigger Bandit" + " Character:Choose" + " Crown:crown of curses" + " Weapon:double_pop_gun" + " Seed:5247",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
