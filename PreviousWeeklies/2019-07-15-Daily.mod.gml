...
/*@rMods@w=#  Ancient Hatred#  Dodge Roll#  Cosmi Gun Enemies#  Confuse#  Elite Dasher#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  lightning pistol#@pSeed@w=10340#Limited Tries:false*/
/*|Ancient Hatred[Crown that trades health for rads over time]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]|Confuse[I guess you had a bit#too much to drink last night, eh?]|Elite Dasher[@qPICK UP ALL THE THINGS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ancient Hatred,Dodge Roll,Cosmi Gun Enemies,Confuse,Elite Dasher");
trace("Character:Choose");
trace("Crown:crown of ancient hatred");
trace("Weapon:lightning pistol");
trace("Seed:10340");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("lightning pistol"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ancient Hatred,Dodge Roll,Cosmi Gun Enemies,Confuse,Elite Dasher" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:lightning pistol" + " Seed:10340";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ancient Hatred,Dodge Roll,Cosmi Gun Enemies,Confuse,Elite Dasher" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:lightning pistol" + " Seed:10340" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ancient Hatred,Dodge Roll,Cosmi Gun Enemies,Confuse,Elite Dasher" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:lightning pistol" + " Seed:10340",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
