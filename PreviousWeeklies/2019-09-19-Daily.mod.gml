...
/*@rMods@w=#  Carpocalypse#  Champions#  Bandit#  banditstack#  Assault Flak Cannon#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Assault Flak Cannon#@pSeed@w=32225#Limited Tries:false*/
/*|Carpocalypse[Beep Beep, motherf***er]|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Carpocalypse,Champions,Bandit,banditstack,Assault Flak Cannon");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carpocalypse,Champions,Bandit,banditstack,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Assault Flak Cannon" + " Seed:32225";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-19 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-19 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-19 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-19 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-19 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carpocalypse,Champions,Bandit,banditstack,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Assault Flak Cannon" + " Seed:32225" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carpocalypse,Champions,Bandit,banditstack,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Assault Flak Cannon" + " Seed:32225",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
