...
/*@rMods@w=#  Chaos Guns#  Assault Flak Cannon#  Chrononaut#  Cooler Hud#  Belly#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Super Chaos Minigun#@pSeed@w=25965#Limited Tries:false*/
/*|Chaos Guns[For when you need a bit#more chaos in your life.]|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Cooler Hud[Symphony of the Throne]|Belly[The only survivor of the#meme that is melee actives]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chaos Guns,Assault Flak Cannon,Chrononaut,Cooler Hud,Belly");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chaos Guns,Assault Flak Cannon,Chrononaut,Cooler Hud,Belly" + " Character:Choose" + " Crown:crown of death" + " Weapon:Super Chaos Minigun" + " Seed:25965";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chaos Guns,Assault Flak Cannon,Chrononaut,Cooler Hud,Belly" + " Character:Choose" + " Crown:crown of death" + " Weapon:Super Chaos Minigun" + " Seed:25965" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chaos Guns,Assault Flak Cannon,Chrononaut,Cooler Hud,Belly" + " Character:Choose" + " Crown:crown of death" + " Weapon:Super Chaos Minigun" + " Seed:25965",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
