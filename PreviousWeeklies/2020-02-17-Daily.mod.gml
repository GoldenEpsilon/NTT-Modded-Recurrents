...
/*@rMods@w=#  Cannondemnation#  Chest Bandits#  Clasweps#  Disk Marrow#  Car#@bCharacter@w=#  Choose#@yCrown@w=#  Pain#@gWeapon@w=#  popoexplosiongun#@pSeed@w=5879#Limited Tries:false*/
/*|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Chest Bandits[Gets rid of bandits in random areas]|Clasweps[You got ultra weapons,#you got zappy weapons,#call now and you'll even get#some crowns thrown in for free!]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cannondemnation,Chest Bandits,Clasweps,Disk Marrow,Car");
trace("Character:Choose");
trace("Crown:Pain");
trace("Weapon:popoexplosiongun");
trace("Seed:5879");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("popoexplosiongun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cannondemnation,Chest Bandits,Clasweps,Disk Marrow,Car" + " Character:Choose" + " Crown:Pain" + " Weapon:popoexplosiongun" + " Seed:5879";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-17 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cannondemnation,Chest Bandits,Clasweps,Disk Marrow,Car" + " Character:Choose" + " Crown:Pain" + " Weapon:popoexplosiongun" + " Seed:5879" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cannondemnation,Chest Bandits,Clasweps,Disk Marrow,Car" + " Character:Choose" + " Crown:Pain" + " Weapon:popoexplosiongun" + " Seed:5879",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
