...
/*@rMods@w=#  Balls#  blackholes#  Carmageddon#  Afterburn#  boys#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  laserwrench#@pSeed@w=16989#Limited Tries:false*/
/*|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Carmageddon[WARNING: May contain drifting]|Afterburn[Massively Buffs Flames]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Balls,blackholes,Carmageddon,Afterburn,boys");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Balls,blackholes,Carmageddon,Afterburn,boys" + " Character:Choose" + " Crown:crown of risk" + " Weapon:laserwrench" + " Seed:16989";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-25 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Balls,blackholes,Carmageddon,Afterburn,boys" + " Character:Choose" + " Crown:crown of risk" + " Weapon:laserwrench" + " Seed:16989" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Balls,blackholes,Carmageddon,Afterburn,boys" + " Character:Choose" + " Crown:crown of risk" + " Weapon:laserwrench" + " Seed:16989",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
