...
/*@rMods@w=#  Big Toothbrush#  Egg#  Fire Riders#  Babylon Shotgun#  Bandit Brute#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Big Toothbrush#@pSeed@w=903#Limited Tries:false*/
/*|Big Toothbrush[a big toothbrush for big mouths]|Egg[You will GIVE#me an @qEGG]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Babylon Shotgun[So much destruction your enemies will babble-on]|Bandit Brute[Randomly replaces bandits with Bandit Brutes]*/
#define init
...
trace("Current Daily:");
trace("Mods=Big Toothbrush,Egg,Fire Riders,Babylon Shotgun,Bandit Brute");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Big Toothbrush,Egg,Fire Riders,Babylon Shotgun,Bandit Brute" + " Character:Choose" + " Crown:crown of death" + " Weapon:Big Toothbrush" + " Seed:903";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-21 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Big Toothbrush,Egg,Fire Riders,Babylon Shotgun,Bandit Brute" + " Character:Choose" + " Crown:crown of death" + " Weapon:Big Toothbrush" + " Seed:903" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Big Toothbrush,Egg,Fire Riders,Babylon Shotgun,Bandit Brute" + " Character:Choose" + " Crown:crown of death" + " Weapon:Big Toothbrush" + " Seed:903",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
