...
/*@rMods@w=#  Bandit#  Challenge Test#  betterLoops#  Cool Hud#  Damage Numbers#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  pop rifle#@pSeed@w=29532#Limited Tries:false*/
/*|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|Challenge Test[GOTTA GO FAST]|betterLoops[Less enemies, more health per enemy in loops.]|Cool Hud[*C418 starts playing*]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bandit,Challenge Test,betterLoops,Cool Hud,Damage Numbers");
trace("Character:Choose");
trace("Crown:crown of death");
trace("Weapon:pop rifle");
trace("Seed:29532");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("pop rifle"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bandit,Challenge Test,betterLoops,Cool Hud,Damage Numbers" + " Character:Choose" + " Crown:crown of death" + " Weapon:pop rifle" + " Seed:29532";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-06 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bandit,Challenge Test,betterLoops,Cool Hud,Damage Numbers" + " Character:Choose" + " Crown:crown of death" + " Weapon:pop rifle" + " Seed:29532" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bandit,Challenge Test,betterLoops,Cool Hud,Damage Numbers" + " Character:Choose" + " Crown:crown of death" + " Weapon:pop rifle" + " Seed:29532",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
