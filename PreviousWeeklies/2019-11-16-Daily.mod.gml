...
/*@rMods@w=#  Minecraft Area#  bouncybolts#  Damage Numbers#  Crazy Bandits#  Colored Projectiles#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  black sword#@pSeed@w=15673#Limited Tries:false*/
/*|Minecraft Area[Who Love Minecraf]|bouncybolts[Upgrade to Bolt Marrow]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Crazy Bandits[THEY HAVE GUNS]|Colored Projectiles[Colors projectiles based on team color]*/
#define init
...
trace("Current Daily:");
trace("Mods=Minecraft Area,bouncybolts,Damage Numbers,Crazy Bandits,Colored Projectiles");
trace("Character:Choose");
trace("Crown:crown of protection");
trace("Weapon:black sword");
trace("Seed:15673");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("black sword"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Minecraft Area,bouncybolts,Damage Numbers,Crazy Bandits,Colored Projectiles" + " Character:Choose" + " Crown:crown of protection" + " Weapon:black sword" + " Seed:15673";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-16 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Minecraft Area,bouncybolts,Damage Numbers,Crazy Bandits,Colored Projectiles" + " Character:Choose" + " Crown:crown of protection" + " Weapon:black sword" + " Seed:15673" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Minecraft Area,bouncybolts,Damage Numbers,Crazy Bandits,Colored Projectiles" + " Character:Choose" + " Crown:crown of protection" + " Weapon:black sword" + " Seed:15673",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
