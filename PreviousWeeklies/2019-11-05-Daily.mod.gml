...
/*@rMods@w=#  Cycle#  Blaac's Weapon Mutations#  Cool Dust#  blaac48#  Boltshard#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Boltshard#@pSeed@w=7371#Limited Tries:false*/
/*|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|Cool Dust[It's not just dust, it's#COOL DUST]|blaac48[The jungle is deeper]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cycle,Blaac's Weapon Mutations,Cool Dust,blaac48,Boltshard");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cycle,Blaac's Weapon Mutations,Cool Dust,blaac48,Boltshard" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Boltshard" + " Seed:7371";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-05 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cycle,Blaac's Weapon Mutations,Cool Dust,blaac48,Boltshard" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Boltshard" + " Seed:7371" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cycle,Blaac's Weapon Mutations,Cool Dust,blaac48,Boltshard" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Boltshard" + " Seed:7371",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
