...
/*@rMods@w=#  Big Dog Snipers#  Ancient Hatred#  Corsair#  Dasher#  xefsstuff#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  thunder cloud pistol#@pSeed@w=11040#Limited Tries:false*/
/*|Big Dog Snipers[The actual boss]|Ancient Hatred[Crown that trades health for rads over time]|Corsair[The Corsair from EtG#Plot A Course]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|xefsstuff[This has a bit of everything.#A bunch of weapons, a few characters,#a butler robot#buddy thingamijig that kills guys...#@qDON'T GIVE HIM A GRENADE LAUNCHER OH N-]*/
#define init
...
trace("Current Daily:");
trace("Mods=Big Dog Snipers,Ancient Hatred,Corsair,Dasher,xefsstuff");
trace("Character:Choose");
trace("Crown:crown of ancient hatred");
trace("Weapon:thunder cloud pistol");
trace("Seed:11040");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("thunder cloud pistol"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Big Dog Snipers,Ancient Hatred,Corsair,Dasher,xefsstuff" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:thunder cloud pistol" + " Seed:11040";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-23 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-23 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-23 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-23 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-23 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Big Dog Snipers,Ancient Hatred,Corsair,Dasher,xefsstuff" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:thunder cloud pistol" + " Seed:11040" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Big Dog Snipers,Ancient Hatred,Corsair,Dasher,xefsstuff" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:thunder cloud pistol" + " Seed:11040",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
