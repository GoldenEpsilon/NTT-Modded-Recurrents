...
/*@rMods@w=#  Bloodking#  Cool Dark#  DIO#  Bandit#  Double Plasma Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  doubleplasma#@pSeed@w=19559#Limited Tries:false*/
/*|Bloodking[Blood, pain, and a half-finished character.]|Cool Dark[spoopy time]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|Double Plasma Minigun[For when you want to#go through even more ammo]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bloodking,Cool Dark,DIO,Bandit,Double Plasma Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bloodking,Cool Dark,DIO,Bandit,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:doubleplasma" + " Seed:19559";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-07 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-07 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-07 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-07 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-07 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bloodking,Cool Dark,DIO,Bandit,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:doubleplasma" + " Seed:19559" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bloodking,Cool Dark,DIO,Bandit,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:doubleplasma" + " Seed:19559",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
