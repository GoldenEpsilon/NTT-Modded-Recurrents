...
/*@rMods@w=#  bouncermini#  Challenge Test#  Buff#  Determination#  Dumb Shells#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  hyperrifle#@pSeed@w=30626#Limited Tries:false*/
/*|bouncermini[FILL THE SKIES WITH BULLETS]|Challenge Test[GOTTA GO FAST]|Buff[Buffs to various weapons,#ultras, mutations,#and more]|Determination[Seeing this filler text#fills you with determination]|Dumb Shells[And on that dumbshell,#it's time to start.]*/
#define init
...
trace("Current Daily:");
trace("Mods=bouncermini,Challenge Test,Buff,Determination,Dumb Shells");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bouncermini,Challenge Test,Buff,Determination,Dumb Shells" + " Character:Choose" + " Crown:crown of risk" + " Weapon:hyperrifle" + " Seed:30626";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-17 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bouncermini,Challenge Test,Buff,Determination,Dumb Shells" + " Character:Choose" + " Crown:crown of risk" + " Weapon:hyperrifle" + " Seed:30626" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bouncermini,Challenge Test,Buff,Determination,Dumb Shells" + " Character:Choose" + " Crown:crown of risk" + " Weapon:hyperrifle" + " Seed:30626",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
