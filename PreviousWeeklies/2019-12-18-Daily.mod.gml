...
/*@rMods@w=#  Detonator B#  Vanthro#  Dasher x10#  Cannoner#  Ash#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  cannoner#@pSeed@w=20902#Limited Tries:false*/
/*|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|Vanthro[...blame 4chan.]|Dasher x10[@qEXTREME DASHING]|Cannoner[EIGHT RADICAL SHOTS]|Ash[Guy from The End Is Nigh.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Detonator B,Vanthro,Dasher x10,Cannoner,Ash");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Detonator B,Vanthro,Dasher x10,Cannoner,Ash" + " Character:Choose" + " Crown:crown of curses" + " Weapon:cannoner" + " Seed:20902";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Detonator B,Vanthro,Dasher x10,Cannoner,Ash" + " Character:Choose" + " Crown:crown of curses" + " Weapon:cannoner" + " Seed:20902" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Detonator B,Vanthro,Dasher x10,Cannoner,Ash" + " Character:Choose" + " Crown:crown of curses" + " Weapon:cannoner" + " Seed:20902",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
