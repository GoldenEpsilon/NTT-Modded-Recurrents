...
/*@rMods@w=#  Fire Riders#  Double Pop Gun#  Cosmi#  Blood#  basicgunallies#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  double_pop_gun#@pSeed@w=6493#Limited Tries:false*/
/*|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Double Pop Gun[Now you can tickle enemies#twice as fast!]|Cosmi[*insert *insert *insert self insert joke here* joke here* joke here*]|Blood[If Nuclear Throne had an M rating...]|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Fire Riders,Double Pop Gun,Cosmi,Blood,basicgunallies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Fire Riders,Double Pop Gun,Cosmi,Blood,basicgunallies" + " Character:Choose" + " Crown:crown of love" + " Weapon:double_pop_gun" + " Seed:6493";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-07 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-07 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-07 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-07 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-07 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Fire Riders,Double Pop Gun,Cosmi,Blood,basicgunallies" + " Character:Choose" + " Crown:crown of love" + " Weapon:double_pop_gun" + " Seed:6493" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Fire Riders,Double Pop Gun,Cosmi,Blood,basicgunallies" + " Character:Choose" + " Crown:crown of love" + " Weapon:double_pop_gun" + " Seed:6493",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
