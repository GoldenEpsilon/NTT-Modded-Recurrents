...
/*@rMods@w=#  DIO#  Cool Dust#  Big Bandit#  Bioluminary Oscillator#  Cycle#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  Bioluminary Oscillator#@pSeed@w=12920#Limited Tries:false*/
/*|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Cool Dust[It's not just dust, it's#COOL DUST]|Big Bandit['Nuff said.]|Bioluminary Oscillator[TARGET AQUIRED]|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]*/
#define init
...
trace("Current Daily:");
trace("Mods=DIO,Cool Dust,Big Bandit,Bioluminary Oscillator,Cycle");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:DIO,Cool Dust,Big Bandit,Bioluminary Oscillator,Cycle" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Bioluminary Oscillator" + " Seed:12920";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-26 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:DIO,Cool Dust,Big Bandit,Bioluminary Oscillator,Cycle" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Bioluminary Oscillator" + " Seed:12920" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":DIO,Cool Dust,Big Bandit,Bioluminary Oscillator,Cycle" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Bioluminary Oscillator" + " Seed:12920",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
