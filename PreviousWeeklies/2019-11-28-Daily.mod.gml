...
/*@rMods@w=#  Brimthrone#  CoDeath#  Boltshard#  Charge Template#  Crazy Traps#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  brimthrone#@pSeed@w=1023#Limited Tries:false*/
/*|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]*/
#define init
...
trace("Current Daily:");
trace("Mods=Brimthrone,CoDeath,Boltshard,Charge Template,Crazy Traps");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Brimthrone,CoDeath,Boltshard,Charge Template,Crazy Traps" + " Character:Choose" + " Crown:CoDeath" + " Weapon:brimthrone" + " Seed:1023";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-28 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Brimthrone,CoDeath,Boltshard,Charge Template,Crazy Traps" + " Character:Choose" + " Crown:CoDeath" + " Weapon:brimthrone" + " Seed:1023" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Brimthrone,CoDeath,Boltshard,Charge Template,Crazy Traps" + " Character:Choose" + " Crown:CoDeath" + " Weapon:brimthrone" + " Seed:1023",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
