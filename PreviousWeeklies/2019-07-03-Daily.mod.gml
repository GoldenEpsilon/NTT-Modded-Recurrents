...
/*@rMods@w=#  basicgunallies#  BAR#  blstorm#  Cuz#  banditstack#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  Big Ass Revolver#@pSeed@w=14936#Limited Tries:false*/
/*|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|BAR[A Big Ass Revolver.]|blstorm[BLEED YOURSELF DRY]|Cuz[Has an energy shield, for some reason.]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]*/
#define init
...
trace("Current Daily:");
trace("Mods=basicgunallies,BAR,blstorm,Cuz,banditstack");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:basicgunallies,BAR,blstorm,Cuz,banditstack" + " Character:Choose" + " Crown:none" + " Weapon:Big Ass Revolver" + " Seed:14936";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:basicgunallies,BAR,blstorm,Cuz,banditstack" + " Character:Choose" + " Crown:none" + " Weapon:Big Ass Revolver" + " Seed:14936" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":basicgunallies,BAR,blstorm,Cuz,banditstack" + " Character:Choose" + " Crown:none" + " Weapon:Big Ass Revolver" + " Seed:14936",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
