...
/*@rMods@w=#  BadEnd#  Elite Turrets 2#  Chrononaut#  Elite Gamers#  Cool Hud#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  idpdslugpistol#@pSeed@w=20959#Limited Tries:false*/
/*|BadEnd[For when you don't want a good end.#(sit after loop 3)]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Elite Gamers[The bad guys got#aiming classes]|Cool Hud[*C418 starts playing*]*/
#define init
...
trace("Current Daily:");
trace("Mods=BadEnd,Elite Turrets 2,Chrononaut,Elite Gamers,Cool Hud");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:BadEnd,Elite Turrets 2,Chrononaut,Elite Gamers,Cool Hud" + " Character:Choose" + " Crown:crown of risk" + " Weapon:idpdslugpistol" + " Seed:20959";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-14 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:BadEnd,Elite Turrets 2,Chrononaut,Elite Gamers,Cool Hud" + " Character:Choose" + " Crown:crown of risk" + " Weapon:idpdslugpistol" + " Seed:20959" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":BadEnd,Elite Turrets 2,Chrononaut,Elite Gamers,Cool Hud" + " Character:Choose" + " Crown:crown of risk" + " Weapon:idpdslugpistol" + " Seed:20959",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
