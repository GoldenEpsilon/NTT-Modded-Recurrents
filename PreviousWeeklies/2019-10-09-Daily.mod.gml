...
/*@rMods@w=#  Cycle#  Ascension 7#  Dogfly#  Blindness#  Elite Rifle#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  eliterifle#@pSeed@w=8760#Limited Tries:false*/
/*|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]|Ascension 7[ASCENSION LEVEL#7/15]|Dogfly[The dog guardians#learned to jump good.]|Blindness[MY GLASSES! I BROKE MY GLASSES!]|Elite Rifle[I'm just gonna grab that,#thank you very much.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cycle,Ascension 7,Dogfly,Blindness,Elite Rifle");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cycle,Ascension 7,Dogfly,Blindness,Elite Rifle" + " Character:Choose" + " Crown:crown of guns" + " Weapon:eliterifle" + " Seed:8760";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-09 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cycle,Ascension 7,Dogfly,Blindness,Elite Rifle" + " Character:Choose" + " Crown:crown of guns" + " Weapon:eliterifle" + " Seed:8760" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cycle,Ascension 7,Dogfly,Blindness,Elite Rifle" + " Character:Choose" + " Crown:crown of guns" + " Weapon:eliterifle" + " Seed:8760",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
