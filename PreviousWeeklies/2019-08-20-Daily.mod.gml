...
/*@rMods@w=#  bow#  Double Plasma Minigun#  bouncermini#  Carbine#  CoDeath#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  bow#@pSeed@w=22338#Limited Tries:false*/
/*|bow[It's a bow.]|Double Plasma Minigun[For when you want to#go through even more ammo]|bouncermini[FILL THE SKIES WITH BULLETS]|Carbine[Pew Pew.]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]*/
#define init
...
trace("Current Daily:");
trace("Mods=bow,Double Plasma Minigun,bouncermini,Carbine,CoDeath");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bow,Double Plasma Minigun,bouncermini,Carbine,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:bow" + " Seed:22338";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-20 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bow,Double Plasma Minigun,bouncermini,Carbine,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:bow" + " Seed:22338" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bow,Double Plasma Minigun,bouncermini,Carbine,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:bow" + " Seed:22338",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
