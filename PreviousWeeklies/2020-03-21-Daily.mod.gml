...
/*@rMods@w=#  Buff#  Chips#  Hard Mode#  DIO#  Double Plasma Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  hyperrifle#@pSeed@w=10598#Limited Tries:false*/
/*|Buff[Buffs to various weapons,#ultras, mutations,#and more]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Hard Mode[Blaac's Hard Mode.#Good luck.#@r@q@qYou'll need it.]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Double Plasma Minigun[For when you want to#go through even more ammo]*/
#define init
...
trace("Current Daily:");
trace("Mods=Buff,Chips,Hard Mode,DIO,Double Plasma Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Buff,Chips,Hard Mode,DIO,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyperrifle" + " Seed:10598";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-21 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Buff,Chips,Hard Mode,DIO,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyperrifle" + " Seed:10598" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Buff,Chips,Hard Mode,DIO,Double Plasma Minigun" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyperrifle" + " Seed:10598",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
