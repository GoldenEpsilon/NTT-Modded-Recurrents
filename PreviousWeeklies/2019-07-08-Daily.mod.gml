...
/*@rMods@w=#  bouncermini#  Boltshard#  bosshudredux#  Chest Bandits#  bossbar#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  Boltshard#@pSeed@w=946#Limited Tries:false*/
/*|bouncermini[FILL THE SKIES WITH BULLETS]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|bosshudredux[For knowing how bad your weapons are,#every once in a while]|Chest Bandits[Gets rid of bandits in random areas]|bossbar[For when you want to know#just how close you were to looping]*/
#define init
...
trace("Current Daily:");
trace("Mods=bouncermini,Boltshard,bosshudredux,Chest Bandits,bossbar");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bouncermini,Boltshard,bosshudredux,Chest Bandits,bossbar" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Boltshard" + " Seed:946";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-08 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bouncermini,Boltshard,bosshudredux,Chest Bandits,bossbar" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Boltshard" + " Seed:946" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bouncermini,Boltshard,bosshudredux,Chest Bandits,bossbar" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Boltshard" + " Seed:946",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
