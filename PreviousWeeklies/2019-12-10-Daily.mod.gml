...
/*@rMods@w=#  boys#  Double Maxigun#  Camper Boys#  Vanthro#  Dumb Shells#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Double Maxigun#@pSeed@w=26444#Limited Tries:false*/
/*|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Double Maxigun[Because miniguns#aren't cool enough]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]|Vanthro[...blame 4chan.]|Dumb Shells[And on that dumbshell,#it's time to start.]*/
#define init
...
trace("Current Daily:");
trace("Mods=boys,Double Maxigun,Camper Boys,Vanthro,Dumb Shells");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:boys,Double Maxigun,Camper Boys,Vanthro,Dumb Shells" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Double Maxigun" + " Seed:26444";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-10 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-10 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-10 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-10 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-10 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:boys,Double Maxigun,Camper Boys,Vanthro,Dumb Shells" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Double Maxigun" + " Seed:26444" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":boys,Double Maxigun,Camper Boys,Vanthro,Dumb Shells" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Double Maxigun" + " Seed:26444",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
