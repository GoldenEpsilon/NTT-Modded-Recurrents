...
/*@rMods@w=#  Crazy Bandits#  Blindness#  Chest Bandits#  Cosmi#  Minecraft Area#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  machinegun#@pSeed@w=17047#Limited Tries:false*/
/*|Crazy Bandits[THEY HAVE GUNS]|Blindness[MY GLASSES! I BROKE MY GLASSES!]|Chest Bandits[Gets rid of bandits in random areas]|Cosmi[*insert *insert *insert self insert joke here* joke here* joke here*]|Minecraft Area[Who Love Minecraf]*/
#define init
...
trace("Current Daily:");
trace("Mods=Crazy Bandits,Blindness,Chest Bandits,Cosmi,Minecraft Area");
trace("Character:Choose");
trace("Crown:crown of risk");
trace("Weapon:machinegun");
trace("Seed:17047");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("machinegun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Crazy Bandits,Blindness,Chest Bandits,Cosmi,Minecraft Area" + " Character:Choose" + " Crown:crown of risk" + " Weapon:machinegun" + " Seed:17047";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-05 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Crazy Bandits,Blindness,Chest Bandits,Cosmi,Minecraft Area" + " Character:Choose" + " Crown:crown of risk" + " Weapon:machinegun" + " Seed:17047" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Crazy Bandits,Blindness,Chest Bandits,Cosmi,Minecraft Area" + " Character:Choose" + " Crown:crown of risk" + " Weapon:machinegun" + " Seed:17047",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
