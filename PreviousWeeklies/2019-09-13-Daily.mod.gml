...
/*@rMods@w=#  Minecraft Area#  Brimthrone#  Assault Flak Cannon#  Drop Me Gun#  Director#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  brimthrone#@pSeed@w=10876#Limited Tries:false*/
/*|Minecraft Area[Who Love Minecraf]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Director[He will direct your enemies#to the nearest exit of life]*/
#define init
...
trace("Current Daily:");
trace("Mods=Minecraft Area,Brimthrone,Assault Flak Cannon,Drop Me Gun,Director");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Minecraft Area,Brimthrone,Assault Flak Cannon,Drop Me Gun,Director" + " Character:Choose" + " Crown:crown of haste" + " Weapon:brimthrone" + " Seed:10876";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-13 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Minecraft Area,Brimthrone,Assault Flak Cannon,Drop Me Gun,Director" + " Character:Choose" + " Crown:crown of haste" + " Weapon:brimthrone" + " Seed:10876" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Minecraft Area,Brimthrone,Assault Flak Cannon,Drop Me Gun,Director" + " Character:Choose" + " Crown:crown of haste" + " Weapon:brimthrone" + " Seed:10876",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
