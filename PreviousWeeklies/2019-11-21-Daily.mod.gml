...
/*@rMods@w=#  Elite Rifle#  CoDeath#  Cannoner#  Charge Template#  Ascension 13#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  cannoner#@pSeed@w=9268#Limited Tries:false*/
/*|Elite Rifle[I'm just gonna grab that,#thank you very much.]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Cannoner[EIGHT RADICAL SHOTS]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Ascension 13[@qASCENSION LEVEL#15/15]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Rifle,CoDeath,Cannoner,Charge Template,Ascension 13");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Rifle,CoDeath,Cannoner,Charge Template,Ascension 13" + " Character:Choose" + " Crown:CoDeath" + " Weapon:cannoner" + " Seed:9268";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-21 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Rifle,CoDeath,Cannoner,Charge Template,Ascension 13" + " Character:Choose" + " Crown:CoDeath" + " Weapon:cannoner" + " Seed:9268" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Rifle,CoDeath,Cannoner,Charge Template,Ascension 13" + " Character:Choose" + " Crown:CoDeath" + " Weapon:cannoner" + " Seed:9268",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
