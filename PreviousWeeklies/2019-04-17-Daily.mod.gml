...
/*@rMods@w=#  Babylon Shotgun#  Beamer#  xefsstuff#  Dasher#  Chips#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  heavy darts#@pSeed@w=4166#Limited Tries:false*/
/*|Babylon Shotgun[So much destruction your enemies will babble-on]|Beamer[BZAAAAAP]|xefsstuff[This has a bit of everything.#A bunch of weapons, a few characters,#a butler robot#buddy thingamijig that kills guys...#@qDON'T GIVE HIM A GRENADE LAUNCHER OH N-]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]*/
#define init
...
trace("Current Daily:");
trace("Mods=Babylon Shotgun,Beamer,xefsstuff,Dasher,Chips");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Babylon Shotgun,Beamer,xefsstuff,Dasher,Chips" + " Character:Choose" + " Crown:none" + " Weapon:heavy darts" + " Seed:4166";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-17 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Babylon Shotgun,Beamer,xefsstuff,Dasher,Chips" + " Character:Choose" + " Crown:none" + " Weapon:heavy darts" + " Seed:4166" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Babylon Shotgun,Beamer,xefsstuff,Dasher,Chips" + " Character:Choose" + " Crown:none" + " Weapon:heavy darts" + " Seed:4166",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
