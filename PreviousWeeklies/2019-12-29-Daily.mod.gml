...
/*@rMods@w=#  AllCrowns#  Dummy#  Bear#  Colored Palace#  Chest Bandits#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  A Fucking Gun#@pSeed@w=9736#Limited Tries:false*/
/*|AllCrowns[Lets you choose your crown#out of all crowns#at a crown vault.]|Dummy[No strings on me!]|Bear[4 da meeeeeeeeee#eeeeemez]|Colored Palace[Gives a palace a#well-deserved paint job.]|Chest Bandits[Gets rid of bandits in random areas]*/
#define init
...
trace("Current Daily:");
trace("Mods=AllCrowns,Dummy,Bear,Colored Palace,Chest Bandits");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:AllCrowns,Dummy,Bear,Colored Palace,Chest Bandits" + " Character:Choose" + " Crown:crown of blood" + " Weapon:A Fucking Gun" + " Seed:9736";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-29 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:AllCrowns,Dummy,Bear,Colored Palace,Chest Bandits" + " Character:Choose" + " Crown:crown of blood" + " Weapon:A Fucking Gun" + " Seed:9736" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":AllCrowns,Dummy,Bear,Colored Palace,Chest Bandits" + " Character:Choose" + " Crown:crown of blood" + " Weapon:A Fucking Gun" + " Seed:9736",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
