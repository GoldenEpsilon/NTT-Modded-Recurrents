...
/*@rMods@w=#  Ascension 13#  Double Time#  Baller#  Cannoner#  Ascension 7#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  cannoner#@pSeed@w=5469#Limited Tries:false*/
/*|Ascension 13[@qASCENSION LEVEL#15/15]|Double Time[GOTTAGOFAST]|Baller[Fire burns...#me like fire...]|Cannoner[EIGHT RADICAL SHOTS]|Ascension 7[ASCENSION LEVEL#7/15]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ascension 13,Double Time,Baller,Cannoner,Ascension 7");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ascension 13,Double Time,Baller,Cannoner,Ascension 7" + " Character:Choose" + " Crown:crown of risk" + " Weapon:cannoner" + " Seed:5469";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-26 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ascension 13,Double Time,Baller,Cannoner,Ascension 7" + " Character:Choose" + " Crown:crown of risk" + " Weapon:cannoner" + " Seed:5469" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ascension 13,Double Time,Baller,Cannoner,Ascension 7" + " Character:Choose" + " Crown:crown of risk" + " Weapon:cannoner" + " Seed:5469",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
