...
/*@rMods@w=#  B#  Colored Palace#  DIO#  Cool Dark#  Bullet Time#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  heavy machinegun#@pSeed@w=11689#Limited Tries:false*/
/*|B[B]|Colored Palace[Gives a palace a#well-deserved paint job.]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Cool Dark[spoopy time]|Bullet Time[The closer you are to bullets#the slower time moves]*/
#define init
...
trace("Current Daily:");
trace("Mods=B,Colored Palace,DIO,Cool Dark,Bullet Time");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:B,Colored Palace,DIO,Cool Dark,Bullet Time" + " Character:Choose" + " Crown:crown of life" + " Weapon:heavy machinegun" + " Seed:11689";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-25 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:B,Colored Palace,DIO,Cool Dark,Bullet Time" + " Character:Choose" + " Crown:crown of life" + " Weapon:heavy machinegun" + " Seed:11689" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":B,Colored Palace,DIO,Cool Dark,Bullet Time" + " Character:Choose" + " Crown:crown of life" + " Weapon:heavy machinegun" + " Seed:11689",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
