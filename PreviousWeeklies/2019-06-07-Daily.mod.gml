...
/*@rMods@w=#  Elite Minigun#  Crazy Bandits#  Bandit#  Carbine#  Bitpack#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  industrialshankgun#@pSeed@w=14723#Limited Tries:false*/
/*|Elite Minigun[Man, it feels nice#using their own guns on them.]|Crazy Bandits[THEY HAVE GUNS]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|Carbine[Pew Pew.]|Bitpack[idk, bitflux made this]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Minigun,Crazy Bandits,Bandit,Carbine,Bitpack");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Minigun,Crazy Bandits,Bandit,Carbine,Bitpack" + " Character:Choose" + " Crown:crown of blood" + " Weapon:industrialshankgun" + " Seed:14723";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-07 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-07 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-07 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-07 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-07 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Minigun,Crazy Bandits,Bandit,Carbine,Bitpack" + " Character:Choose" + " Crown:crown of blood" + " Weapon:industrialshankgun" + " Seed:14723" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Minigun,Crazy Bandits,Bandit,Carbine,Bitpack" + " Character:Choose" + " Crown:crown of blood" + " Weapon:industrialshankgun" + " Seed:14723",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
