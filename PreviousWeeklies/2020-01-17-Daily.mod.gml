...
/*@rMods@w=#  Bloodking#  Hard Mode#  C Ultras#  Carpocalypse#  Elite Turrets 2#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  harpoon gun#@pSeed@w=15268#Limited Tries:false*/
/*|Bloodking[Blood, pain, and a half-finished character.]|Hard Mode[Blaac's Hard Mode.#Good luck.#@r@q@qYou'll need it.]|C Ultras[Adds a C ultra to all#the vanilla characters]|Carpocalypse[Beep Beep, motherf***er]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bloodking,Hard Mode,C Ultras,Carpocalypse,Elite Turrets 2");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bloodking,Hard Mode,C Ultras,Carpocalypse,Elite Turrets 2" + " Character:Choose" + " Crown:crown of death" + " Weapon:harpoon gun" + " Seed:15268";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-17 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bloodking,Hard Mode,C Ultras,Carpocalypse,Elite Turrets 2" + " Character:Choose" + " Crown:crown of death" + " Weapon:harpoon gun" + " Seed:15268" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bloodking,Hard Mode,C Ultras,Carpocalypse,Elite Turrets 2" + " Character:Choose" + " Crown:crown of death" + " Weapon:harpoon gun" + " Seed:15268",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
