...
/*@rMods@w=#  Blindness#  basicgunallies#  Area Health#  Disk Marrow#  Double Pop Gun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  double_pop_gun#@pSeed@w=19998#Limited Tries:false*/
/*|Blindness[MY GLASSES! I BROKE MY GLASSES!]|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|Area Health[A nice mod that shows how many enemies are left.]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Double Pop Gun[Now you can tickle enemies#twice as fast!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Blindness,basicgunallies,Area Health,Disk Marrow,Double Pop Gun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Blindness,basicgunallies,Area Health,Disk Marrow,Double Pop Gun" + " Character:Choose" + " Crown:crown of death" + " Weapon:double_pop_gun" + " Seed:19998";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Blindness,basicgunallies,Area Health,Disk Marrow,Double Pop Gun" + " Character:Choose" + " Crown:crown of death" + " Weapon:double_pop_gun" + " Seed:19998" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Blindness,basicgunallies,Area Health,Disk Marrow,Double Pop Gun" + " Character:Choose" + " Crown:crown of death" + " Weapon:double_pop_gun" + " Seed:19998",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
