...
/*@rMods@w=#  Assault Flak Cannon#  Elite Dasher#  Double Plasma Minigun#  Drop Me Gun#  Bullet Time#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  doubleplasma#@pSeed@w=10923#Limited Tries:false*/
/*|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]|Elite Dasher[@qPICK UP ALL THE THINGS]|Double Plasma Minigun[For when you want to#go through even more ammo]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Bullet Time[The closer you are to bullets#the slower time moves]*/
#define init
...
trace("Current Daily:");
trace("Mods=Assault Flak Cannon,Elite Dasher,Double Plasma Minigun,Drop Me Gun,Bullet Time");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Assault Flak Cannon,Elite Dasher,Double Plasma Minigun,Drop Me Gun,Bullet Time" + " Character:Choose" + " Crown:crown of curses" + " Weapon:doubleplasma" + " Seed:10923";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Assault Flak Cannon,Elite Dasher,Double Plasma Minigun,Drop Me Gun,Bullet Time" + " Character:Choose" + " Crown:crown of curses" + " Weapon:doubleplasma" + " Seed:10923" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Assault Flak Cannon,Elite Dasher,Double Plasma Minigun,Drop Me Gun,Bullet Time" + " Character:Choose" + " Crown:crown of curses" + " Weapon:doubleplasma" + " Seed:10923",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
