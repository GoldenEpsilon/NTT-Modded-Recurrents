...
/*@rMods@w=#  Boltshard#  Druid#  Cannoner#  Bandit Brute#  Belly#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  Boltshard#@pSeed@w=27505#Limited Tries:false*/
/*|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|Druid[Not Getafix,#but he'll do.]|Cannoner[EIGHT RADICAL SHOTS]|Bandit Brute[Randomly replaces bandits with Bandit Brutes]|Belly[The only survivor of the#meme that is melee actives]*/
#define init
...
trace("Current Daily:");
trace("Mods=Boltshard,Druid,Cannoner,Bandit Brute,Belly");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Boltshard,Druid,Cannoner,Bandit Brute,Belly" + " Character:Choose" + " Crown:crown of love" + " Weapon:Boltshard" + " Seed:27505";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-26 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Boltshard,Druid,Cannoner,Bandit Brute,Belly" + " Character:Choose" + " Crown:crown of love" + " Weapon:Boltshard" + " Seed:27505" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Boltshard,Druid,Cannoner,Bandit Brute,Belly" + " Character:Choose" + " Crown:crown of love" + " Weapon:Boltshard" + " Seed:27505",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
