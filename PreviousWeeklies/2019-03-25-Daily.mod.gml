...
/*@rMods@w=#  Bioluminary Oscillator#  Double Maxigun#  Balls#  !ARMAGGEDNN#  Charge Template#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  Inspector Slugger#@pSeed@w=8294#Limited Tries:false*/
/*|Bioluminary Oscillator[TARGET AQUIRED]|Double Maxigun[Because miniguns#aren't cool enough]|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bioluminary Oscillator,Double Maxigun,Balls,!ARMAGGEDNN,Charge Template");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bioluminary Oscillator,Double Maxigun,Balls,!ARMAGGEDNN,Charge Template" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Inspector Slugger" + " Seed:8294";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-25 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bioluminary Oscillator,Double Maxigun,Balls,!ARMAGGEDNN,Charge Template" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Inspector Slugger" + " Seed:8294" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bioluminary Oscillator,Double Maxigun,Balls,!ARMAGGEDNN,Charge Template" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Inspector Slugger" + " Seed:8294",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
