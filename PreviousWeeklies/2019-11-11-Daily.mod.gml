...
/*@rMods@w=#  Cool Flame#  Bear#  Big Toothbrush#  Grappler#  Crazy Traps#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  A Fucking Gun#@pSeed@w=1845#Limited Tries:false*/
/*|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Bear[4 da meeeeeeeeee#eeeeemez]|Big Toothbrush[a big toothbrush for big mouths]|Grappler[For when you really want to throw someone at someone else.]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Flame,Bear,Big Toothbrush,Grappler,Crazy Traps");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Flame,Bear,Big Toothbrush,Grappler,Crazy Traps" + " Character:Choose" + " Crown:crown of protection" + " Weapon:A Fucking Gun" + " Seed:1845";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Flame,Bear,Big Toothbrush,Grappler,Crazy Traps" + " Character:Choose" + " Crown:crown of protection" + " Weapon:A Fucking Gun" + " Seed:1845" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Flame,Bear,Big Toothbrush,Grappler,Crazy Traps" + " Character:Choose" + " Crown:crown of protection" + " Weapon:A Fucking Gun" + " Seed:1845",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
