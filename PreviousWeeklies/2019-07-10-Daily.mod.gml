...
/*@rMods@w=#  bouncermini#  Crazy Allies#  Charge Template#  Detailer#  Drama Kill#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  ct#@pSeed@w=26888#Limited Tries:false*/
/*|bouncermini[FILL THE SKIES WITH BULLETS]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Detailer[Adds pretty patches of things]|Drama Kill[For when you want#to feel really good#for defeating Big Bandit]*/
#define init
...
trace("Current Daily:");
trace("Mods=bouncermini,Crazy Allies,Charge Template,Detailer,Drama Kill");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bouncermini,Crazy Allies,Charge Template,Detailer,Drama Kill" + " Character:Choose" + " Crown:crown of curses" + " Weapon:ct" + " Seed:26888";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-10 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-10 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-10 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-10 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-10 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bouncermini,Crazy Allies,Charge Template,Detailer,Drama Kill" + " Character:Choose" + " Crown:crown of curses" + " Weapon:ct" + " Seed:26888" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bouncermini,Crazy Allies,Charge Template,Detailer,Drama Kill" + " Character:Choose" + " Crown:crown of curses" + " Weapon:ct" + " Seed:26888",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
