...
/*@rMods@w=#  Bloodking#  Clasweps#  Dumb Shells#  Colored Palace#  Bandit#@bCharacter@w=#  Choose#@yCrown@w=#  Slaughter#@gWeapon@w=#  chargeflak#@pSeed@w=364#Limited Tries:false*/
/*|Bloodking[Blood, pain, and a half-finished character.]|Clasweps[You got ultra weapons,#you got zappy weapons,#call now and you'll even get#some crowns thrown in for free!]|Dumb Shells[And on that dumbshell,#it's time to start.]|Colored Palace[Gives a palace a#well-deserved paint job.]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bloodking,Clasweps,Dumb Shells,Colored Palace,Bandit");
trace("Character:Choose");
trace("Crown:Slaughter");
trace("Weapon:chargeflak");
trace("Seed:364");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("chargeflak"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bloodking,Clasweps,Dumb Shells,Colored Palace,Bandit" + " Character:Choose" + " Crown:Slaughter" + " Weapon:chargeflak" + " Seed:364";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-19 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-19 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-19 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-19 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-19 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bloodking,Clasweps,Dumb Shells,Colored Palace,Bandit" + " Character:Choose" + " Crown:Slaughter" + " Weapon:chargeflak" + " Seed:364" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bloodking,Clasweps,Dumb Shells,Colored Palace,Bandit" + " Character:Choose" + " Crown:Slaughter" + " Weapon:chargeflak" + " Seed:364",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
