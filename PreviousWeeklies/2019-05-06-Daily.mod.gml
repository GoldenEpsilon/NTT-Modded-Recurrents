...
/*@rMods@w=#  Ash#  basicgunallies#  Fire Riders#  Druid#  Dumb Shells#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  splinter pistol#@pSeed@w=4144#Limited Tries:false*/
/*|Ash[Guy from The End Is Nigh.]|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Druid[Not Getafix,#but he'll do.]|Dumb Shells[And on that dumbshell,#it's time to start.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ash,basicgunallies,Fire Riders,Druid,Dumb Shells");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ash,basicgunallies,Fire Riders,Druid,Dumb Shells" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:splinter pistol" + " Seed:4144";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-06 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ash,basicgunallies,Fire Riders,Druid,Dumb Shells" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:splinter pistol" + " Seed:4144" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ash,basicgunallies,Fire Riders,Druid,Dumb Shells" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:splinter pistol" + " Seed:4144",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
