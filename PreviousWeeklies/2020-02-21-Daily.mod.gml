...
/*@rMods@w=#  Cool Trails#  Bandit#  Blindness#  Dummy#  Doctor#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  hyper slugger#@pSeed@w=6839#Limited Tries:false*/
/*|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|Blindness[MY GLASSES! I BROKE MY GLASSES!]|Dummy[No strings on me!]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Trails,Bandit,Blindness,Dummy,Doctor");
trace("Character:Choose");
trace("Crown:crown of haste");
trace("Weapon:hyper slugger");
trace("Seed:6839");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("hyper slugger"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Trails,Bandit,Blindness,Dummy,Doctor" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyper slugger" + " Seed:6839";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-21 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Trails,Bandit,Blindness,Dummy,Doctor" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyper slugger" + " Seed:6839" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Trails,Bandit,Blindness,Dummy,Doctor" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyper slugger" + " Seed:6839",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
