...
/*@rMods@w=#  Chaos Guns#  Balls#  Detonator#  Cool Dark#  Area Health#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  Super Chaos Minigun#@pSeed@w=11708#Limited Tries:false*/
/*|Chaos Guns[For when you need a bit#more chaos in your life.]|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]|Detonator[Active has a radius#where when you release#it blows up your bullets]|Cool Dark[spoopy time]|Area Health[A nice mod that shows how many enemies are left.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chaos Guns,Balls,Detonator,Cool Dark,Area Health");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chaos Guns,Balls,Detonator,Cool Dark,Area Health" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Super Chaos Minigun" + " Seed:11708";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-12 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-12 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-12 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-12 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-12 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chaos Guns,Balls,Detonator,Cool Dark,Area Health" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Super Chaos Minigun" + " Seed:11708" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chaos Guns,Balls,Detonator,Cool Dark,Area Health" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Super Chaos Minigun" + " Seed:11708",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
