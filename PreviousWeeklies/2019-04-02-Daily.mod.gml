...
/*@rMods@w=#  CoDeath#  Boltshard#  Determination#  Charge Template#  C Ultras#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  ct#@pSeed@w=19933#Limited Tries:false*/
/*|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|Determination[Seeing this filler text#fills you with determination]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|C Ultras[Adds a C ultra to all#the vanilla characters]*/
#define init
...
trace("Current Daily:");
trace("Mods=CoDeath,Boltshard,Determination,Charge Template,C Ultras");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:CoDeath,Boltshard,Determination,Charge Template,C Ultras" + " Character:Choose" + " Crown:CoDeath" + " Weapon:ct" + " Seed:19933";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-02 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-02 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-02 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-02 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-02 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:CoDeath,Boltshard,Determination,Charge Template,C Ultras" + " Character:Choose" + " Crown:CoDeath" + " Weapon:ct" + " Seed:19933" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":CoDeath,Boltshard,Determination,Charge Template,C Ultras" + " Character:Choose" + " Crown:CoDeath" + " Weapon:ct" + " Seed:19933",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
