...
/*@rMods@w=#  Minecraft Area#  Cool Flame#  Defect#  !ARMAGGEDNN#  Beamer#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  IDPD Rocket#@pSeed@w=2291#Limited Tries:false*/
/*|Minecraft Area[Who Love Minecraf]|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Defect[BEEP BOOP I AM ROBOT]|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]|Beamer[BZAAAAAP]*/
#define init
...
trace("Current Daily:");
trace("Mods=Minecraft Area,Cool Flame,Defect,!ARMAGGEDNN,Beamer");
trace("Character:Choose");
trace("Crown:crown of haste");
trace("Weapon:IDPD Rocket");
trace("Seed:2291");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("IDPD Rocket"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Minecraft Area,Cool Flame,Defect,!ARMAGGEDNN,Beamer" + " Character:Choose" + " Crown:crown of haste" + " Weapon:IDPD Rocket" + " Seed:2291";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-05 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Minecraft Area,Cool Flame,Defect,!ARMAGGEDNN,Beamer" + " Character:Choose" + " Crown:crown of haste" + " Weapon:IDPD Rocket" + " Seed:2291" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Minecraft Area,Cool Flame,Defect,!ARMAGGEDNN,Beamer" + " Character:Choose" + " Crown:crown of haste" + " Weapon:IDPD Rocket" + " Seed:2291",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
