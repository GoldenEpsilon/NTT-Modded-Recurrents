...
/*@rMods@w=#  Buff#  Charge Template#  Chaos Guns#  CoD Proper#  Cricket#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  smartgun#@pSeed@w=32040#Limited Tries:false*/
/*|Buff[Buffs to various weapons,#ultras, mutations,#and more]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Chaos Guns[For when you need a bit#more chaos in your life.]|Cricket[Batter up!#...or whatever they say for cricket.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Buff,Charge Template,Chaos Guns,CoD Proper,Cricket");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Buff,Charge Template,Chaos Guns,CoD Proper,Cricket" + " Character:Choose" + " Crown:crown of risk" + " Weapon:smartgun" + " Seed:32040";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-21 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Buff,Charge Template,Chaos Guns,CoD Proper,Cricket" + " Character:Choose" + " Crown:crown of risk" + " Weapon:smartgun" + " Seed:32040" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Buff,Charge Template,Chaos Guns,CoD Proper,Cricket" + " Character:Choose" + " Crown:crown of risk" + " Weapon:smartgun" + " Seed:32040",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
