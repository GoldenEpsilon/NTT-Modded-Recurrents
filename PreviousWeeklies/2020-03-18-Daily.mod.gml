...
/*@rMods@w=#  Druid#  Director#  Big Bandit#  xefsstuff#  Elite Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  cloud blaster#@pSeed@w=27501#Limited Tries:false*/
/*|Druid[Not Getafix,#but he'll do.]|Director[He will direct your enemies#to the nearest exit of life]|Big Bandit['Nuff said.]|xefsstuff[This has a bit of everything.#A bunch of weapons, a few characters,#a butler robot#buddy thingamijig that kills guys...#@qDON'T GIVE HIM A GRENADE LAUNCHER OH N-]|Elite Minigun[Man, it feels nice#using their own guns on them.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Druid,Director,Big Bandit,xefsstuff,Elite Minigun");
trace("Character:Choose");
trace("Crown:crown of life");
trace("Weapon:cloud blaster");
trace("Seed:27501");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("cloud blaster"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Druid,Director,Big Bandit,xefsstuff,Elite Minigun" + " Character:Choose" + " Crown:crown of life" + " Weapon:cloud blaster" + " Seed:27501";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-18 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Druid,Director,Big Bandit,xefsstuff,Elite Minigun" + " Character:Choose" + " Crown:crown of life" + " Weapon:cloud blaster" + " Seed:27501" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Druid,Director,Big Bandit,xefsstuff,Elite Minigun" + " Character:Choose" + " Crown:crown of life" + " Weapon:cloud blaster" + " Seed:27501",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
