...
/*@rMods@w=#  Druid#  Drop Me Gun#  Cursed Hyper Crystal#  Freak Arena#  Cool Shadows#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  splinter pistol#@pSeed@w=7998#Limited Tries:false*/
/*|Druid[Not Getafix,#but he'll do.]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]|Freak Arena[Do you think you can survive#in the @qFREAK ARENA???]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Druid,Drop Me Gun,Cursed Hyper Crystal,Freak Arena,Cool Shadows");
trace("Character:Choose");
trace("Crown:none");
trace("Weapon:splinter pistol");
trace("Seed:7998");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("splinter pistol"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Druid,Drop Me Gun,Cursed Hyper Crystal,Freak Arena,Cool Shadows" + " Character:Choose" + " Crown:none" + " Weapon:splinter pistol" + " Seed:7998";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-04 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Druid,Drop Me Gun,Cursed Hyper Crystal,Freak Arena,Cool Shadows" + " Character:Choose" + " Crown:none" + " Weapon:splinter pistol" + " Seed:7998" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Druid,Drop Me Gun,Cursed Hyper Crystal,Freak Arena,Cool Shadows" + " Character:Choose" + " Crown:none" + " Weapon:splinter pistol" + " Seed:7998",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
