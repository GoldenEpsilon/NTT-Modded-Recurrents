...
/*@rMods@w=#  Bitpack#  Dummy#  Colored Projectiles#  Crazy Allies#  CoDeath#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  industrialshankgun#@pSeed@w=10792#Limited Tries:false*/
/*|Bitpack[idk, bitflux made this]|Dummy[No strings on me!]|Colored Projectiles[Colors projectiles based on team color]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/Dummy/dummy.race.gml"));
...
trace("Current Daily:");
trace("Mods=Bitpack,Dummy,Colored Projectiles,Crazy Allies,CoDeath");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bitpack,Dummy,Colored Projectiles,Crazy Allies,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:industrialshankgun" + " Seed:10792";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bitpack,Dummy,Colored Projectiles,Crazy Allies,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:industrialshankgun" + " Seed:10792" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bitpack,Dummy,Colored Projectiles,Crazy Allies,CoDeath" + " Character:Choose" + " Crown:CoDeath" + " Weapon:industrialshankgun" + " Seed:10792",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
