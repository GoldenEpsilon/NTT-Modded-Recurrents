...
/*@rMods@w=#  bouncermini#  CoDeath#  Cool Hud#  Bigger Bandit#  Crypt Of The Necrodancer#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  super slugger#@pSeed@w=5857#Limited Tries:false*/
/*|bouncermini[FILL THE SKIES WITH BULLETS]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Cool Hud[*C418 starts playing*]|Bigger Bandit[Ever gotten bored of Big Dog?]|Crypt Of The Necrodancer[Move to the beat!]*/
#define init
...
trace("Current Daily:");
trace("Mods=bouncermini,CoDeath,Cool Hud,Bigger Bandit,Crypt Of The Necrodancer");
trace("Character:Choose");
trace("Crown:CoDeath");
trace("Weapon:super slugger");
trace("Seed:5857");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("super slugger"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bouncermini,CoDeath,Cool Hud,Bigger Bandit,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:CoDeath" + " Weapon:super slugger" + " Seed:5857";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-29 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bouncermini,CoDeath,Cool Hud,Bigger Bandit,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:CoDeath" + " Weapon:super slugger" + " Seed:5857" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bouncermini,CoDeath,Cool Hud,Bigger Bandit,Crypt Of The Necrodancer" + " Character:Choose" + " Crown:CoDeath" + " Weapon:super slugger" + " Seed:5857",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
