...
/*@rMods@w=#  Crazy Bandits#  Colored Projectiles#  Brimthrone#  Elite Minigun#  Baller#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  brimthrone#@pSeed@w=9036#Limited Tries:false*/
/*|Crazy Bandits[THEY HAVE GUNS]|Colored Projectiles[Colors projectiles based on team color]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Elite Minigun[Man, it feels nice#using their own guns on them.]|Baller[Fire burns...#me like fire...]*/
#define init
...
trace("Current Daily:");
trace("Mods=Crazy Bandits,Colored Projectiles,Brimthrone,Elite Minigun,Baller");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Crazy Bandits,Colored Projectiles,Brimthrone,Elite Minigun,Baller" + " Character:Choose" + " Crown:crown of blood" + " Weapon:brimthrone" + " Seed:9036";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Crazy Bandits,Colored Projectiles,Brimthrone,Elite Minigun,Baller" + " Character:Choose" + " Crown:crown of blood" + " Weapon:brimthrone" + " Seed:9036" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Crazy Bandits,Colored Projectiles,Brimthrone,Elite Minigun,Baller" + " Character:Choose" + " Crown:crown of blood" + " Weapon:brimthrone" + " Seed:9036",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
