...
/*@rMods@w=#  Elite Turrets#  Elite Gamers#  Devastator#  Airhorn#  Disk Marrow#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Airhorn#@pSeed@w=24417#Limited Tries:false*/
/*|Elite Turrets[V.2.0.maybe-we-#got-all-the-bugs-#this-time]|Elite Gamers[The bad guys got#aiming classes]|Devastator[Changes the devastator#to make it more interesting]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Turrets,Elite Gamers,Devastator,Airhorn,Disk Marrow");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Turrets,Elite Gamers,Devastator,Airhorn,Disk Marrow" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Airhorn" + " Seed:24417";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-16 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Turrets,Elite Gamers,Devastator,Airhorn,Disk Marrow" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Airhorn" + " Seed:24417" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Turrets,Elite Gamers,Devastator,Airhorn,Disk Marrow" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Airhorn" + " Seed:24417",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
