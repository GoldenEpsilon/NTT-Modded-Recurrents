...
/*@rMods@w=#  C Ultras#  Cool Hud#  Area Health#  Big Dog Snipers#  Druid#@bCharacter@w=#  druid#@yCrown@w=#  crown of life#@gWeapon@w=#  shovel#@pSeed@w=2296#Limited Tries:false*/
/*|C Ultras[Adds a C ultra to all#the vanilla characters]|Cool Hud[*C418 starts playing*]|Area Health[A nice mod that shows how many enemies are left.]|Big Dog Snipers[The actual boss]|Druid[Not Getafix,#but he'll do.]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/bigdogsnipers.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Druid_Race/druid.race.gml"));
...
trace("Mods=C Ultras,Cool Hud,Area Health,Big Dog Snipers,Druid");
trace("Character:druid");
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("druid")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("druid"))){
					instance_destroy();
				}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:C Ultras,Cool Hud,Area Health,Big Dog Snipers,Druid" + " Character:druid" + " Crown:crown of life" + " Weapon:shovel" + " Seed:2296";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-02 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-06-02 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-06-02 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-02 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-06-02 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:C Ultras,Cool Hud,Area Health,Big Dog Snipers,Druid" + " Character:druid" + " Crown:crown of life" + " Weapon:shovel" + " Seed:2296" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":C Ultras,Cool Hud,Area Health,Big Dog Snipers,Druid" + " Character:druid" + " Crown:crown of life" + " Weapon:shovel" + " Seed:2296",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race="druid";}else{global.Race = race;}}
