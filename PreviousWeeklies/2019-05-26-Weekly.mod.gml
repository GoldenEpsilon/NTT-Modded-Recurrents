...
/*@rMods@w=#  Cool Shadows#  Camper Boys#  Buff#  Crazy Allies#  Cannoner#@bCharacter@w=#  crystal#@yCrown@w=#  crown of luck#@gWeapon@w=#  cannoner#@pSeed@w=9083#Limited Tries:false*/
/*|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]|Buff[Buffs to various weapons,#ultras, mutations,#and more]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Cannoner[EIGHT RADICAL SHOTS]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/coolshadows.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/camperboys.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/buff.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/gunwarrant.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/hardertokill.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/heavyauto.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/heavynadelauncher.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/hyperrifle.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/luckyshot.skill.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/projectilestyle.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/rogueultras.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/sharpteeth.skill.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/smartgun.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/steroidsultras.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/teleport.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Buff/waterboost.mod.gml"));
...
trace("Mods=Cool Shadows,Camper Boys,Buff,Crazy Allies,Cannoner");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Shadows,Camper Boys,Buff,Crazy Allies,Cannoner" + " Character:crystal" + " Crown:crown of luck" + " Weapon:cannoner" + " Seed:9083";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-26 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-05-26 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-05-26 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-26 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-05-26 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Shadows,Camper Boys,Buff,Crazy Allies,Cannoner" + " Character:crystal" + " Crown:crown of luck" + " Weapon:cannoner" + " Seed:9083" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Shadows,Camper Boys,Buff,Crazy Allies,Cannoner" + " Character:crystal" + " Crown:crown of luck" + " Weapon:cannoner" + " Seed:9083",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race="crystal";}else{global.Race = race;}}
