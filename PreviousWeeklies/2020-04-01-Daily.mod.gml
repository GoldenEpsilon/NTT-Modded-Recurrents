...
/*@rMods@w=#  Detailer#  bosshudredux#  Cool Dust#  bossbar#  Discs#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  Disc Gun#@pSeed@w=17231#Limited Tries:false*/
/*|Detailer[Adds pretty patches of things]|bosshudredux[For knowing how bad your weapons are,#every once in a while]|Cool Dust[It's not just dust, it's#COOL DUST]|bossbar[For when you want to know#just how close you were to looping]|Discs[Because, you know, we need#MORE discs killing us.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Detailer,bosshudredux,Cool Dust,bossbar,Discs");
trace("Character:Choose");
trace("Crown:crown of haste");
trace("Weapon:Disc Gun");
trace("Seed:17231");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Detailer,bosshudredux,Cool Dust,bossbar,Discs" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Disc Gun" + " Seed:17231";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-01 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Detailer,bosshudredux,Cool Dust,bossbar,Discs" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Disc Gun" + " Seed:17231" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Detailer,bosshudredux,Cool Dust,bossbar,Discs" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Disc Gun" + " Seed:17231",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
