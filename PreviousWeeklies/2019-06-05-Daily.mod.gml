...
/*@rMods@w=#  Double Laser Minigun#  Auto Revolver#  Dumb Shells#  Cuz#  Elite Dasher#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  doublelaser#@pSeed@w=31045#Limited Tries:false*/
/*|Double Laser Minigun[Great for personal rave parties!]|Auto Revolver[A truly auto weapon.]|Dumb Shells[And on that dumbshell,#it's time to start.]|Cuz[Has an energy shield, for some reason.]|Elite Dasher[@qPICK UP ALL THE THINGS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Laser Minigun,Auto Revolver,Dumb Shells,Cuz,Elite Dasher");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Laser Minigun,Auto Revolver,Dumb Shells,Cuz,Elite Dasher" + " Character:Choose" + " Crown:crown of protection" + " Weapon:doublelaser" + " Seed:31045";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-05 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Laser Minigun,Auto Revolver,Dumb Shells,Cuz,Elite Dasher" + " Character:Choose" + " Crown:crown of protection" + " Weapon:doublelaser" + " Seed:31045" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Laser Minigun,Auto Revolver,Dumb Shells,Cuz,Elite Dasher" + " Character:Choose" + " Crown:crown of protection" + " Weapon:doublelaser" + " Seed:31045",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
