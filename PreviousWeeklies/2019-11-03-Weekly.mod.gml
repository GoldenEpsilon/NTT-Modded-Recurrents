...
/*@rMods@w=#  Crypt Of The Necrodancer#  Fire Riders#  Cool Dust#  BAR#  Afterburn#@bCharacter@w=#  fireriders#@yCrown@w=#  crown of love#@gWeapon@w=#  Big Ass Revolver#@pSeed@w=24048#Limited Tries:false*/
/*|Crypt Of The Necrodancer[Move to the beat!]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Cool Dust[It's not just dust, it's#COOL DUST]|BAR[A Big Ass Revolver.]|Afterburn[Massively Buffs Flames]*/
#define init
...
trace("Mods=Crypt Of The Necrodancer,Fire Riders,Cool Dust,BAR,Afterburn");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Crypt Of The Necrodancer,Fire Riders,Cool Dust,BAR,Afterburn" + " Character:fireriders" + " Crown:crown of love" + " Weapon:Big Ass Revolver" + " Seed:24048";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-03 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-11-03 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-11-03 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-03 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-11-03 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Crypt Of The Necrodancer,Fire Riders,Cool Dust,BAR,Afterburn" + " Character:fireriders" + " Crown:crown of love" + " Weapon:Big Ass Revolver" + " Seed:24048" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Crypt Of The Necrodancer,Fire Riders,Cool Dust,BAR,Afterburn" + " Character:fireriders" + " Crown:crown of love" + " Weapon:Big Ass Revolver" + " Seed:24048",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("fireriders");}else{global.Race = race;}}
