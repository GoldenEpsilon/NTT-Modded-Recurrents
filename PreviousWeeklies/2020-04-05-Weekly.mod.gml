...
/*@rMods@w=#  Auto Revolver#  Challenge Test#  blackholes#  Ascension 10#  Double Time#@bCharacter@w=#  steroids#@yCrown@w=#  crown of guns#@gWeapon@w=#  Auto Revolver#@pSeed@w=8174#Limited Tries:false*/
/*|Auto Revolver[A truly auto weapon.]|Challenge Test[GOTTA GO FAST]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Ascension 10[ASCENSION LEVEL#10/15]|Double Time[GOTTAGOFAST]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/challenge_test.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/blackholes.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/ascension10.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/Double Time.mod.gml"));
...
trace("Mods=Auto Revolver,Challenge Test,blackholes,Ascension 10,Double Time");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Revolver,Challenge Test,blackholes,Ascension 10,Double Time" + " Character:steroids" + " Crown:crown of guns" + " Weapon:Auto Revolver" + " Seed:8174";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-05 Weekly.txt")){
		prevScores = string_load(global.alias + " 2020-04-05 Weekly.txt");
		while(!file_loaded(global.alias + " 2020-04-05 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-05 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2020-04-05 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Revolver,Challenge Test,blackholes,Ascension 10,Double Time" + " Character:steroids" + " Crown:crown of guns" + " Weapon:Auto Revolver" + " Seed:8174" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Revolver,Challenge Test,blackholes,Ascension 10,Double Time" + " Character:steroids" + " Crown:crown of guns" + " Weapon:Auto Revolver" + " Seed:8174",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
