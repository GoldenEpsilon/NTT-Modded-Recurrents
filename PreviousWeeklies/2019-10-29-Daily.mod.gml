...
/*@rMods@w=#  Disk Marrow#  Cannondemnation#  Auto Bullet Shotgun#  Camper Boys#  Blood#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=20269#Limited Tries:false*/
/*|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]|Blood[If Nuclear Throne had an M rating...]*/
#define init
...
trace("Current Daily:");
trace("Mods=Disk Marrow,Cannondemnation,Auto Bullet Shotgun,Camper Boys,Blood");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Disk Marrow,Cannondemnation,Auto Bullet Shotgun,Camper Boys,Blood" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Auto Bullet Shotgun" + " Seed:20269";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-29 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Disk Marrow,Cannondemnation,Auto Bullet Shotgun,Camper Boys,Blood" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Auto Bullet Shotgun" + " Seed:20269" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Disk Marrow,Cannondemnation,Auto Bullet Shotgun,Camper Boys,Blood" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Auto Bullet Shotgun" + " Seed:20269",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
