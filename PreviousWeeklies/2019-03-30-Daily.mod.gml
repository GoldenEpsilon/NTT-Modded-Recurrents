...
/*@rMods@w=#  Double Time#  Double Pop Gun#  C Ultras#  Bigger Bandit#  Big Dog#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  double_pop_gun#@pSeed@w=14691#Limited Tries:false*/
/*|Double Time[GOTTAGOFAST]|Double Pop Gun[Now you can tickle enemies#twice as fast!]|C Ultras[Adds a C ultra to all#the vanilla characters]|Bigger Bandit[Ever gotten bored of Big Dog?]|Big Dog[HE IS AWAKENED]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Time,Double Pop Gun,C Ultras,Bigger Bandit,Big Dog");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Time,Double Pop Gun,C Ultras,Bigger Bandit,Big Dog" + " Character:Choose" + " Crown:crown of luck" + " Weapon:double_pop_gun" + " Seed:14691";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Time,Double Pop Gun,C Ultras,Bigger Bandit,Big Dog" + " Character:Choose" + " Crown:crown of luck" + " Weapon:double_pop_gun" + " Seed:14691" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Time,Double Pop Gun,C Ultras,Bigger Bandit,Big Dog" + " Character:Choose" + " Crown:crown of luck" + " Weapon:double_pop_gun" + " Seed:14691",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
