...
/*@rMods@w=#  blaac48#  Carmageddon#  Challenge Mode#  Double Plasma Minigun#  Derpweps#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  supergrenadesnake#@pSeed@w=19294#Limited Tries:false*/
/*|blaac48[The jungle is deeper]|Carmageddon[WARNING: May contain drifting]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]|Double Plasma Minigun[For when you want to#go through even more ammo]|Derpweps[Derrrrrrrrrrrp]*/
#define init
...
trace("Current Daily:");
trace("Mods=blaac48,Carmageddon,Challenge Mode,Double Plasma Minigun,Derpweps");
trace("Character:Choose");
trace("Crown:crown of luck");
trace("Weapon:supergrenadesnake");
trace("Seed:19294");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("supergrenadesnake"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:blaac48,Carmageddon,Challenge Mode,Double Plasma Minigun,Derpweps" + " Character:Choose" + " Crown:crown of luck" + " Weapon:supergrenadesnake" + " Seed:19294";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:blaac48,Carmageddon,Challenge Mode,Double Plasma Minigun,Derpweps" + " Character:Choose" + " Crown:crown of luck" + " Weapon:supergrenadesnake" + " Seed:19294" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":blaac48,Carmageddon,Challenge Mode,Double Plasma Minigun,Derpweps" + " Character:Choose" + " Crown:crown of luck" + " Weapon:supergrenadesnake" + " Seed:19294",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
