...
/*@rMods@w=#  Doctor#  Balls#  Vanthro#  Blaac's Weapon Mutations#  Chips#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  laserwrench#@pSeed@w=7596#Limited Tries:false*/
/*|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]|Vanthro[...blame 4chan.]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]*/
#define init
...
trace("Current Daily:");
trace("Mods=Doctor,Balls,Vanthro,Blaac's Weapon Mutations,Chips");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Doctor,Balls,Vanthro,Blaac's Weapon Mutations,Chips" + " Character:Choose" + " Crown:crown of guns" + " Weapon:laserwrench" + " Seed:7596";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-28 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Doctor,Balls,Vanthro,Blaac's Weapon Mutations,Chips" + " Character:Choose" + " Crown:crown of guns" + " Weapon:laserwrench" + " Seed:7596" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Doctor,Balls,Vanthro,Blaac's Weapon Mutations,Chips" + " Character:Choose" + " Crown:crown of guns" + " Weapon:laserwrench" + " Seed:7596",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
