...
/*@rMods@w=#  Detailer#  Death Pit#  !ARMAGGEDNN#  Champions#  Elite Turrets#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  Inspector Sword#@pSeed@w=28962#Limited Tries:false*/
/*|Detailer[Adds pretty patches of things]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Elite Turrets[V.2.0.maybe-we-#got-all-the-bugs-#this-time]*/
#define init
...
trace("Current Daily:");
trace("Mods=Detailer,Death Pit,!ARMAGGEDNN,Champions,Elite Turrets");
trace("Character:Choose");
trace("Crown:deathpit");
trace("Weapon:Inspector Sword");
trace("Seed:28962");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Inspector Sword"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Detailer,Death Pit,!ARMAGGEDNN,Champions,Elite Turrets" + " Character:Choose" + " Crown:deathpit" + " Weapon:Inspector Sword" + " Seed:28962";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-27 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Detailer,Death Pit,!ARMAGGEDNN,Champions,Elite Turrets" + " Character:Choose" + " Crown:deathpit" + " Weapon:Inspector Sword" + " Seed:28962" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Detailer,Death Pit,!ARMAGGEDNN,Champions,Elite Turrets" + " Character:Choose" + " Crown:deathpit" + " Weapon:Inspector Sword" + " Seed:28962",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
