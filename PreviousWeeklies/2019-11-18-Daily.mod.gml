...
/*@rMods@w=#  Bullet Time#  Confuse#  Cool Hud#  Cricket#  Car#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Cricket#@pSeed@w=26373#Limited Tries:false*/
/*|Bullet Time[The closer you are to bullets#the slower time moves]|Confuse[I guess you had a bit#too much to drink last night, eh?]|Cool Hud[*C418 starts playing*]|Cricket[Batter up!#...or whatever they say for cricket.]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bullet Time,Confuse,Cool Hud,Cricket,Car");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bullet Time,Confuse,Cool Hud,Cricket,Car" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Cricket" + " Seed:26373";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bullet Time,Confuse,Cool Hud,Cricket,Car" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Cricket" + " Seed:26373" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bullet Time,Confuse,Cool Hud,Cricket,Car" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Cricket" + " Seed:26373",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
