...
/*@rMods@w=#  Dodge Roll#  Detailer#  Devastator#  Crypt Of The Necrodancer#  Ancient Hatred#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  grenade launcher#@pSeed@w=1659#Limited Tries:false*/
/*|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]|Detailer[Adds pretty patches of things]|Devastator[Changes the devastator#to make it more interesting]|Crypt Of The Necrodancer[Move to the beat!]|Ancient Hatred[Crown that trades health for rads over time]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dodge Roll,Detailer,Devastator,Crypt Of The Necrodancer,Ancient Hatred");
trace("Character:Choose");
trace("Crown:crown of ancient hatred");
trace("Weapon:grenade launcher");
trace("Seed:1659");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("grenade launcher"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dodge Roll,Detailer,Devastator,Crypt Of The Necrodancer,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:grenade launcher" + " Seed:1659";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-24 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-24 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-24 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-24 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-24 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dodge Roll,Detailer,Devastator,Crypt Of The Necrodancer,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:grenade launcher" + " Seed:1659" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dodge Roll,Detailer,Devastator,Crypt Of The Necrodancer,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:grenade launcher" + " Seed:1659",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
