...
/*@rMods@w=#  C Ultras#  Drop Me Gun#  Cannondemnation#  Dodge Roll#  Double Laser Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  doublelaser#@pSeed@w=9464#Limited Tries:false*/
/*|C Ultras[Adds a C ultra to all#the vanilla characters]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]|Double Laser Minigun[Great for personal rave parties!]*/
#define init
...
trace("Current Daily:");
trace("Mods=C Ultras,Drop Me Gun,Cannondemnation,Dodge Roll,Double Laser Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:C Ultras,Drop Me Gun,Cannondemnation,Dodge Roll,Double Laser Minigun" + " Character:Choose" + " Crown:crown of love" + " Weapon:doublelaser" + " Seed:9464";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-18 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:C Ultras,Drop Me Gun,Cannondemnation,Dodge Roll,Double Laser Minigun" + " Character:Choose" + " Crown:crown of love" + " Weapon:doublelaser" + " Seed:9464" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":C Ultras,Drop Me Gun,Cannondemnation,Dodge Roll,Double Laser Minigun" + " Character:Choose" + " Crown:crown of love" + " Weapon:doublelaser" + " Seed:9464",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
