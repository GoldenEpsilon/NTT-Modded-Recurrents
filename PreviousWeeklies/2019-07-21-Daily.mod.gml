...
/*@rMods@w=#  Afterburn#  Auto Bullet Shotgun#  Carpocalypse#  Airhorn#  Bitpack#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=24304#Limited Tries:false*/
/*|Afterburn[Massively Buffs Flames]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Carpocalypse[Beep Beep, motherf***er]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Bitpack[idk, bitflux made this]*/
#define init
...
trace("Current Daily:");
trace("Mods=Afterburn,Auto Bullet Shotgun,Carpocalypse,Airhorn,Bitpack");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Afterburn,Auto Bullet Shotgun,Carpocalypse,Airhorn,Bitpack" + " Character:Choose" + " Crown:crown of life" + " Weapon:Auto Bullet Shotgun" + " Seed:24304";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-21 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Afterburn,Auto Bullet Shotgun,Carpocalypse,Airhorn,Bitpack" + " Character:Choose" + " Crown:crown of life" + " Weapon:Auto Bullet Shotgun" + " Seed:24304" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Afterburn,Auto Bullet Shotgun,Carpocalypse,Airhorn,Bitpack" + " Character:Choose" + " Crown:crown of life" + " Weapon:Auto Bullet Shotgun" + " Seed:24304",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
