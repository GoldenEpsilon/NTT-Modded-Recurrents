...
/*@rMods@w=#  Bitpack#  Cuz#  Cool Shadows#  Chrononaut#  Assault Flak Cannon#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Assault Flak Cannon#@pSeed@w=18681#Limited Tries:false*/
/*|Bitpack[idk, bitflux made this]|Cuz[Has an energy shield, for some reason.]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bitpack,Cuz,Cool Shadows,Chrononaut,Assault Flak Cannon");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bitpack,Cuz,Cool Shadows,Chrononaut,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Assault Flak Cannon" + " Seed:18681";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-17 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bitpack,Cuz,Cool Shadows,Chrononaut,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Assault Flak Cannon" + " Seed:18681" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bitpack,Cuz,Cool Shadows,Chrononaut,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Assault Flak Cannon" + " Seed:18681",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
