...
/*@rMods@w=#  Fire Riders#  Car#  bow#  Carpocalypse#  Bigger Bandit#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  bow#@pSeed@w=1022#Limited Tries:false*/
/*|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]|bow[It's a bow.]|Carpocalypse[Beep Beep, motherf***er]|Bigger Bandit[Ever gotten bored of Big Dog?]*/
#define init
...
while (!file_loaded(string_trim(data+"fireriders.race.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"Car.race.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"bow.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"carpocalypse.mod.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"biggerbandit.mod.gml"))) {wait 1;}
...
trace("Current Daily:");
trace("Mods=Fire Riders,Car,bow,Carpocalypse,Bigger Bandit");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Fire Riders,Car,bow,Carpocalypse,Bigger Bandit" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:bow" + " Seed:1022";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Fire Riders,Car,bow,Carpocalypse,Bigger Bandit" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:bow" + " Seed:1022" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Fire Riders,Car,bow,Carpocalypse,Bigger Bandit" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:bow" + " Seed:1022",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
