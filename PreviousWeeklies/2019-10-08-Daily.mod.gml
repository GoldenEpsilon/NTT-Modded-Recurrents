...
/*@rMods@w=#  Ascension 10#  blackholes#  Some Crowns#  Death Pit#  Crazy Allies#@bCharacter@w=#  Choose#@yCrown@w=#  carnage#@gWeapon@w=#  ultra shovel#@pSeed@w=14801#Limited Tries:false*/
/*|Ascension 10[ASCENSION LEVEL#10/15]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Some Crowns[Adds... Some Crowns.#dunno what you expected]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ascension 10,blackholes,Some Crowns,Death Pit,Crazy Allies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ascension 10,blackholes,Some Crowns,Death Pit,Crazy Allies" + " Character:Choose" + " Crown:carnage" + " Weapon:ultra shovel" + " Seed:14801";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-08 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ascension 10,blackholes,Some Crowns,Death Pit,Crazy Allies" + " Character:Choose" + " Crown:carnage" + " Weapon:ultra shovel" + " Seed:14801" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ascension 10,blackholes,Some Crowns,Death Pit,Crazy Allies" + " Character:Choose" + " Crown:carnage" + " Weapon:ultra shovel" + " Seed:14801",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
