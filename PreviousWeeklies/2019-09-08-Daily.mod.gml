...
/*@rMods@w=#  Ash#  Cannondemnation#  brad#  Cooler Hud#  bouncybolts#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  cannondemnation#@pSeed@w=22887#Limited Tries:false*/
/*|Ash[Guy from The End Is Nigh.]|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|brad[Adds Brad from LISA: the Painful RPG]|Cooler Hud[Symphony of the Throne]|bouncybolts[Upgrade to Bolt Marrow]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ash,Cannondemnation,brad,Cooler Hud,bouncybolts");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ash,Cannondemnation,brad,Cooler Hud,bouncybolts" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannondemnation" + " Seed:22887";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-08 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-08 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-08 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-08 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-08 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ash,Cannondemnation,brad,Cooler Hud,bouncybolts" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannondemnation" + " Seed:22887" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ash,Cannondemnation,brad,Cooler Hud,bouncybolts" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannondemnation" + " Seed:22887",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
