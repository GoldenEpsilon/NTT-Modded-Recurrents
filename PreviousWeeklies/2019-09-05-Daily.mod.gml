...
/*@rMods@w=#  CoD Proper#  CoDeath#  Cosmi#  Cool Dark#  Dodge Roll#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  golden laser pistol#@pSeed@w=15509#Limited Tries:false*/
/*|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Cosmi[*insert *insert *insert self insert joke here* joke here* joke here*]|Cool Dark[spoopy time]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]*/
#define init
...
trace("Current Daily:");
trace("Mods=CoD Proper,CoDeath,Cosmi,Cool Dark,Dodge Roll");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:CoD Proper,CoDeath,Cosmi,Cool Dark,Dodge Roll" + " Character:Choose" + " Crown:CoDeath" + " Weapon:golden laser pistol" + " Seed:15509";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-05 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:CoD Proper,CoDeath,Cosmi,Cool Dark,Dodge Roll" + " Character:Choose" + " Crown:CoDeath" + " Weapon:golden laser pistol" + " Seed:15509" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":CoD Proper,CoDeath,Cosmi,Cool Dark,Dodge Roll" + " Character:Choose" + " Crown:CoDeath" + " Weapon:golden laser pistol" + " Seed:15509",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
