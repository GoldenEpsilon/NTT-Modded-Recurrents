...
/*@rMods@w=#  Carmageddon#  Beamer#  Double Maxigun#  Car#  BadEnd#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  Beamer#@pSeed@w=21869#Limited Tries:false*/
/*|Carmageddon[WARNING: May contain drifting]|Beamer[BZAAAAAP]|Double Maxigun[Because miniguns#aren't cool enough]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]|BadEnd[For when you don't want a good end.#(sit after loop 3)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Carmageddon,Beamer,Double Maxigun,Car,BadEnd");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carmageddon,Beamer,Double Maxigun,Car,BadEnd" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Beamer" + " Seed:21869";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-06 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carmageddon,Beamer,Double Maxigun,Car,BadEnd" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Beamer" + " Seed:21869" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carmageddon,Beamer,Double Maxigun,Car,BadEnd" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Beamer" + " Seed:21869",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
