...
/*@rMods@w=#  Ascension 15#  Ascension 10#  Disk Marrow#  Dasher#  Blaac's Weapon Mutations#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  ultra laser pistol#@pSeed@w=19674#Limited Tries:false*/
/*|Ascension 15[@qASCENSION LEVEL#@q15/15]|Ascension 10[ASCENSION LEVEL#10/15]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ascension 15,Ascension 10,Disk Marrow,Dasher,Blaac's Weapon Mutations");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ascension 15,Ascension 10,Disk Marrow,Dasher,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of risk" + " Weapon:ultra laser pistol" + " Seed:19674";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-20 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ascension 15,Ascension 10,Disk Marrow,Dasher,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of risk" + " Weapon:ultra laser pistol" + " Seed:19674" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ascension 15,Ascension 10,Disk Marrow,Dasher,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of risk" + " Weapon:ultra laser pistol" + " Seed:19674",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
