...
/*@rMods@w=#  Bomber#  Detonator B#  Bigger Bandit#  Bioluminary Oscillator#  Bullet Time#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Bioluminary Oscillator#@pSeed@w=22759#Limited Tries:false*/
/*|Bomber[Bombs?]|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|Bigger Bandit[Ever gotten bored of Big Dog?]|Bioluminary Oscillator[TARGET AQUIRED]|Bullet Time[The closer you are to bullets#the slower time moves]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bomber,Detonator B,Bigger Bandit,Bioluminary Oscillator,Bullet Time");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bomber,Detonator B,Bigger Bandit,Bioluminary Oscillator,Bullet Time" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Bioluminary Oscillator" + " Seed:22759";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-25 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bomber,Detonator B,Bigger Bandit,Bioluminary Oscillator,Bullet Time" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Bioluminary Oscillator" + " Seed:22759" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bomber,Detonator B,Bigger Bandit,Bioluminary Oscillator,Bullet Time" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Bioluminary Oscillator" + " Seed:22759",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
