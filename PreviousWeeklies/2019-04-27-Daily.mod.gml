...
/*@rMods@w=#  Damage Numbers#  Colored Palace#  Chrononaut#  Discs#  Ancient Hatred#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  Disc Shotgun#@pSeed@w=15565#Limited Tries:false*/
/*|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Colored Palace[Gives a palace a#well-deserved paint job.]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Discs[Because, you know, we need#MORE discs killing us.]|Ancient Hatred[Crown that trades health for rads over time]*/
#define init
...
trace("Current Daily:");
trace("Mods=Damage Numbers,Colored Palace,Chrononaut,Discs,Ancient Hatred");
trace("Character:Choose");
trace("Crown:crown of ancient hatred");
trace("Weapon:Disc Shotgun");
trace("Seed:15565");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Disc Shotgun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Damage Numbers,Colored Palace,Chrononaut,Discs,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Disc Shotgun" + " Seed:15565";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-27 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Damage Numbers,Colored Palace,Chrononaut,Discs,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Disc Shotgun" + " Seed:15565" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Damage Numbers,Colored Palace,Chrononaut,Discs,Ancient Hatred" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Disc Shotgun" + " Seed:15565",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
