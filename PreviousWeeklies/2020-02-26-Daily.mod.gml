...
/*@rMods@w=#  AllCrowns#  Ash#  Druid#  Bomber#  Camper Boys#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  incinerator#@pSeed@w=5400#Limited Tries:false*/
/*|AllCrowns[Lets you choose your crown#out of all crowns#at a crown vault.]|Ash[Guy from The End Is Nigh.]|Druid[Not Getafix,#but he'll do.]|Bomber[Bombs?]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]*/
#define init
...
trace("Current Daily:");
trace("Mods=AllCrowns,Ash,Druid,Bomber,Camper Boys");
trace("Character:Choose");
trace("Crown:crown of destiny");
trace("Weapon:incinerator");
trace("Seed:5400");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("incinerator"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:AllCrowns,Ash,Druid,Bomber,Camper Boys" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:incinerator" + " Seed:5400";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-26 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:AllCrowns,Ash,Druid,Bomber,Camper Boys" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:incinerator" + " Seed:5400" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":AllCrowns,Ash,Druid,Bomber,Camper Boys" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:incinerator" + " Seed:5400",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
