...
/*@rMods@w=#  Cannondemnation#  banditstack#  Dasher#  Cool Trails#  Bandit Brute#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  cannondemnation#@pSeed@w=28937#Limited Tries:false*/
/*|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Bandit Brute[Randomly replaces bandits with Bandit Brutes]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cannondemnation,banditstack,Dasher,Cool Trails,Bandit Brute");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cannondemnation,banditstack,Dasher,Cool Trails,Bandit Brute" + " Character:Choose" + " Crown:none" + " Weapon:cannondemnation" + " Seed:28937";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-28 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cannondemnation,banditstack,Dasher,Cool Trails,Bandit Brute" + " Character:Choose" + " Crown:none" + " Weapon:cannondemnation" + " Seed:28937" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cannondemnation,banditstack,Dasher,Cool Trails,Bandit Brute" + " Character:Choose" + " Crown:none" + " Weapon:cannondemnation" + " Seed:28937",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
