...
/*@rMods@w=#  Car#  Ascension 13#  6 Weapons#  Hard Mode#  Dogfly#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  heavy idpd#@pSeed@w=366#Limited Tries:false*/
/*|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]|Ascension 13[@qASCENSION LEVEL#15/15]|6 Weapons[Gives you one weapon slot#for each ammo type like DOOM]|Hard Mode[Blaac's Hard Mode.#Good luck.#@r@q@qYou'll need it.]|Dogfly[The dog guardians#learned to jump good.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Car,Ascension 13,6 Weapons,Hard Mode,Dogfly");
trace("Character:Choose");
trace("Crown:crown of luck");
trace("Weapon:heavy idpd");
trace("Seed:366");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("heavy idpd"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Car,Ascension 13,6 Weapons,Hard Mode,Dogfly" + " Character:Choose" + " Crown:crown of luck" + " Weapon:heavy idpd" + " Seed:366";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-09 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Car,Ascension 13,6 Weapons,Hard Mode,Dogfly" + " Character:Choose" + " Crown:crown of luck" + " Weapon:heavy idpd" + " Seed:366" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Car,Ascension 13,6 Weapons,Hard Mode,Dogfly" + " Character:Choose" + " Crown:crown of luck" + " Weapon:heavy idpd" + " Seed:366",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
