...
/*@rMods@w=#  Elite Turrets 2#  Cycle#  Area Health#  Doctor#  Big Toothbrush#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  Big Toothbrush#@pSeed@w=1358#Limited Tries:false*/
/*|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]|Area Health[A nice mod that shows how many enemies are left.]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]|Big Toothbrush[a big toothbrush for big mouths]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Turrets 2,Cycle,Area Health,Doctor,Big Toothbrush");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Turrets 2,Cycle,Area Health,Doctor,Big Toothbrush" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Big Toothbrush" + " Seed:1358";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-22 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Turrets 2,Cycle,Area Health,Doctor,Big Toothbrush" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Big Toothbrush" + " Seed:1358" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Turrets 2,Cycle,Area Health,Doctor,Big Toothbrush" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Big Toothbrush" + " Seed:1358",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
