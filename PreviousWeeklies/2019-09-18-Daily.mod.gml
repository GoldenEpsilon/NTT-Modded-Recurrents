...
/*@rMods@w=#  Bandit Brute#  Some Crowns#  Cool Dust#  DIO#  Boltshard#@bCharacter@w=#  Choose#@yCrown@w=#  stamina#@gWeapon@w=#  Boltshard#@pSeed@w=22266#Limited Tries:false*/
/*|Bandit Brute[Randomly replaces bandits with Bandit Brutes]|Some Crowns[Adds... Some Crowns.#dunno what you expected]|Cool Dust[It's not just dust, it's#COOL DUST]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bandit Brute,Some Crowns,Cool Dust,DIO,Boltshard");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bandit Brute,Some Crowns,Cool Dust,DIO,Boltshard" + " Character:Choose" + " Crown:stamina" + " Weapon:Boltshard" + " Seed:22266";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-18 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-18 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-18 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-18 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-18 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bandit Brute,Some Crowns,Cool Dust,DIO,Boltshard" + " Character:Choose" + " Crown:stamina" + " Weapon:Boltshard" + " Seed:22266" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bandit Brute,Some Crowns,Cool Dust,DIO,Boltshard" + " Character:Choose" + " Crown:stamina" + " Weapon:Boltshard" + " Seed:22266",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
