...
/*@rMods@w=#  Drama Kill#  blaac48#  Dogfly#  Freak Arena#  Dodge Roll#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  golden revolver#@pSeed@w=26958#Limited Tries:false*/
/*|Drama Kill[For when you want#to feel really good#for defeating Big Bandit]|blaac48[The jungle is deeper]|Dogfly[The dog guardians#learned to jump good.]|Freak Arena[Do you think you can survive#in the @qFREAK ARENA???]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Drama Kill,blaac48,Dogfly,Freak Arena,Dodge Roll");
trace("Character:Choose");
trace("Crown:crown of blood");
trace("Weapon:golden revolver");
trace("Seed:26958");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("golden revolver"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Drama Kill,blaac48,Dogfly,Freak Arena,Dodge Roll" + " Character:Choose" + " Crown:crown of blood" + " Weapon:golden revolver" + " Seed:26958";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-13 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Drama Kill,blaac48,Dogfly,Freak Arena,Dodge Roll" + " Character:Choose" + " Crown:crown of blood" + " Weapon:golden revolver" + " Seed:26958" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Drama Kill,blaac48,Dogfly,Freak Arena,Dodge Roll" + " Character:Choose" + " Crown:crown of blood" + " Weapon:golden revolver" + " Seed:26958",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
