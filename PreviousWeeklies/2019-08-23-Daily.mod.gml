...
/*@rMods@w=#  Devastator#  Damage Numbers#  Brimthrone#  B#  Elite Gamers#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  brimthrone#@pSeed@w=532#Limited Tries:false*/
/*|Devastator[Changes the devastator#to make it more interesting]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|B[B]|Elite Gamers[The bad guys got#aiming classes]*/
#define init
...
trace("Current Daily:");
trace("Mods=Devastator,Damage Numbers,Brimthrone,B,Elite Gamers");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Devastator,Damage Numbers,Brimthrone,B,Elite Gamers" + " Character:Choose" + " Crown:none" + " Weapon:brimthrone" + " Seed:532";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-23 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-23 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-23 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-23 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-23 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Devastator,Damage Numbers,Brimthrone,B,Elite Gamers" + " Character:Choose" + " Crown:none" + " Weapon:brimthrone" + " Seed:532" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Devastator,Damage Numbers,Brimthrone,B,Elite Gamers" + " Character:Choose" + " Crown:none" + " Weapon:brimthrone" + " Seed:532",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
