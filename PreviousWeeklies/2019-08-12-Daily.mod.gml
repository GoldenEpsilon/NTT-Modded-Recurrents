...
/*@rMods@w=#  Elite Turrets 2#  Cannoner#  Airhorn#  Bioluminary Oscillator#  Baldi#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Bioluminary Oscillator#@pSeed@w=8656#Limited Tries:false*/
/*|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Cannoner[EIGHT RADICAL SHOTS]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Bioluminary Oscillator[TARGET AQUIRED]|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Turrets 2,Cannoner,Airhorn,Bioluminary Oscillator,Baldi");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Turrets 2,Cannoner,Airhorn,Bioluminary Oscillator,Baldi" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Bioluminary Oscillator" + " Seed:8656";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-12 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-12 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-12 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-12 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-12 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Turrets 2,Cannoner,Airhorn,Bioluminary Oscillator,Baldi" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Bioluminary Oscillator" + " Seed:8656" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Turrets 2,Cannoner,Airhorn,Bioluminary Oscillator,Baldi" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Bioluminary Oscillator" + " Seed:8656",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
