...
/*@rMods@w=#  Airhorn#  Area Health#  !ARMAGGEDNN#  basicgunallies#  Auto Revolver#@bCharacter@w=#  crystal#@yCrown@w=#  crown of luck#@gWeapon@w=#  Golden Shovel#@pSeed@w=1501*/
/*#*/
#define init
...
trace("Current Daily:");
trace("Mods=Airhorn,Area Health,!ARMAGGEDNN,basicgunallies,Auto Revolver");
trace("Character=crystal");
trace("Crown=crown of luck");
trace("Weapon=Golden Shovel");
trace("Seed=1501");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Golden Shovel"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.canStart){with(CharSelect){instance_change(CustomObject, 0);name=mod_current;race=string_lower(string_trim("crystal"));}}
else{with(instances_matching(CustomObject, "name", mod_current)){instance_change(CharSelect, 0);}with(CharSelect){race=string_lower(string_trim("crystal"));visible=true;}}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Airhorn,Area Health,!ARMAGGEDNN,basicgunallies,Auto Revolver" + " Character=crystal" + " Crown=crown of luck" + " Weapon=Golden Shovel" + " Seed=1501";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Airhorn,Area Health,!ARMAGGEDNN,basicgunallies,Auto Revolver" + " Character=crystal" + " Crown=crown of luck" + " Weapon=Golden Shovel" + " Seed=1501" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(leaderboard, "Mods:Airhorn,Area Health,!ARMAGGEDNN,basicgunallies,Auto Revolver" + " Character=crystal" + " Crown=crown of luck")) > 1){
			file_delete("sha.txt");
...
global.start = true;
with(Player){race=string_lower(string_trim("crystal"));}
give_crown();
