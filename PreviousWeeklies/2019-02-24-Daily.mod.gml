...
/*@rMods@w=#  banditstack#  Baldi#  BAR#  Babylon Shotgun#  Auto Revolver#@bCharacter@w=#  Baldi#@yCrown@w=#  crown of life#@gWeapon@w=#  Big Ass Revolver#@pSeed@w=14173*/
/*#*/
#define init
...
mod_load(string_trim("data/Daily.mod/banditstack.mod.gml"));
mod_load(string_trim("data/Daily.mod/Baldi/baldi.race.gml"));
mod_load(string_trim("data/Daily.mod/Baldi/ruler.wep.gml"));
mod_load(string_trim("data/Daily.mod/Baldi/ultraruler.wep.gml"));
...
trace("Current Daily:");
trace("Mods=banditstack,Baldi,BAR,Babylon Shotgun,Auto Revolver");
...
if(!global.canStart){with(CharSelect){instance_change(CustomObject, 0);name=mod_current;race=string_lower(string_trim("Baldi"));}}
else{with(instances_matching(CustomObject, "name", mod_current)){instance_change(CharSelect, 0);}with(CharSelect){race=string_lower(string_trim("Baldi"));visible=true;}}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:banditstack,Baldi,BAR,Babylon Shotgun,Auto Revolver" + " Character=Baldi" + " Crown=crown of life" + " Weapon=Big Ass Revolver" + " Seed=14173";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:banditstack,Baldi,BAR,Babylon Shotgun,Auto Revolver" + " Character=Baldi" + " Crown=crown of life" + " Weapon=Big Ass Revolver" + " Seed=14173" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(leaderboard, "Mods:banditstack,Baldi,BAR,Babylon Shotgun,Auto Revolver" + " Character=Baldi" + " Crown=crown of life")) > 1){
			file_delete("sha.txt");
...
global.start = true;
with(Player){race=string_lower(string_trim("Baldi"));}
give_crown();
