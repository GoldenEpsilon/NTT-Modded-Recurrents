...
/*@rMods@w=#  Bigger Bandit#  Discs#  Cuz#  Chest Bandits#  Camper Boys#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  Splinter Gun Disc Gun#@pSeed@w=508#Limited Tries:false*/
/*|Bigger Bandit[Ever gotten bored of Big Dog?]|Discs[Because, you know, we need#MORE discs killing us.]|Cuz[Has an energy shield, for some reason.]|Chest Bandits[Gets rid of bandits in random areas]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bigger Bandit,Discs,Cuz,Chest Bandits,Camper Boys");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bigger Bandit,Discs,Cuz,Chest Bandits,Camper Boys" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Splinter Gun Disc Gun" + " Seed:508";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-02 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-02 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-02 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-02 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-02 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bigger Bandit,Discs,Cuz,Chest Bandits,Camper Boys" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Splinter Gun Disc Gun" + " Seed:508" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bigger Bandit,Discs,Cuz,Chest Bandits,Camper Boys" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Splinter Gun Disc Gun" + " Seed:508",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
