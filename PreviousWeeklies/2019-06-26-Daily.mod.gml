...
/*@rMods@w=#  Elite Gamers#  Carmageddon#  Challenge Mode#  Disk Marrow#  Area Health#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  gun gun#@pSeed@w=5309#Limited Tries:false*/
/*|Elite Gamers[The bad guys got#aiming classes]|Carmageddon[WARNING: May contain drifting]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Area Health[A nice mod that shows how many enemies are left.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Elite Gamers,Carmageddon,Challenge Mode,Disk Marrow,Area Health");
trace("Character:Choose");
trace("Crown:crown of love");
trace("Weapon:gun gun");
trace("Seed:5309");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Gamers,Carmageddon,Challenge Mode,Disk Marrow,Area Health" + " Character:Choose" + " Crown:crown of love" + " Weapon:gun gun" + " Seed:5309";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-26 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Gamers,Carmageddon,Challenge Mode,Disk Marrow,Area Health" + " Character:Choose" + " Crown:crown of love" + " Weapon:gun gun" + " Seed:5309" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Gamers,Carmageddon,Challenge Mode,Disk Marrow,Area Health" + " Character:Choose" + " Crown:crown of love" + " Weapon:gun gun" + " Seed:5309",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
