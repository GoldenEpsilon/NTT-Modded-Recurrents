...
/*@rMods@w=#  Bigger Bandit#  Ash#  Cosmi Gun Enemies#  xefsstuff#  Double Maxigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  cicada#@pSeed@w=32454#Limited Tries:false*/
/*|Bigger Bandit[Ever gotten bored of Big Dog?]|Ash[Guy from The End Is Nigh.]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]|xefsstuff[This has a bit of everything.#A bunch of weapons, a few characters,#a butler robot#buddy thingamijig that kills guys...#@qDON'T GIVE HIM A GRENADE LAUNCHER OH N-]|Double Maxigun[Because miniguns#aren't cool enough]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bigger Bandit,Ash,Cosmi Gun Enemies,xefsstuff,Double Maxigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bigger Bandit,Ash,Cosmi Gun Enemies,xefsstuff,Double Maxigun" + " Character:Choose" + " Crown:crown of haste" + " Weapon:cicada" + " Seed:32454";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-13 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bigger Bandit,Ash,Cosmi Gun Enemies,xefsstuff,Double Maxigun" + " Character:Choose" + " Crown:crown of haste" + " Weapon:cicada" + " Seed:32454" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bigger Bandit,Ash,Cosmi Gun Enemies,xefsstuff,Double Maxigun" + " Character:Choose" + " Crown:crown of haste" + " Weapon:cicada" + " Seed:32454",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
