...
/*@rMods@w=#  Auto Bullet Shotgun#  Ally QOL#  Charge Template 2#  Cool Shadows#  Double Maxigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Double Maxigun#@pSeed@w=8586#Limited Tries:false*/
/*|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Ally QOL[Makes allies look nicer.#Because this only#really affects Rebel,#rebel is more likely#for this recurrent.]|Charge Template 2[Why aren't people more descriptive, like#"a shotgun that uses a pistol sprite that shoots#more bullets the more you charge that#happens to be a charge template"?]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Double Maxigun[Because miniguns#aren't cool enough]*/
#define init
...
trace("Current Daily:");
trace("Mods=Auto Bullet Shotgun,Ally QOL,Charge Template 2,Cool Shadows,Double Maxigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Bullet Shotgun,Ally QOL,Charge Template 2,Cool Shadows,Double Maxigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:Double Maxigun" + " Seed:8586";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Bullet Shotgun,Ally QOL,Charge Template 2,Cool Shadows,Double Maxigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:Double Maxigun" + " Seed:8586" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Bullet Shotgun,Ally QOL,Charge Template 2,Cool Shadows,Double Maxigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:Double Maxigun" + " Seed:8586",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
