...
/*@rMods@w=#  Dodge Roll#  Blindness#  Crazy Traps#  Elite Turrets 2#  Druid#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  grenade launcher#@pSeed@w=6047#Limited Tries:false*/
/*|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]|Blindness[MY GLASSES! I BROKE MY GLASSES!]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Druid[Not Getafix,#but he'll do.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dodge Roll,Blindness,Crazy Traps,Elite Turrets 2,Druid");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dodge Roll,Blindness,Crazy Traps,Elite Turrets 2,Druid" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:grenade launcher" + " Seed:6047";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-27 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dodge Roll,Blindness,Crazy Traps,Elite Turrets 2,Druid" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:grenade launcher" + " Seed:6047" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dodge Roll,Blindness,Crazy Traps,Elite Turrets 2,Druid" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:grenade launcher" + " Seed:6047",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
