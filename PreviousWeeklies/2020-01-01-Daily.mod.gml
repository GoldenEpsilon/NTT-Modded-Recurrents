...
/*@rMods@w=#  !ARMAGGEDNN#  Elite Turrets 2#  Crazy Bandits#  Big Bandit#  Afterburn#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Explosive Shovel#@pSeed@w=30245#Limited Tries:false*/
/*|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Crazy Bandits[THEY HAVE GUNS]|Big Bandit['Nuff said.]|Afterburn[Massively Buffs Flames]*/
#define init
...
trace("Current Daily:");
trace("Mods=!ARMAGGEDNN,Elite Turrets 2,Crazy Bandits,Big Bandit,Afterburn");
trace("Character:Choose");
trace("Crown:crown of protection");
trace("Weapon:Explosive Shovel");
trace("Seed:30245");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Explosive Shovel"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:!ARMAGGEDNN,Elite Turrets 2,Crazy Bandits,Big Bandit,Afterburn" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Explosive Shovel" + " Seed:30245";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-01 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:!ARMAGGEDNN,Elite Turrets 2,Crazy Bandits,Big Bandit,Afterburn" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Explosive Shovel" + " Seed:30245" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":!ARMAGGEDNN,Elite Turrets 2,Crazy Bandits,Big Bandit,Afterburn" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Explosive Shovel" + " Seed:30245",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
