...
/*@rMods@w=#  Derpweps#  Balls#  Cricket#  betterLoops#  Challenge Mode#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  laserwrench#@pSeed@w=22463#Limited Tries:false*/
/*|Derpweps[Derrrrrrrrrrrp]|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]|Cricket[Batter up!#...or whatever they say for cricket.]|betterLoops[Less enemies, more health per enemy in loops.]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Derpweps,Balls,Cricket,betterLoops,Challenge Mode");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Derpweps,Balls,Cricket,betterLoops,Challenge Mode" + " Character:Choose" + " Crown:crown of luck" + " Weapon:laserwrench" + " Seed:22463";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-14 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Derpweps,Balls,Cricket,betterLoops,Challenge Mode" + " Character:Choose" + " Crown:crown of luck" + " Weapon:laserwrench" + " Seed:22463" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Derpweps,Balls,Cricket,betterLoops,Challenge Mode" + " Character:Choose" + " Crown:crown of luck" + " Weapon:laserwrench" + " Seed:22463",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
