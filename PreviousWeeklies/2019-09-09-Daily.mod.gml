...
/*@rMods@w=#  Colored Projectiles#  Big Dog#  Auto Bullet Shotgun#  Camper Boys#  Babylon Shotgun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=6606#Limited Tries:false*/
/*|Colored Projectiles[Colors projectiles based on team color]|Big Dog[HE IS AWAKENED]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]|Babylon Shotgun[So much destruction your enemies will babble-on]*/
#define init
...
trace("Current Daily:");
trace("Mods=Colored Projectiles,Big Dog,Auto Bullet Shotgun,Camper Boys,Babylon Shotgun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Colored Projectiles,Big Dog,Auto Bullet Shotgun,Camper Boys,Babylon Shotgun" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Auto Bullet Shotgun" + " Seed:6606";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-09 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Colored Projectiles,Big Dog,Auto Bullet Shotgun,Camper Boys,Babylon Shotgun" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Auto Bullet Shotgun" + " Seed:6606" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Colored Projectiles,Big Dog,Auto Bullet Shotgun,Camper Boys,Babylon Shotgun" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Auto Bullet Shotgun" + " Seed:6606",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
