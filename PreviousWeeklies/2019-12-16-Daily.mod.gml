...
/*@rMods@w=#  Ascension 15#  Derpweps#  Boltshard#  Bandit#  Colored Palace#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  Boltshard#@pSeed@w=5707#Limited Tries:false*/
/*|Ascension 15[@qASCENSION LEVEL#@q15/15]|Derpweps[Derrrrrrrrrrrp]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|Colored Palace[Gives a palace a#well-deserved paint job.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ascension 15,Derpweps,Boltshard,Bandit,Colored Palace");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ascension 15,Derpweps,Boltshard,Bandit,Colored Palace" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Boltshard" + " Seed:5707";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-16 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ascension 15,Derpweps,Boltshard,Bandit,Colored Palace" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Boltshard" + " Seed:5707" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ascension 15,Derpweps,Boltshard,Bandit,Colored Palace" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Boltshard" + " Seed:5707",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
