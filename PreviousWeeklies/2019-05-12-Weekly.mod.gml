...
/*@rMods@w=#  Carpocalypse#  Bloodking#  betterLoops#  banditstack#  Cooler Hud#@bCharacter@w=#  bloodking#@yCrown@w=#  crown of guns#@gWeapon@w=#  golden laser pistol#@pSeed@w=10539#Limited Tries:false*/
/*|Carpocalypse[Beep Beep, motherf***er]|Bloodking[Blood, pain, and a half-finished character.]|betterLoops[Less enemies, more health per enemy in loops.]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]|Cooler Hud[Symphony of the Throne]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/Bloodking/fishexample.race.gml"));
wait mod_load(string_trim("data/Weekly.mod/betterLoops.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/banditstack.mod.gml"));
...
trace("Mods=Carpocalypse,Bloodking,betterLoops,banditstack,Cooler Hud");
trace("Character:bloodking");
trace("Crown:crown of guns");
trace("Weapon:golden laser pistol");
trace("Seed:10539");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("golden laser pistol"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("bloodking")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("bloodking"))){
					instance_destroy();
				}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carpocalypse,Bloodking,betterLoops,banditstack,Cooler Hud" + " Character:bloodking" + " Crown:crown of guns" + " Weapon:golden laser pistol" + " Seed:10539";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-12 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-05-12 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-05-12 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-12 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-05-12 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carpocalypse,Bloodking,betterLoops,banditstack,Cooler Hud" + " Character:bloodking" + " Crown:crown of guns" + " Weapon:golden laser pistol" + " Seed:10539" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carpocalypse,Bloodking,betterLoops,banditstack,Cooler Hud" + " Character:bloodking" + " Crown:crown of guns" + " Weapon:golden laser pistol" + " Seed:10539",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race="bloodking";}else{global.Race = race;}}
