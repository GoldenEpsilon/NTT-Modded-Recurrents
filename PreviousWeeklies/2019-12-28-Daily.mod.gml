...
/*@rMods@w=#  Ascension 5#  Cannoner#  boys#  Disk Marrow#  Cosmi Gun Enemies#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  cannoner#@pSeed@w=31998#Limited Tries:false*/
/*|Ascension 5[ASCENSION LEVEL#5/15]|Cannoner[EIGHT RADICAL SHOTS]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ascension 5,Cannoner,boys,Disk Marrow,Cosmi Gun Enemies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ascension 5,Cannoner,boys,Disk Marrow,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:31998";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-28 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ascension 5,Cannoner,boys,Disk Marrow,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:31998" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ascension 5,Cannoner,boys,Disk Marrow,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:cannoner" + " Seed:31998",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
