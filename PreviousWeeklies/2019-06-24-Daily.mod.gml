...
/*@rMods@w=#  Charge Template#  Chrononaut#  Cursed Hyper Crystal#  Bullet Time#  Bitpack#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  ct#@pSeed@w=15437#Limited Tries:false*/
/*|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]|Bullet Time[The closer you are to bullets#the slower time moves]|Bitpack[idk, bitflux made this]*/
#define init
...
trace("Current Daily:");
trace("Mods=Charge Template,Chrononaut,Cursed Hyper Crystal,Bullet Time,Bitpack");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Charge Template,Chrononaut,Cursed Hyper Crystal,Bullet Time,Bitpack" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:ct" + " Seed:15437";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-24 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-24 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-24 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-24 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-24 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Charge Template,Chrononaut,Cursed Hyper Crystal,Bullet Time,Bitpack" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:ct" + " Seed:15437" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Charge Template,Chrononaut,Cursed Hyper Crystal,Bullet Time,Bitpack" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:ct" + " Seed:15437",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
