...
/*@rMods@w=#  Defect#  Crazy Traps#  Bigger Bandit#  Babylon Shotgun#  Double Laser Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  Babylon Shotgun#@pSeed@w=5611#Limited Tries:false*/
/*|Defect[BEEP BOOP I AM ROBOT]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|Bigger Bandit[Ever gotten bored of Big Dog?]|Babylon Shotgun[So much destruction your enemies will babble-on]|Double Laser Minigun[Great for personal rave parties!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Defect,Crazy Traps,Bigger Bandit,Babylon Shotgun,Double Laser Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Defect,Crazy Traps,Bigger Bandit,Babylon Shotgun,Double Laser Minigun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Babylon Shotgun" + " Seed:5611";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-02 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-02 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-02 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-02 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-02 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Defect,Crazy Traps,Bigger Bandit,Babylon Shotgun,Double Laser Minigun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Babylon Shotgun" + " Seed:5611" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Defect,Crazy Traps,Bigger Bandit,Babylon Shotgun,Double Laser Minigun" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Babylon Shotgun" + " Seed:5611",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
