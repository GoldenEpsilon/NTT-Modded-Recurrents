...
/*@rMods@w=#  Charge Template 2#  Death Pit#  Crazy Traps#  DIO#  Dummy#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  charge_template#@pSeed@w=6367#Limited Tries:false*/
/*|Charge Template 2[Why aren't people more descriptive, like#"a shotgun that uses a pistol sprite that shoots#more bullets the more you charge that#happens to be a charge template"?]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Dummy[No strings on me!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Charge Template 2,Death Pit,Crazy Traps,DIO,Dummy");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Charge Template 2,Death Pit,Crazy Traps,DIO,Dummy" + " Character:Choose" + " Crown:deathpit" + " Weapon:charge_template" + " Seed:6367";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-12 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-12 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-12 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-12 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-12 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Charge Template 2,Death Pit,Crazy Traps,DIO,Dummy" + " Character:Choose" + " Crown:deathpit" + " Weapon:charge_template" + " Seed:6367" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Charge Template 2,Death Pit,Crazy Traps,DIO,Dummy" + " Character:Choose" + " Crown:deathpit" + " Weapon:charge_template" + " Seed:6367",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
