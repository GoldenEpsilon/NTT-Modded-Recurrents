...
/*@rMods@w=#  Dumb Shells#  Bandit Brute#  Baller#  Champions#  Chrononaut#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  idpdslugpistol#@pSeed@w=29581#Limited Tries:false*/
/*|Dumb Shells[And on that dumbshell,#it's time to start.]|Bandit Brute[Randomly replaces bandits with Bandit Brutes]|Baller[Fire burns...#me like fire...]|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Chrononaut[Wibbley Wobbley#Timey Wimey]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dumb Shells,Bandit Brute,Baller,Champions,Chrononaut");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dumb Shells,Bandit Brute,Baller,Champions,Chrononaut" + " Character:Choose" + " Crown:crown of curses" + " Weapon:idpdslugpistol" + " Seed:29581";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-29 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dumb Shells,Bandit Brute,Baller,Champions,Chrononaut" + " Character:Choose" + " Crown:crown of curses" + " Weapon:idpdslugpistol" + " Seed:29581" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dumb Shells,Bandit Brute,Baller,Champions,Chrononaut" + " Character:Choose" + " Crown:crown of curses" + " Weapon:idpdslugpistol" + " Seed:29581",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
