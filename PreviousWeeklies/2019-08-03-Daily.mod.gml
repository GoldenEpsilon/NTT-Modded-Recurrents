...
/*@rMods@w=#  Baldi#  CoDeath#  Drop Me Gun#  bow#  Dummy#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  bow#@pSeed@w=1798#Limited Tries:false*/
/*|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|bow[It's a bow.]|Dummy[No strings on me!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Baldi,CoDeath,Drop Me Gun,bow,Dummy");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Baldi,CoDeath,Drop Me Gun,bow,Dummy" + " Character:Choose" + " Crown:CoDeath" + " Weapon:bow" + " Seed:1798";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Baldi,CoDeath,Drop Me Gun,bow,Dummy" + " Character:Choose" + " Crown:CoDeath" + " Weapon:bow" + " Seed:1798" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Baldi,CoDeath,Drop Me Gun,bow,Dummy" + " Character:Choose" + " Crown:CoDeath" + " Weapon:bow" + " Seed:1798",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
