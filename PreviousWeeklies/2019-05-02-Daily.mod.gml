...
/*@rMods@w=#  Blood#  Detonator#  Disk Marrow#  Dasher#  Cannoner#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  cannoner#@pSeed@w=25453#Limited Tries:false*/
/*|Blood[If Nuclear Throne had an M rating...]|Detonator[Active has a radius#where when you release#it blows up your bullets]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Cannoner[EIGHT RADICAL SHOTS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Blood,Detonator,Disk Marrow,Dasher,Cannoner");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Blood,Detonator,Disk Marrow,Dasher,Cannoner" + " Character:Choose" + " Crown:crown of curses" + " Weapon:cannoner" + " Seed:25453";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-02 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-02 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-02 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-02 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-02 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Blood,Detonator,Disk Marrow,Dasher,Cannoner" + " Character:Choose" + " Crown:crown of curses" + " Weapon:cannoner" + " Seed:25453" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Blood,Detonator,Disk Marrow,Dasher,Cannoner" + " Character:Choose" + " Crown:crown of curses" + " Weapon:cannoner" + " Seed:25453",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
