...
/*@rMods@w=#  Druid#  Baller#  Colored Palace#  Challenge Mode#  Armageddon Cannon#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Armageddon Cannon#@pSeed@w=23917#Limited Tries:false*/
/*|Druid[Not Getafix,#but he'll do.]|Baller[Fire burns...#me like fire...]|Colored Palace[Gives a palace a#well-deserved paint job.]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]|Armageddon Cannon[Some people just want to watch the world burn...]*/
#define init
...
trace("Current Daily:");
trace("Mods=Druid,Baller,Colored Palace,Challenge Mode,Armageddon Cannon");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Druid,Baller,Colored Palace,Challenge Mode,Armageddon Cannon" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Armageddon Cannon" + " Seed:23917";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-27 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Druid,Baller,Colored Palace,Challenge Mode,Armageddon Cannon" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Armageddon Cannon" + " Seed:23917" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Druid,Baller,Colored Palace,Challenge Mode,Armageddon Cannon" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Armageddon Cannon" + " Seed:23917",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
