...
/*@rMods@w=#  Bigger Bandit#  BL2#  Charge Template#  Boltshard#  Big Bandit#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Boltshard#@pSeed@w=4873#Limited Tries:false*/
/*|Bigger Bandit[Ever gotten bored of Big Dog?]|BL2[Borderlands 2 x Nuclear Throne]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|Big Bandit['Nuff said.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bigger Bandit,BL2,Charge Template,Boltshard,Big Bandit");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bigger Bandit,BL2,Charge Template,Boltshard,Big Bandit" + " Character:Choose" + " Crown:crown of death" + " Weapon:Boltshard" + " Seed:4873";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-25 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bigger Bandit,BL2,Charge Template,Boltshard,Big Bandit" + " Character:Choose" + " Crown:crown of death" + " Weapon:Boltshard" + " Seed:4873" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bigger Bandit,BL2,Charge Template,Boltshard,Big Bandit" + " Character:Choose" + " Crown:crown of death" + " Weapon:Boltshard" + " Seed:4873",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
