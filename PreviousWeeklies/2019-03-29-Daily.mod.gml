...
/*@rMods@w=#  Chest Bandits#  C Ultras#  Bioluminary Oscillator#  Drop Me Gun#  bossbar#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  Bioluminary Oscillator#@pSeed@w=4658#Limited Tries:false*/
/*|Chest Bandits[Gets rid of bandits in random areas]|C Ultras[Adds a C ultra to all#the vanilla characters]|Bioluminary Oscillator[TARGET AQUIRED]|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|bossbar[For when you want to know#just how close you were to looping]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chest Bandits,C Ultras,Bioluminary Oscillator,Drop Me Gun,bossbar");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chest Bandits,C Ultras,Bioluminary Oscillator,Drop Me Gun,bossbar" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Bioluminary Oscillator" + " Seed:4658";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-29 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chest Bandits,C Ultras,Bioluminary Oscillator,Drop Me Gun,bossbar" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Bioluminary Oscillator" + " Seed:4658" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chest Bandits,C Ultras,Bioluminary Oscillator,Drop Me Gun,bossbar" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Bioluminary Oscillator" + " Seed:4658",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
