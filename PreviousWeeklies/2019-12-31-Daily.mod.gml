...
/*@rMods@w=#  Detonator B#  B#  Cycle#  Fire Riders#  Dasher#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  super disc gun#@pSeed@w=2349#Limited Tries:false*/
/*|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|B[B]|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Detonator B,B,Cycle,Fire Riders,Dasher");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Detonator B,B,Cycle,Fire Riders,Dasher" + " Character:Choose" + " Crown:crown of haste" + " Weapon:super disc gun" + " Seed:2349";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-31 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-31 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-31 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-31 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-31 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Detonator B,B,Cycle,Fire Riders,Dasher" + " Character:Choose" + " Crown:crown of haste" + " Weapon:super disc gun" + " Seed:2349" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Detonator B,B,Cycle,Fire Riders,Dasher" + " Character:Choose" + " Crown:crown of haste" + " Weapon:super disc gun" + " Seed:2349",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
