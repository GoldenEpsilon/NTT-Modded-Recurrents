...
/*@rMods@w=#  Cricket#  Chrononaut#  Dasher#  bow#  Dogfly#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  Cricket#@pSeed@w=1517#Limited Tries:false*/
/*|Cricket[Batter up!#...or whatever they say for cricket.]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|bow[It's a bow.]|Dogfly[The dog guardians#learned to jump good.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cricket,Chrononaut,Dasher,bow,Dogfly");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cricket,Chrononaut,Dasher,bow,Dogfly" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Cricket" + " Seed:1517";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-25 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cricket,Chrononaut,Dasher,bow,Dogfly" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Cricket" + " Seed:1517" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cricket,Chrononaut,Dasher,bow,Dogfly" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Cricket" + " Seed:1517",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
