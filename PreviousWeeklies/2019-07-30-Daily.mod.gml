...
/*@rMods@w=#  Belly#  Crypt Of The Necrodancer#  Chrononaut#  Cricket#  Chips#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  idpdslugpistol#@pSeed@w=24740#Limited Tries:false*/
/*|Belly[The only survivor of the#meme that is melee actives]|Crypt Of The Necrodancer[Move to the beat!]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Cricket[Batter up!#...or whatever they say for cricket.]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]*/
#define init
...
trace("Current Daily:");
trace("Mods=Belly,Crypt Of The Necrodancer,Chrononaut,Cricket,Chips");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Belly,Crypt Of The Necrodancer,Chrononaut,Cricket,Chips" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:idpdslugpistol" + " Seed:24740";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-30 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Belly,Crypt Of The Necrodancer,Chrononaut,Cricket,Chips" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:idpdslugpistol" + " Seed:24740" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Belly,Crypt Of The Necrodancer,Chrononaut,Cricket,Chips" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:idpdslugpistol" + " Seed:24740",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
