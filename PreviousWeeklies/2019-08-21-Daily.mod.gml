...
/*@rMods@w=#  Auto Bullet Shotgun#  Chrononaut#  Elite Gamers#  Dodge Roll#  Baldi#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  Baldi/ultraruler#@pSeed@w=995#Limited Tries:false*/
/*|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Elite Gamers[The bad guys got#aiming classes]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Auto Bullet Shotgun,Chrononaut,Elite Gamers,Dodge Roll,Baldi");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Bullet Shotgun,Chrononaut,Elite Gamers,Dodge Roll,Baldi" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Baldi/ultraruler" + " Seed:995";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-21 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Bullet Shotgun,Chrononaut,Elite Gamers,Dodge Roll,Baldi" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Baldi/ultraruler" + " Seed:995" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Bullet Shotgun,Chrononaut,Elite Gamers,Dodge Roll,Baldi" + " Character:Choose" + " Crown:crown of luck" + " Weapon:Baldi/ultraruler" + " Seed:995",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
