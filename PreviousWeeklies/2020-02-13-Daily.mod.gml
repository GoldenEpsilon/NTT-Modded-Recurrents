...
/*@rMods@w=#  Damage Numbers#  Charge Template 2#  Cool Dust#  Ascension 5#  Blaac's Weapon Mutations#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  charge_template#@pSeed@w=27798#Limited Tries:false*/
/*|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Charge Template 2[Why aren't people more descriptive, like#"a shotgun that uses a pistol sprite that shoots#more bullets the more you charge that#happens to be a charge template"?]|Cool Dust[It's not just dust, it's#COOL DUST]|Ascension 5[ASCENSION LEVEL#5/15]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Damage Numbers,Charge Template 2,Cool Dust,Ascension 5,Blaac's Weapon Mutations");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Damage Numbers,Charge Template 2,Cool Dust,Ascension 5,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of curses" + " Weapon:charge_template" + " Seed:27798";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-13 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Damage Numbers,Charge Template 2,Cool Dust,Ascension 5,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of curses" + " Weapon:charge_template" + " Seed:27798" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Damage Numbers,Charge Template 2,Cool Dust,Ascension 5,Blaac's Weapon Mutations" + " Character:Choose" + " Crown:crown of curses" + " Weapon:charge_template" + " Seed:27798",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
