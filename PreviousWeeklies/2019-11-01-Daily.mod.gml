...
/*@rMods@w=#  Challenge Test#  Crazy Allies#  Bigger Bandit#  Chips#  Egg#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  heavy auto crossbow#@pSeed@w=20729#Limited Tries:false*/
/*|Challenge Test[GOTTA GO FAST]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Bigger Bandit[Ever gotten bored of Big Dog?]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Egg[You will GIVE#me an @qEGG]*/
#define init
...
trace("Current Daily:");
trace("Mods=Challenge Test,Crazy Allies,Bigger Bandit,Chips,Egg");
trace("Character:Choose");
trace("Crown:crown of love");
trace("Weapon:heavy auto crossbow");
trace("Seed:20729");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("heavy auto crossbow"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Challenge Test,Crazy Allies,Bigger Bandit,Chips,Egg" + " Character:Choose" + " Crown:crown of love" + " Weapon:heavy auto crossbow" + " Seed:20729";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Challenge Test,Crazy Allies,Bigger Bandit,Chips,Egg" + " Character:Choose" + " Crown:crown of love" + " Weapon:heavy auto crossbow" + " Seed:20729" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Challenge Test,Crazy Allies,Bigger Bandit,Chips,Egg" + " Character:Choose" + " Crown:crown of love" + " Weapon:heavy auto crossbow" + " Seed:20729",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
