...
/*@rMods@w=#  Double Time#  Ancient Hatred#  Derpweps#  Colored Palace#  BAR#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  flamesnake#@pSeed@w=25751#Limited Tries:false*/
/*|Double Time[GOTTAGOFAST]|Ancient Hatred[Crown that trades health for rads over time]|Derpweps[Derrrrrrrrrrrp]|Colored Palace[Gives a palace a#well-deserved paint job.]|BAR[A Big Ass Revolver.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Time,Ancient Hatred,Derpweps,Colored Palace,BAR");
trace("Character:Choose");
trace("Crown:crown of ancient hatred");
trace("Weapon:flamesnake");
trace("Seed:25751");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("flamesnake"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Time,Ancient Hatred,Derpweps,Colored Palace,BAR" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:flamesnake" + " Seed:25751";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-25 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Time,Ancient Hatred,Derpweps,Colored Palace,BAR" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:flamesnake" + " Seed:25751" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Time,Ancient Hatred,Derpweps,Colored Palace,BAR" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:flamesnake" + " Seed:25751",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
