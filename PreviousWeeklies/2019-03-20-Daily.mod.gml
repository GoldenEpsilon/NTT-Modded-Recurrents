...
/*@rMods@w=#  Cool Shadows#  blstorm#  Croc Man#  Auto Bullet Shotgun#  Double Pop Gun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  double_pop_gun#@pSeed@w=1628#Limited Tries:false*/
/*|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|blstorm[BLEED YOURSELF DRY]|Croc Man[From what I can tell you either#don't take damage from anything or#you die instantly because of that passive.]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Double Pop Gun[Now you can tickle enemies#twice as fast!]*/
#define init
...
while (!file_loaded(string_trim(data+"Croc_Man/crocman.race.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Walk.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Ultras.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Teleport.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Teleport.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Select.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Radial.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Portrait.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Map.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Idle.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Hurt.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/CrushHit.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Ankh.png"))) {wait 1;}
while (!file_loaded(string_trim(data+"Croc_Man/Ankh.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"Auto_Bullet_Shotgun.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"double_pop_gun.wep.gml"))) {wait 1;}
...
trace("Current Daily:");
trace("Mods=Cool Shadows,blstorm,Croc Man,Auto Bullet Shotgun,Double Pop Gun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Shadows,blstorm,Croc Man,Auto Bullet Shotgun,Double Pop Gun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:double_pop_gun" + " Seed:1628";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-20 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Shadows,blstorm,Croc Man,Auto Bullet Shotgun,Double Pop Gun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:double_pop_gun" + " Seed:1628" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Shadows,blstorm,Croc Man,Auto Bullet Shotgun,Double Pop Gun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:double_pop_gun" + " Seed:1628",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
