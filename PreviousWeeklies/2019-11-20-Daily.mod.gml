...
/*@rMods@w=#  Grappler#  Devastator#  Cool Trails#  Adjustable Ammo#  All Is Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  seeker shotgun#@pSeed@w=28354#Limited Tries:false*/
/*|Grappler[For when you really want to throw someone at someone else.]|Devastator[Changes the devastator#to make it more interesting]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Adjustable Ammo[Push the limits#of ammo storage!]|All Is Minigun[ALL WILL PERISH.#Inifinite fire rate,#no ammo concerns.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Grappler,Devastator,Cool Trails,Adjustable Ammo,All Is Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Grappler,Devastator,Cool Trails,Adjustable Ammo,All Is Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:seeker shotgun" + " Seed:28354";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-20 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Grappler,Devastator,Cool Trails,Adjustable Ammo,All Is Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:seeker shotgun" + " Seed:28354" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Grappler,Devastator,Cool Trails,Adjustable Ammo,All Is Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:seeker shotgun" + " Seed:28354",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
