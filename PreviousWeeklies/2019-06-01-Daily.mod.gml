...
/*@rMods@w=#  Chest Bandits#  Determination#  Elite Gamers#  Discs#  Doctor#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  Flame Disc Gun#@pSeed@w=32710#Limited Tries:false*/
/*|Chest Bandits[Gets rid of bandits in random areas]|Determination[Seeing this filler text#fills you with determination]|Elite Gamers[The bad guys got#aiming classes]|Discs[Because, you know, we need#MORE discs killing us.]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]*/
#define init
...
downloadmod("Determination/determination.skill.gml");
downloadmod("Determination/determinationbutton.png");
downloadmod("Determination/determinationicon.png");
...
wait mod_load(string_trim("data/Daily.mod/Determination/determination.skill.gml"));
wait mod_load(string_trim("data/Daily.mod/elitegamers.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Chest Bandits,Determination,Elite Gamers,Discs,Doctor");
trace("Character:Choose");
trace("Crown:crown of haste");
trace("Weapon:Flame Disc Gun");
trace("Seed:32710");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Flame Disc Gun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chest Bandits,Determination,Elite Gamers,Discs,Doctor" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Flame Disc Gun" + " Seed:32710";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chest Bandits,Determination,Elite Gamers,Discs,Doctor" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Flame Disc Gun" + " Seed:32710" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chest Bandits,Determination,Elite Gamers,Discs,Doctor" + " Character:Choose" + " Crown:crown of haste" + " Weapon:Flame Disc Gun" + " Seed:32710",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
