...
/*@rMods@w=#  Carmageddon#  C Ultras#  Crypt Of The Necrodancer#  Camper Boys#  All Is Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  eraser#@pSeed@w=18906#Limited Tries:false*/
/*|Carmageddon[WARNING: May contain drifting]|C Ultras[Adds a C ultra to all#the vanilla characters]|Crypt Of The Necrodancer[Move to the beat!]|Camper Boys[Convinces the vanilla mutants#to let the custom characters#warm up by the fire]|All Is Minigun[ALL WILL PERISH.#Inifinite fire rate,#no ammo concerns.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Carmageddon,C Ultras,Crypt Of The Necrodancer,Camper Boys,All Is Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carmageddon,C Ultras,Crypt Of The Necrodancer,Camper Boys,All Is Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:eraser" + " Seed:18906";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-10 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-10 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-10 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-10 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-10 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carmageddon,C Ultras,Crypt Of The Necrodancer,Camper Boys,All Is Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:eraser" + " Seed:18906" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carmageddon,C Ultras,Crypt Of The Necrodancer,Camper Boys,All Is Minigun" + " Character:Choose" + " Crown:crown of death" + " Weapon:eraser" + " Seed:18906",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
