...
/*@rMods@w=#  Elite Minigun#  Brimthrone#  Minecraft Area#  Dasher x10#  Baller#@bCharacter@w=#  dasherx10#@yCrown@w=#  crown of blood#@gWeapon@w=#  brimthrone#@pSeed@w=13140#Limited Tries:false*/
/*|Elite Minigun[Man, it feels nice#using their own guns on them.]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Minecraft Area[Who Love Minecraf]|Dasher x10[@qEXTREME DASHING]|Baller[Fire burns...#me like fire...]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/dasherx10.race.gml"));
wait mod_load(string_trim("data/Weekly.mod/baller.race.gml"));
...
trace("Mods=Elite Minigun,Brimthrone,Minecraft Area,Dasher x10,Baller");
trace("Character:dasherx10");
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("dasherx10")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("dasherx10"))){
					instance_destroy();
				}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Elite Minigun,Brimthrone,Minecraft Area,Dasher x10,Baller" + " Character:dasherx10" + " Crown:crown of blood" + " Weapon:brimthrone" + " Seed:13140";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-21 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-04-21 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-04-21 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-21 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-04-21 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Elite Minigun,Brimthrone,Minecraft Area,Dasher x10,Baller" + " Character:dasherx10" + " Crown:crown of blood" + " Weapon:brimthrone" + " Seed:13140" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Elite Minigun,Brimthrone,Minecraft Area,Dasher x10,Baller" + " Character:dasherx10" + " Crown:crown of blood" + " Weapon:brimthrone" + " Seed:13140",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race="dasherx10";}else{global.Race = race;}}
