...
/*@rMods@w=#  Belly#  Cannondemnation#  Crazy Allies#  bosshudredux#  Elite Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  elitemini#@pSeed@w=1261#Limited Tries:false*/
/*|Belly[The only survivor of the#meme that is melee actives]|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|bosshudredux[For knowing how bad your weapons are,#every once in a while]|Elite Minigun[Man, it feels nice#using their own guns on them.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Belly,Cannondemnation,Crazy Allies,bosshudredux,Elite Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Belly,Cannondemnation,Crazy Allies,bosshudredux,Elite Minigun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:elitemini" + " Seed:1261";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-14 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Belly,Cannondemnation,Crazy Allies,bosshudredux,Elite Minigun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:elitemini" + " Seed:1261" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Belly,Cannondemnation,Crazy Allies,bosshudredux,Elite Minigun" + " Character:Choose" + " Crown:crown of luck" + " Weapon:elitemini" + " Seed:1261",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
