...
/*@rMods@w=#  Drop Me Gun#  Crazy Allies#  Detonator B#  Big Toothbrush#  blaac48#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  Big Toothbrush#@pSeed@w=12845#Limited Tries:false*/
/*|Drop Me Gun[Hold "Pick up weapon"#for a couple seconds#to drop your weapon.]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|Big Toothbrush[a big toothbrush for big mouths]|blaac48[The jungle is deeper]*/
#define init
...
trace("Current Daily:");
trace("Mods=Drop Me Gun,Crazy Allies,Detonator B,Big Toothbrush,blaac48");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Drop Me Gun,Crazy Allies,Detonator B,Big Toothbrush,blaac48" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Big Toothbrush" + " Seed:12845";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Drop Me Gun,Crazy Allies,Detonator B,Big Toothbrush,blaac48" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Big Toothbrush" + " Seed:12845" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Drop Me Gun,Crazy Allies,Detonator B,Big Toothbrush,blaac48" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Big Toothbrush" + " Seed:12845",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
