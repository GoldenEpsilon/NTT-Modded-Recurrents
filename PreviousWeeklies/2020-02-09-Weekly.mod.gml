...
/*@rMods@w=#  Bioluminary Oscillator#  bow#  DIO#  Crazy Bandits#  Elite Minigun#@bCharacter@w=#  DIO#@yCrown@w=#  crown of hatred#@gWeapon@w=#  Bioluminary Oscillator#@pSeed@w=29408#Limited Tries:false*/
/*|Bioluminary Oscillator[TARGET AQUIRED]|bow[It's a bow.]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Crazy Bandits[THEY HAVE GUNS]|Elite Minigun[Man, it feels nice#using their own guns on them.]*/
#define init
...
trace("Mods=Bioluminary Oscillator,bow,DIO,Crazy Bandits,Elite Minigun");
trace("Character:DIO");
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("DIO")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("DIO"))){
					instance_destroy();
				}
				exit;
			}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bioluminary Oscillator,bow,DIO,Crazy Bandits,Elite Minigun" + " Character:DIO" + " Crown:crown of hatred" + " Weapon:Bioluminary Oscillator" + " Seed:29408";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-09 Weekly.txt")){
		prevScores = string_load(global.alias + " 2020-02-09 Weekly.txt");
		while(!file_loaded(global.alias + " 2020-02-09 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-09 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2020-02-09 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bioluminary Oscillator,bow,DIO,Crazy Bandits,Elite Minigun" + " Character:DIO" + " Crown:crown of hatred" + " Weapon:Bioluminary Oscillator" + " Seed:29408" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bioluminary Oscillator,bow,DIO,Crazy Bandits,Elite Minigun" + " Character:DIO" + " Crown:crown of hatred" + " Weapon:Bioluminary Oscillator" + " Seed:29408",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("DIO");}else{global.Race = race;}}
