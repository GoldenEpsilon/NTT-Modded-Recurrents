...
/*@rMods@w=#  Detonator B#  blaac48#  Derpweps#  BAR#  brad#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  lasersnake#@pSeed@w=29628#Limited Tries:false*/
/*|Detonator B[Alternate version, where#active blows up#your bullets anywhere]|blaac48[The jungle is deeper]|Derpweps[Derrrrrrrrrrrp]|BAR[A Big Ass Revolver.]|brad[Adds Brad from LISA: the Painful RPG]*/
#define init
...
trace("Current Daily:");
trace("Mods=Detonator B,blaac48,Derpweps,BAR,brad");
trace("Character:Choose");
trace("Crown:crown of protection");
trace("Weapon:lasersnake");
trace("Seed:29628");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("lasersnake"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Detonator B,blaac48,Derpweps,BAR,brad" + " Character:Choose" + " Crown:crown of protection" + " Weapon:lasersnake" + " Seed:29628";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Detonator B,blaac48,Derpweps,BAR,brad" + " Character:Choose" + " Crown:crown of protection" + " Weapon:lasersnake" + " Seed:29628" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Detonator B,blaac48,Derpweps,BAR,brad" + " Character:Choose" + " Crown:crown of protection" + " Weapon:lasersnake" + " Seed:29628",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
