...
/*@rMods@w=#  Bigger Bandit#  Ash#  Baldi#  Chrononaut#  Cosmi Gun Enemies#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  idpdslugpistol#@pSeed@w=27125#Limited Tries:false*/
/*|Bigger Bandit[Ever gotten bored of Big Dog?]|Ash[Guy from The End Is Nigh.]|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bigger Bandit,Ash,Baldi,Chrononaut,Cosmi Gun Enemies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bigger Bandit,Ash,Baldi,Chrononaut,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of haste" + " Weapon:idpdslugpistol" + " Seed:27125";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-22 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bigger Bandit,Ash,Baldi,Chrononaut,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of haste" + " Weapon:idpdslugpistol" + " Seed:27125" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bigger Bandit,Ash,Baldi,Chrononaut,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of haste" + " Weapon:idpdslugpistol" + " Seed:27125",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
