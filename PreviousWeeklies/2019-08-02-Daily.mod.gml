...
/*@rMods@w=#  Cool Dust#  Bitpack#  Croc Man#  Dogfly#  Crazy Bandits#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  industrialshankgun#@pSeed@w=15119#Limited Tries:false*/
/*|Cool Dust[It's not just dust, it's#COOL DUST]|Bitpack[idk, bitflux made this]|Croc Man[From what I can tell you either#don't take damage from anything or#you die instantly because of that passive.]|Dogfly[The dog guardians#learned to jump good.]|Crazy Bandits[THEY HAVE GUNS]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/cooldust.mod.gml"));
...
wait mod_load(string_trim("data/Daily.mod/crazybandits.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Cool Dust,Bitpack,Croc Man,Dogfly,Crazy Bandits");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Dust,Bitpack,Croc Man,Dogfly,Crazy Bandits" + " Character:Choose" + " Crown:none" + " Weapon:industrialshankgun" + " Seed:15119";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-02 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-02 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-02 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-02 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-02 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Dust,Bitpack,Croc Man,Dogfly,Crazy Bandits" + " Character:Choose" + " Crown:none" + " Weapon:industrialshankgun" + " Seed:15119" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Dust,Bitpack,Croc Man,Dogfly,Crazy Bandits" + " Character:Choose" + " Crown:none" + " Weapon:industrialshankgun" + " Seed:15119",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
