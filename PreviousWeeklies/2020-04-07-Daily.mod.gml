...
/*@rMods@w=#  Auto Revolver#  Auto Bullet Shotgun#  Blood#  Double Time#  Double Pop Gun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=14964#Limited Tries:false*/
/*|Auto Revolver[A truly auto weapon.]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Blood[If Nuclear Throne had an M rating...]|Double Time[GOTTAGOFAST]|Double Pop Gun[Now you can tickle enemies#twice as fast!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Auto Revolver,Auto Bullet Shotgun,Blood,Double Time,Double Pop Gun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Revolver,Auto Bullet Shotgun,Blood,Double Time,Double Pop Gun" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Auto Bullet Shotgun" + " Seed:14964";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-07 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-07 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-07 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-07 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-07 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Revolver,Auto Bullet Shotgun,Blood,Double Time,Double Pop Gun" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Auto Bullet Shotgun" + " Seed:14964" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Revolver,Auto Bullet Shotgun,Blood,Double Time,Double Pop Gun" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Auto Bullet Shotgun" + " Seed:14964",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
