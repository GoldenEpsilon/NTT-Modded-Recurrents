...
/*@rMods@w=#  Disk Marrow#  blackholes#  AllCrowns#  bouncermini#  Dumb Shells#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  laser minigun#@pSeed@w=5513#Limited Tries:false*/
/*|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|AllCrowns[Lets you choose your crown#out of all crowns#at a crown vault.]|bouncermini[FILL THE SKIES WITH BULLETS]|Dumb Shells[And on that dumbshell,#it's time to start.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Disk Marrow,blackholes,AllCrowns,bouncermini,Dumb Shells");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Disk Marrow,blackholes,AllCrowns,bouncermini,Dumb Shells" + " Character:Choose" + " Crown:crown of death" + " Weapon:laser minigun" + " Seed:5513";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-22 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Disk Marrow,blackholes,AllCrowns,bouncermini,Dumb Shells" + " Character:Choose" + " Crown:crown of death" + " Weapon:laser minigun" + " Seed:5513" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Disk Marrow,blackholes,AllCrowns,bouncermini,Dumb Shells" + " Character:Choose" + " Crown:crown of death" + " Weapon:laser minigun" + " Seed:5513",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
