...
trace("Mods=Lots O' Muts, Paper Mario, Level Balls, Freak Arena, Nuke Launcher");
trace("Character=nukelauncher");
trace("Crown=crwn_risk");
trace("Weapon=wep_heavy_revolver");
trace("Seed=32133");
...
file_download(string_trim("https://raw.githubusercontent.com/GoldenEpsilon/NTT-Modded-Recurrents/master/WeeklyMods/nukelauncher.race.gml"), string_trim("ShdBloom.mod.gml"));
...
while (!file_loaded(string_trim("nukelauncher.race.gml"))) {wait 1;}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Lots O' Muts, Paper Mario, Level Balls, Freak Arena, Bloom Shader" + " Character=char_rogue" + " Crown=crwn_risk" + " Weapon=wep_heavy_revolver" + " Seed=32133";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2018-10-28 weekly.txt")){
		prevScores = string_load(global.alias + " 2018-10-28 weekly.txt");
		while(!file_loaded(global.alias + " 2018-10-28 weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2018-10-28 weekly.txt");
	}else{
		string_save(score, global.alias + " 2018-10-28 weekly.txt");
	}
}
#define draw_pause
//Anti-Cheat
