...
/*@rMods@w=#  bosshudredux#  Fire Riders#  Chest Bandits#  DIO#  Challenge Mode#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  golden assault rifle#@pSeed@w=13007#Limited Tries:false*/
/*|bosshudredux[For knowing how bad your weapons are,#every once in a while]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Chest Bandits[Gets rid of bandits in random areas]|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]*/
#define init
...
trace("Current Daily:");
trace("Mods=bosshudredux,Fire Riders,Chest Bandits,DIO,Challenge Mode");
trace("Character:Choose");
trace("Crown:crown of hatred");
trace("Weapon:golden assault rifle");
trace("Seed:13007");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("golden assault rifle"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bosshudredux,Fire Riders,Chest Bandits,DIO,Challenge Mode" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:golden assault rifle" + " Seed:13007";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-04 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bosshudredux,Fire Riders,Chest Bandits,DIO,Challenge Mode" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:golden assault rifle" + " Seed:13007" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bosshudredux,Fire Riders,Chest Bandits,DIO,Challenge Mode" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:golden assault rifle" + " Seed:13007",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
