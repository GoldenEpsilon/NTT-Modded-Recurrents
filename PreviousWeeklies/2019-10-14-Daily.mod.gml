...
/*@rMods@w=#  Belly#  Cool Dust#  Airhorn#  Blaac's Weapon Mutations#  Grappler#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  Airhorn#@pSeed@w=1426#Limited Tries:false*/
/*|Belly[The only survivor of the#meme that is melee actives]|Cool Dust[It's not just dust, it's#COOL DUST]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|Grappler[For when you really want to throw someone at someone else.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Belly,Cool Dust,Airhorn,Blaac's Weapon Mutations,Grappler");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Belly,Cool Dust,Airhorn,Blaac's Weapon Mutations,Grappler" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Airhorn" + " Seed:1426";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-14 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Belly,Cool Dust,Airhorn,Blaac's Weapon Mutations,Grappler" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Airhorn" + " Seed:1426" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Belly,Cool Dust,Airhorn,Blaac's Weapon Mutations,Grappler" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Airhorn" + " Seed:1426",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
