...
/*@rMods@w=#  Charge Template 2#  Dumb Shells#  Big Dog Snipers#  Cool Dark#  Dasher#@bCharacter@w=#  dasher#@yCrown@w=#  crown of death#@gWeapon@w=#  charge_template#@pSeed@w=9144#Limited Tries:false*/
/*|Charge Template 2[Why aren't people more descriptive, like#"a shotgun that uses a pistol sprite that shoots#more bullets the more you charge that#happens to be a charge template"?]|Dumb Shells[And on that dumbshell,#it's time to start.]|Big Dog Snipers[The actual boss]|Cool Dark[spoopy time]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]*/
#define init
...
wait mod_load(string_trim("data/Weekly.mod/charge_template.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/dumbshells.mod.gml"));
...
trace("Mods=Charge Template 2,Dumb Shells,Big Dog Snipers,Cool Dark,Dasher");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Charge Template 2,Dumb Shells,Big Dog Snipers,Cool Dark,Dasher" + " Character:dasher" + " Crown:crown of death" + " Weapon:charge_template" + " Seed:9144";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-22 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-09-22 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-09-22 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-22 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-09-22 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Charge Template 2,Dumb Shells,Big Dog Snipers,Cool Dark,Dasher" + " Character:dasher" + " Crown:crown of death" + " Weapon:charge_template" + " Seed:9144" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Charge Template 2,Dumb Shells,Big Dog Snipers,Cool Dark,Dasher" + " Character:dasher" + " Crown:crown of death" + " Weapon:charge_template" + " Seed:9144",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
