...
/*@rMods@w=#  Airhorn#  Grappler#  Charge Template#  Fire Riders#  !ARMAGGEDNN#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  ct#@pSeed@w=9480#Limited Tries:false*/
/*|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Grappler[For when you really want to throw someone at someone else.]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Airhorn,Grappler,Charge Template,Fire Riders,!ARMAGGEDNN");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Airhorn,Grappler,Charge Template,Fire Riders,!ARMAGGEDNN" + " Character:Choose" + " Crown:crown of protection" + " Weapon:ct" + " Seed:9480";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-09 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Airhorn,Grappler,Charge Template,Fire Riders,!ARMAGGEDNN" + " Character:Choose" + " Crown:crown of protection" + " Weapon:ct" + " Seed:9480" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Airhorn,Grappler,Charge Template,Fire Riders,!ARMAGGEDNN" + " Character:Choose" + " Crown:crown of protection" + " Weapon:ct" + " Seed:9480",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
