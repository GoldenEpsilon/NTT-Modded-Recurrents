...
/*@rMods@w=#  Cannoner#  BAR#  Dasher x10#  Elite Turrets 2#  B#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Big Ass Revolver#@pSeed@w=24831#Limited Tries:false*/
/*|Cannoner[EIGHT RADICAL SHOTS]|BAR[A Big Ass Revolver.]|Dasher x10[@qEXTREME DASHING]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|B[B]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cannoner,BAR,Dasher x10,Elite Turrets 2,B");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cannoner,BAR,Dasher x10,Elite Turrets 2,B" + " Character:Choose" + " Crown:crown of death" + " Weapon:Big Ass Revolver" + " Seed:24831";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-22 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cannoner,BAR,Dasher x10,Elite Turrets 2,B" + " Character:Choose" + " Crown:crown of death" + " Weapon:Big Ass Revolver" + " Seed:24831" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cannoner,BAR,Dasher x10,Elite Turrets 2,B" + " Character:Choose" + " Crown:crown of death" + " Weapon:Big Ass Revolver" + " Seed:24831",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
