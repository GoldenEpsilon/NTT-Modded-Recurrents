...
/*@rMods@w=#  Adjustable Ammo#  Beamer#  Double Time#  Afterburn#  Challenge Test#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  Beamer#@pSeed@w=4955#Limited Tries:false*/
/*|Adjustable Ammo[Push the limits#of ammo storage!]|Beamer[BZAAAAAP]|Double Time[GOTTAGOFAST]|Afterburn[Massively Buffs Flames]|Challenge Test[GOTTA GO FAST]*/
#define init
...
trace("Current Daily:");
trace("Mods=Adjustable Ammo,Beamer,Double Time,Afterburn,Challenge Test");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Adjustable Ammo,Beamer,Double Time,Afterburn,Challenge Test" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Beamer" + " Seed:4955";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-02 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-02 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-02 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-02 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-02 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Adjustable Ammo,Beamer,Double Time,Afterburn,Challenge Test" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Beamer" + " Seed:4955" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Adjustable Ammo,Beamer,Double Time,Afterburn,Challenge Test" + " Character:Choose" + " Crown:crown of blood" + " Weapon:Beamer" + " Seed:4955",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
