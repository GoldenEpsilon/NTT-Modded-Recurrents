...
/*@rMods@w=#  Chaos Guns#  Crazy Traps#  Airhorn#  Ascension 10#  Director#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Airhorn#@pSeed@w=19683#Limited Tries:false*/
/*|Chaos Guns[For when you need a bit#more chaos in your life.]|Crazy Traps[Not only does#this place flame traps everywhere,#they have random projectiles, too!#Have fun with disc traps....]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Ascension 10[ASCENSION LEVEL#10/15]|Director[He will direct your enemies#to the nearest exit of life]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/ascension10.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Chaos Guns,Crazy Traps,Airhorn,Ascension 10,Director");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chaos Guns,Crazy Traps,Airhorn,Ascension 10,Director" + " Character:Choose" + " Crown:crown of death" + " Weapon:Airhorn" + " Seed:19683";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-13 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chaos Guns,Crazy Traps,Airhorn,Ascension 10,Director" + " Character:Choose" + " Crown:crown of death" + " Weapon:Airhorn" + " Seed:19683" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chaos Guns,Crazy Traps,Airhorn,Ascension 10,Director" + " Character:Choose" + " Crown:crown of death" + " Weapon:Airhorn" + " Seed:19683",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
