...
trace("Mods=NTT X10 Ver 1.0.1, Level Balls, Bandit Cannon, Nuke Launcher, MS Paint Shader");
trace("Character=char_venuz");
trace("Crown=crwn_hatred");
trace("Weapon=wep_golden_bazooka");
trace("Seed=116");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:NTT X10 Ver 1.0.1, Level Balls, Bandit Cannon, Nuke Launcher, MS Paint Shader" + " Character=char_venuz" + " Crown=crwn_hatred" + " Weapon=wep_golden_bazooka" + " Seed=116";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2018-12-23 weekly.txt")){
		prevScores = string_load(global.alias + " 2018-12-23 weekly.txt");
		while(!file_loaded(global.alias + " 2018-12-23 weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2018-12-23 weekly.txt");
	}else{
		string_save(score, global.alias + " 2018-12-23 weekly.txt");
	}
}
#define draw_pause
//Anti-Cheat
