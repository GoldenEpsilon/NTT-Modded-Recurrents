...
/*@rMods@w=#  Cannondemnation#  bouncybolts#  Cool Shadows#  Balls#  Colored Palace#@bCharacter@w=#  Choose#@yCrown@w=#  crown of life#@gWeapon@w=#  cannondemnation#@pSeed@w=6197#Limited Tries:false*/
/*|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|bouncybolts[Upgrade to Bolt Marrow]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Balls[Boing. Thwakk. Boing. Boing. Thwakk. CRASH!!!]|Colored Palace[Gives a palace a#well-deserved paint job.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cannondemnation,bouncybolts,Cool Shadows,Balls,Colored Palace");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cannondemnation,bouncybolts,Cool Shadows,Balls,Colored Palace" + " Character:Choose" + " Crown:crown of life" + " Weapon:cannondemnation" + " Seed:6197";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-14 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cannondemnation,bouncybolts,Cool Shadows,Balls,Colored Palace" + " Character:Choose" + " Crown:crown of life" + " Weapon:cannondemnation" + " Seed:6197" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cannondemnation,bouncybolts,Cool Shadows,Balls,Colored Palace" + " Character:Choose" + " Crown:crown of life" + " Weapon:cannondemnation" + " Seed:6197",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
