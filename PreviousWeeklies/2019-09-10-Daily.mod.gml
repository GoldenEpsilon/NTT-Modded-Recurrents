...
/*@rMods@w=#  Champions#  Determination#  Babylon Shotgun#  Elite Turrets 2#  Elite Dasher#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Babylon Shotgun#@pSeed@w=11434#Limited Tries:false*/
/*|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Determination[Seeing this filler text#fills you with determination]|Babylon Shotgun[So much destruction your enemies will babble-on]|Elite Turrets 2[Electric Boogaloo.#(affects Labs turrets)]|Elite Dasher[@qPICK UP ALL THE THINGS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Champions,Determination,Babylon Shotgun,Elite Turrets 2,Elite Dasher");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Champions,Determination,Babylon Shotgun,Elite Turrets 2,Elite Dasher" + " Character:Choose" + " Crown:crown of death" + " Weapon:Babylon Shotgun" + " Seed:11434";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-10 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-10 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-10 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-10 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-10 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Champions,Determination,Babylon Shotgun,Elite Turrets 2,Elite Dasher" + " Character:Choose" + " Crown:crown of death" + " Weapon:Babylon Shotgun" + " Seed:11434" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Champions,Determination,Babylon Shotgun,Elite Turrets 2,Elite Dasher" + " Character:Choose" + " Crown:crown of death" + " Weapon:Babylon Shotgun" + " Seed:11434",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
