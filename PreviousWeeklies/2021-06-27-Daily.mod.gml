...
/*@rMods@w=#  DIO#  Death Pit#  Elite Dasher#  Croc Man#  Director#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  ultra revolver#@pSeed@w=26375#Limited Tries:false*/
/*|DIO[You thought it was Fish,#@qBUT IT WAS ME, DIO!]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Elite Dasher[@qPICK UP ALL THE THINGS]|Croc Man[From what I can tell you either#don't take damage from anything or#you die instantly because of that passive.]|Director[He will direct your enemies#to the nearest exit of life]*/
#define init
...
trace("Current Daily:");
trace("Mods=DIO,Death Pit,Elite Dasher,Croc Man,Director");
trace("Character:Choose");
trace("Crown:deathpit");
trace("Weapon:ultra revolver");
trace("Seed:26375");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("ultra revolver"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:DIO,Death Pit,Elite Dasher,Croc Man,Director" + " Character:Choose" + " Crown:deathpit" + " Weapon:ultra revolver" + " Seed:26375";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2021-06-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2021-06-27 Daily.txt");
		while(!file_loaded(global.alias + " 2021-06-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2021-06-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2021-06-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:DIO,Death Pit,Elite Dasher,Croc Man,Director" + " Character:Choose" + " Crown:deathpit" + " Weapon:ultra revolver" + " Seed:26375" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":DIO,Death Pit,Elite Dasher,Croc Man,Director" + " Character:Choose" + " Crown:deathpit" + " Weapon:ultra revolver" + " Seed:26375",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
