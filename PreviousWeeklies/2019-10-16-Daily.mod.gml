...
/*@rMods@w=#  B#  Cool Trails#  BL2#  Elite Rifle#  Cycle#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  eliterifle#@pSeed@w=32729#Limited Tries:false*/
/*|B[B]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|BL2[Borderlands 2 x Nuclear Throne]|Elite Rifle[I'm just gonna grab that,#thank you very much.]|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]*/
#define init
...
trace("Current Daily:");
trace("Mods=B,Cool Trails,BL2,Elite Rifle,Cycle");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:B,Cool Trails,BL2,Elite Rifle,Cycle" + " Character:Choose" + " Crown:crown of protection" + " Weapon:eliterifle" + " Seed:32729";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-16 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-16 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-16 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-16 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-16 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:B,Cool Trails,BL2,Elite Rifle,Cycle" + " Character:Choose" + " Crown:crown of protection" + " Weapon:eliterifle" + " Seed:32729" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":B,Cool Trails,BL2,Elite Rifle,Cycle" + " Character:Choose" + " Crown:crown of protection" + " Weapon:eliterifle" + " Seed:32729",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
