...
/*@rMods@w=#  Devastator#  Crazy Allies#  Bioluminary Oscillator#  Colored Palace#  Assault Flak Cannon#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  Bioluminary Oscillator#@pSeed@w=22904#Limited Tries:false*/
/*|Devastator[Changes the devastator#to make it more interesting]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Bioluminary Oscillator[TARGET AQUIRED]|Colored Palace[Gives a palace a#well-deserved paint job.]|Assault Flak Cannon[What do you expect an Assault Flak Cannon to do?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Devastator,Crazy Allies,Bioluminary Oscillator,Colored Palace,Assault Flak Cannon");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Devastator,Crazy Allies,Bioluminary Oscillator,Colored Palace,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Bioluminary Oscillator" + " Seed:22904";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Devastator,Crazy Allies,Bioluminary Oscillator,Colored Palace,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Bioluminary Oscillator" + " Seed:22904" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Devastator,Crazy Allies,Bioluminary Oscillator,Colored Palace,Assault Flak Cannon" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Bioluminary Oscillator" + " Seed:22904",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
