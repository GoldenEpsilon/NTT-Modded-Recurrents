...
/*@rMods@w=#  Grappler#  Elite Rifle#  Double Maxigun#  Colored Palace#  Elite Minigun#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  Double Maxigun#@pSeed@w=24133#Limited Tries:false*/
/*|Grappler[For when you really want to throw someone at someone else.]|Elite Rifle[I'm just gonna grab that,#thank you very much.]|Double Maxigun[Because miniguns#aren't cool enough]|Colored Palace[Gives a palace a#well-deserved paint job.]|Elite Minigun[Man, it feels nice#using their own guns on them.]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/eliterifle.wep.gml"));
...
trace("Current Daily:");
trace("Mods=Grappler,Elite Rifle,Double Maxigun,Colored Palace,Elite Minigun");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Grappler,Elite Rifle,Double Maxigun,Colored Palace,Elite Minigun" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Double Maxigun" + " Seed:24133";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-04 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Grappler,Elite Rifle,Double Maxigun,Colored Palace,Elite Minigun" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Double Maxigun" + " Seed:24133" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Grappler,Elite Rifle,Double Maxigun,Colored Palace,Elite Minigun" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Double Maxigun" + " Seed:24133",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
