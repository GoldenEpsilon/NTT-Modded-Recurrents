...
/*@rMods@w=#  banditstack#  Crazy Allies#  Death Pit#  Chaos Guns#  Carpocalypse#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  Hyper Chaos Minigun#@pSeed@w=3073#Limited Tries:false*/
/*|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Chaos Guns[For when you need a bit#more chaos in your life.]|Carpocalypse[Beep Beep, motherf***er]*/
#define init
...
trace("Current Daily:");
trace("Mods=banditstack,Crazy Allies,Death Pit,Chaos Guns,Carpocalypse");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:banditstack,Crazy Allies,Death Pit,Chaos Guns,Carpocalypse" + " Character:Choose" + " Crown:deathpit" + " Weapon:Hyper Chaos Minigun" + " Seed:3073";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-06 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:banditstack,Crazy Allies,Death Pit,Chaos Guns,Carpocalypse" + " Character:Choose" + " Crown:deathpit" + " Weapon:Hyper Chaos Minigun" + " Seed:3073" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":banditstack,Crazy Allies,Death Pit,Chaos Guns,Carpocalypse" + " Character:Choose" + " Crown:deathpit" + " Weapon:Hyper Chaos Minigun" + " Seed:3073",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
