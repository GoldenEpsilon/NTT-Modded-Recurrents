...
/*@rMods@w=#  Buff#  blackholes#  Chrononaut#  Charge Template#  Cool Dark#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  hyperrifle#@pSeed@w=32660#Limited Tries:false*/
/*|Buff[Buffs to various weapons,#ultras, mutations,#and more]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]|Cool Dark[spoopy time]*/
#define init
...
trace("Current Daily:");
trace("Mods=Buff,blackholes,Chrononaut,Charge Template,Cool Dark");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Buff,blackholes,Chrononaut,Charge Template,Cool Dark" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyperrifle" + " Seed:32660";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-04 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Buff,blackholes,Chrononaut,Charge Template,Cool Dark" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyperrifle" + " Seed:32660" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Buff,blackholes,Chrononaut,Charge Template,Cool Dark" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyperrifle" + " Seed:32660",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
