...
/*@rMods@w=#  Buff#  Cool Dust#  Double Laser Minigun#  Director#  Blindness#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  hyperrifle#@pSeed@w=8012#Limited Tries:false*/
/*|Buff[Buffs to various weapons,#ultras, mutations,#and more]|Cool Dust[It's not just dust, it's#COOL DUST]|Double Laser Minigun[Great for personal rave parties!]|Director[He will direct your enemies#to the nearest exit of life]|Blindness[MY GLASSES! I BROKE MY GLASSES!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Buff,Cool Dust,Double Laser Minigun,Director,Blindness");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Buff,Cool Dust,Double Laser Minigun,Director,Blindness" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyperrifle" + " Seed:8012";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-23 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-23 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-23 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-23 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-23 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Buff,Cool Dust,Double Laser Minigun,Director,Blindness" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyperrifle" + " Seed:8012" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Buff,Cool Dust,Double Laser Minigun,Director,Blindness" + " Character:Choose" + " Crown:crown of haste" + " Weapon:hyperrifle" + " Seed:8012",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
