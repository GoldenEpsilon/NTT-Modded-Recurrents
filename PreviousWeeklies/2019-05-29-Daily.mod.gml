...
/*@rMods@w=#  Cool Hud#  Damage Numbers#  Bullet Time#  Cool Dark#  Determination#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  heavy assault rifle#@pSeed@w=7489#Limited Tries:false*/
/*|Cool Hud[*C418 starts playing*]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Bullet Time[The closer you are to bullets#the slower time moves]|Cool Dark[spoopy time]|Determination[Seeing this filler text#fills you with determination]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Hud,Damage Numbers,Bullet Time,Cool Dark,Determination");
trace("Character:Choose");
trace("Crown:none");
trace("Weapon:heavy assault rifle");
trace("Seed:7489");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("heavy assault rifle"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Hud,Damage Numbers,Bullet Time,Cool Dark,Determination" + " Character:Choose" + " Crown:none" + " Weapon:heavy assault rifle" + " Seed:7489";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-29 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Hud,Damage Numbers,Bullet Time,Cool Dark,Determination" + " Character:Choose" + " Crown:none" + " Weapon:heavy assault rifle" + " Seed:7489" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Hud,Damage Numbers,Bullet Time,Cool Dark,Determination" + " Character:Choose" + " Crown:none" + " Weapon:heavy assault rifle" + " Seed:7489",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
