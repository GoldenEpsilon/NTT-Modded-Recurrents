...
/*@rMods@w=#  Death Pit#  Double Laser Minigun#  Damage Numbers#  betterLoops#  Cosmi Gun Enemies#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  doublelaser#@pSeed@w=13668#Limited Tries:false*/
/*|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Double Laser Minigun[Great for personal rave parties!]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|betterLoops[Less enemies, more health per enemy in loops.]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Death Pit,Double Laser Minigun,Damage Numbers,betterLoops,Cosmi Gun Enemies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Death Pit,Double Laser Minigun,Damage Numbers,betterLoops,Cosmi Gun Enemies" + " Character:Choose" + " Crown:deathpit" + " Weapon:doublelaser" + " Seed:13668";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-04 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-04 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-04 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-04 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-04 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Death Pit,Double Laser Minigun,Damage Numbers,betterLoops,Cosmi Gun Enemies" + " Character:Choose" + " Crown:deathpit" + " Weapon:doublelaser" + " Seed:13668" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Death Pit,Double Laser Minigun,Damage Numbers,betterLoops,Cosmi Gun Enemies" + " Character:Choose" + " Crown:deathpit" + " Weapon:doublelaser" + " Seed:13668",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
