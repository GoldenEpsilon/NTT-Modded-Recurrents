...
/*@rMods@w=#  brad#  Airhorn#  Blood#  Elite Minigun#  Drama Kill#@bCharacter@w=#  brad#@yCrown@w=#  crown of blood#@gWeapon@w=#  nowep#@pSeed@w=13568#Limited Tries:false*/
/*|brad[Adds Brad from LISA: the Painful RPG]|Airhorn[Ever wanted to use the airhorn with every character?##Now you can!]|Blood[If Nuclear Throne had an M rating...]|Elite Minigun[Man, it feels nice#using their own guns on them.]|Drama Kill[For when you want#to feel really good#for defeating Big Bandit]*/
#define init
...
trace("Mods=brad,Airhorn,Blood,Elite Minigun,Drama Kill");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:brad,Airhorn,Blood,Elite Minigun,Drama Kill" + " Character:brad" + " Crown:crown of blood" + " Weapon:nowep" + " Seed:13568";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-28 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-10-28 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-10-28 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-28 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-10-28 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:brad,Airhorn,Blood,Elite Minigun,Drama Kill" + " Character:brad" + " Crown:crown of blood" + " Weapon:nowep" + " Seed:13568" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":brad,Airhorn,Blood,Elite Minigun,Drama Kill" + " Character:brad" + " Crown:crown of blood" + " Weapon:nowep" + " Seed:13568",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
