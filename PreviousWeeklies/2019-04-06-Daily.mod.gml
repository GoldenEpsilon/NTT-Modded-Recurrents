...
/*@rMods@w=#  Buff#  Auto Bullet Shotgun#  Double Time#  betterLoops#  Bioluminary Oscillator#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=2506#Limited Tries:false*/
/*|Buff[Buffs to various weapons,#ultras, mutations,#and more]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Double Time[GOTTAGOFAST]|betterLoops[Less enemies, more health per enemy in loops.]|Bioluminary Oscillator[TARGET AQUIRED]*/
#define init
...
trace("Current Daily:");
trace("Mods=Buff,Auto Bullet Shotgun,Double Time,betterLoops,Bioluminary Oscillator");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Buff,Auto Bullet Shotgun,Double Time,betterLoops,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Auto Bullet Shotgun" + " Seed:2506";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-06 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Buff,Auto Bullet Shotgun,Double Time,betterLoops,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Auto Bullet Shotgun" + " Seed:2506" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Buff,Auto Bullet Shotgun,Double Time,betterLoops,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Auto Bullet Shotgun" + " Seed:2506",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
