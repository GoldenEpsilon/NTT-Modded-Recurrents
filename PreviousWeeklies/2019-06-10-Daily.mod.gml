...
/*@rMods@w=#  Bullet Time#  Cooler Hud#  Dogfly#  Dumb Shells#  Cycle#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  toxic launcher#@pSeed@w=13911#Limited Tries:false*/
/*|Bullet Time[The closer you are to bullets#the slower time moves]|Cooler Hud[Symphony of the Throne]|Dogfly[The dog guardians#learned to jump good.]|Dumb Shells[And on that dumbshell,#it's time to start.]|Cycle[Makes it so you keep gaining mutations, but you only keep the last 10]*/
#define init
...
trace("Current Daily:");
trace("Mods=Bullet Time,Cooler Hud,Dogfly,Dumb Shells,Cycle");
trace("Character:Choose");
trace("Crown:crown of hatred");
trace("Weapon:toxic launcher");
trace("Seed:13911");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("toxic launcher"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Bullet Time,Cooler Hud,Dogfly,Dumb Shells,Cycle" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:toxic launcher" + " Seed:13911";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-10 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-10 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-10 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-10 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-10 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Bullet Time,Cooler Hud,Dogfly,Dumb Shells,Cycle" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:toxic launcher" + " Seed:13911" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Bullet Time,Cooler Hud,Dogfly,Dumb Shells,Cycle" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:toxic launcher" + " Seed:13911",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
