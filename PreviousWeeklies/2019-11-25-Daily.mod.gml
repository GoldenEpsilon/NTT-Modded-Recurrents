...
/*@rMods@w=#  Dasher x10#  AllSkills#  Cool Shadows#  Bloodking#  Crazy Bandits#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  hyper rifle#@pSeed@w=28395#Limited Tries:false*/
/*|Dasher x10[@qEXTREME DASHING]|AllSkills[Congratulations!#You just got the perfect#mutation set.]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Bloodking[Blood, pain, and a half-finished character.]|Crazy Bandits[THEY HAVE GUNS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dasher x10,AllSkills,Cool Shadows,Bloodking,Crazy Bandits");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dasher x10,AllSkills,Cool Shadows,Bloodking,Crazy Bandits" + " Character:Choose" + " Crown:none" + " Weapon:hyper rifle" + " Seed:28395";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-25 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-25 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-25 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-25 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-25 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dasher x10,AllSkills,Cool Shadows,Bloodking,Crazy Bandits" + " Character:Choose" + " Crown:none" + " Weapon:hyper rifle" + " Seed:28395" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dasher x10,AllSkills,Cool Shadows,Bloodking,Crazy Bandits" + " Character:Choose" + " Crown:none" + " Weapon:hyper rifle" + " Seed:28395",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
