...
/*@rMods@w=#  6 Weapons#  Dasher#  Cool Shadows#  Cool Trails#  bouncybolts#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  golden plasma gun#@pSeed@w=6939#Limited Tries:false*/
/*|6 Weapons[Gives you one weapon slot#for each ammo type like DOOM]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|bouncybolts[Upgrade to Bolt Marrow]*/
#define init
...
trace("Current Daily:");
trace("Mods=6 Weapons,Dasher,Cool Shadows,Cool Trails,bouncybolts");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:6 Weapons,Dasher,Cool Shadows,Cool Trails,bouncybolts" + " Character:Choose" + " Crown:crown of love" + " Weapon:golden plasma gun" + " Seed:6939";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-31 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-31 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-31 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-31 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-31 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:6 Weapons,Dasher,Cool Shadows,Cool Trails,bouncybolts" + " Character:Choose" + " Crown:crown of love" + " Weapon:golden plasma gun" + " Seed:6939" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":6 Weapons,Dasher,Cool Shadows,Cool Trails,bouncybolts" + " Character:Choose" + " Crown:crown of love" + " Weapon:golden plasma gun" + " Seed:6939",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
