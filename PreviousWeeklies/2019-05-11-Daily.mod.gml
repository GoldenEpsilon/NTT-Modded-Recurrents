...
/*@rMods@w=#  BAR#  Druid#  Big Dog Snipers#  Defect#  Devastator#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  Big Ass Revolver#@pSeed@w=24099#Limited Tries:false*/
/*|BAR[A Big Ass Revolver.]|Druid[Not Getafix,#but he'll do.]|Big Dog Snipers[The actual boss]|Defect[BEEP BOOP I AM ROBOT]|Devastator[Changes the devastator#to make it more interesting]*/
#define init
...
trace("Current Daily:");
trace("Mods=BAR,Druid,Big Dog Snipers,Defect,Devastator");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:BAR,Druid,Big Dog Snipers,Defect,Devastator" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Big Ass Revolver" + " Seed:24099";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:BAR,Druid,Big Dog Snipers,Defect,Devastator" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Big Ass Revolver" + " Seed:24099" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":BAR,Druid,Big Dog Snipers,Defect,Devastator" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Big Ass Revolver" + " Seed:24099",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
