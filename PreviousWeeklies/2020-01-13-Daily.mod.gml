...
/*@rMods@w=#  boys#  Brimthrone#  Death Pit#  Cool Trails#  B#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  brimthrone#@pSeed@w=26510#Limited Tries:false*/
/*|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|B[B]*/
#define init
...
trace("Current Daily:");
trace("Mods=boys,Brimthrone,Death Pit,Cool Trails,B");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:boys,Brimthrone,Death Pit,Cool Trails,B" + " Character:Choose" + " Crown:deathpit" + " Weapon:brimthrone" + " Seed:26510";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-13 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:boys,Brimthrone,Death Pit,Cool Trails,B" + " Character:Choose" + " Crown:deathpit" + " Weapon:brimthrone" + " Seed:26510" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":boys,Brimthrone,Death Pit,Cool Trails,B" + " Character:Choose" + " Crown:deathpit" + " Weapon:brimthrone" + " Seed:26510",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
