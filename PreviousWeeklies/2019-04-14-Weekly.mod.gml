...
/*@rMods@w=#  Blindness#  Challenge Mode#  Cool Shadows#  Minecraft Area#  Dodge Roll#@bCharacter@w=#  frog#@yCrown@w=#  none#@gWeapon@w=#  shotgun#@pSeed@w=24404#Limited Tries:false*/
/*|Blindness[MY GLASSES! I BROKE MY GLASSES!]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Minecraft Area[Who Love Minecraf]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]*/
#define init
...
trace("Mods=Blindness,Challenge Mode,Cool Shadows,Minecraft Area,Dodge Roll");
trace("Character:frog");
trace("Crown:none");
trace("Weapon:shotgun");
trace("Seed:24404");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Blindness,Challenge Mode,Cool Shadows,Minecraft Area,Dodge Roll" + " Character:frog" + " Crown:none" + " Weapon:shotgun" + " Seed:24404";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-14 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-04-14 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-04-14 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-14 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-04-14 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Blindness,Challenge Mode,Cool Shadows,Minecraft Area,Dodge Roll" + " Character:frog" + " Crown:none" + " Weapon:shotgun" + " Seed:24404" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Blindness,Challenge Mode,Cool Shadows,Minecraft Area,Dodge Roll" + " Character:frog" + " Crown:none" + " Weapon:shotgun" + " Seed:24404",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race="frog";}else{global.Race = race;}}
