...
/*@rMods@w=#  Fire Riders#  Damage Numbers#  bouncermini#  Blood#  Champions#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  lightning rifle#@pSeed@w=19324#Limited Tries:false*/
/*|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|bouncermini[FILL THE SKIES WITH BULLETS]|Blood[If Nuclear Throne had an M rating...]|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]*/
#define init
...
trace("Current Daily:");
trace("Mods=Fire Riders,Damage Numbers,bouncermini,Blood,Champions");
trace("Character:Choose");
trace("Crown:crown of death");
trace("Weapon:lightning rifle");
trace("Seed:19324");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("lightning rifle"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Fire Riders,Damage Numbers,bouncermini,Blood,Champions" + " Character:Choose" + " Crown:crown of death" + " Weapon:lightning rifle" + " Seed:19324";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Fire Riders,Damage Numbers,bouncermini,Blood,Champions" + " Character:Choose" + " Crown:crown of death" + " Weapon:lightning rifle" + " Seed:19324" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Fire Riders,Damage Numbers,bouncermini,Blood,Champions" + " Character:Choose" + " Crown:crown of death" + " Weapon:lightning rifle" + " Seed:19324",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
