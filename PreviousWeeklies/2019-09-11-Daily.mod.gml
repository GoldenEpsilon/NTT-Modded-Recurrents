...
/*@rMods@w=#  !ARMAGGEDNN#  boys#  Druid#  Champions#  Big Dog#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  Blood Shovel#@pSeed@w=29839#Limited Tries:false*/
/*|!ARMAGGEDNN[This big mod for NTT is adding 36 new Suppa CrAzY WeaPONs##Also there is a customarea (you will die)]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Druid[Not Getafix,#but he'll do.]|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Big Dog[HE IS AWAKENED]*/
#define init
...
trace("Current Daily:");
trace("Mods=!ARMAGGEDNN,boys,Druid,Champions,Big Dog");
trace("Character:Choose");
trace("Crown:crown of curses");
trace("Weapon:Blood Shovel");
trace("Seed:29839");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Blood Shovel"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:!ARMAGGEDNN,boys,Druid,Champions,Big Dog" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Blood Shovel" + " Seed:29839";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:!ARMAGGEDNN,boys,Druid,Champions,Big Dog" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Blood Shovel" + " Seed:29839" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":!ARMAGGEDNN,boys,Druid,Champions,Big Dog" + " Character:Choose" + " Crown:crown of curses" + " Weapon:Blood Shovel" + " Seed:29839",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
