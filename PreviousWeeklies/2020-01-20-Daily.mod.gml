...
/*@rMods@w=#  Some Crowns#  Bear#  Auto Bullet Shotgun#  Cool Dust#  blackholes#@bCharacter@w=#  Choose#@yCrown@w=#  bullethell#@gWeapon@w=#  A Fucking Gun#@pSeed@w=6474#Limited Tries:false*/
/*|Some Crowns[Adds... Some Crowns.#dunno what you expected]|Bear[4 da meeeeeeeeee#eeeeemez]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Cool Dust[It's not just dust, it's#COOL DUST]|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]*/
#define init
...
trace("Current Daily:");
trace("Mods=Some Crowns,Bear,Auto Bullet Shotgun,Cool Dust,blackholes");
trace("Character:Choose");
trace("Crown:bullethell");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Some Crowns,Bear,Auto Bullet Shotgun,Cool Dust,blackholes" + " Character:Choose" + " Crown:bullethell" + " Weapon:A Fucking Gun" + " Seed:6474";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-20 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Some Crowns,Bear,Auto Bullet Shotgun,Cool Dust,blackholes" + " Character:Choose" + " Crown:bullethell" + " Weapon:A Fucking Gun" + " Seed:6474" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Some Crowns,Bear,Auto Bullet Shotgun,Cool Dust,blackholes" + " Character:Choose" + " Crown:bullethell" + " Weapon:A Fucking Gun" + " Seed:6474",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
