...
/*@rMods@w=#  Cool Shadows#  Cuz#  Freak Arena#  Ascension 15#  Devastator#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  golden crossbow#@pSeed@w=219#Limited Tries:false*/
/*|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]|Cuz[Has an energy shield, for some reason.]|Freak Arena[Do you think you can survive#in the @qFREAK ARENA???]|Ascension 15[@qASCENSION LEVEL#@q15/15]|Devastator[Changes the devastator#to make it more interesting]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Shadows,Cuz,Freak Arena,Ascension 15,Devastator");
trace("Character:Choose");
trace("Crown:crown of guns");
trace("Weapon:golden crossbow");
trace("Seed:219");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("golden crossbow"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Shadows,Cuz,Freak Arena,Ascension 15,Devastator" + " Character:Choose" + " Crown:crown of guns" + " Weapon:golden crossbow" + " Seed:219";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-27 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-27 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-27 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-27 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-27 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Shadows,Cuz,Freak Arena,Ascension 15,Devastator" + " Character:Choose" + " Crown:crown of guns" + " Weapon:golden crossbow" + " Seed:219" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Shadows,Cuz,Freak Arena,Ascension 15,Devastator" + " Character:Choose" + " Crown:crown of guns" + " Weapon:golden crossbow" + " Seed:219",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
