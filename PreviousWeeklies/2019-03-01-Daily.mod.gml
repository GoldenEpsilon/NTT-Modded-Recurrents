...
/*@rMods@w=#  Cool Flame#  Chips#  Car#  Area Health#  xefsstuff#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  cloud cannon#@pSeed@w=22011#Limited Tries:false*/
/*|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]|Area Health[A nice mod that shows how many enemies are left.]|xefsstuff[This has a bit of everything.#A bunch of weapons, a few characters,#a butler robot#buddy thingamijig that kills guys...#@qDON'T GIVE HIM A GRENADE LAUNCHER OH N-]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/coolflame.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Cool Flame,Chips,Car,Area Health,xefsstuff");
trace("Character=Choose");
trace("Crown=crown of blood");
trace("Weapon=cloud cannon");
trace("Seed=22011");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("cloud cannon"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Flame,Chips,Car,Area Health,xefsstuff" + " Character=Choose" + " Crown=crown of blood" + " Weapon=cloud cannon" + " Seed=22011";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Flame,Chips,Car,Area Health,xefsstuff" + " Character=Choose" + " Crown=crown of blood" + " Weapon=cloud cannon" + " Seed=22011" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all("ods:Cool Flame,Chips,Car,Area Health,xefsstuff" + " Character=Choose" + " Crown=crown of blood" + " Weapon=cloud cannon" + " Seed=22011",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
