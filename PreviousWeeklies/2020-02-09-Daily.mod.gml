...
/*@rMods@w=#  Auto Revolver#  Damage Numbers#  Cool Flame#  Challenge Mode#  bouncermini#@bCharacter@w=#  Choose#@yCrown@w=#  crown of hatred#@gWeapon@w=#  Auto Revolver#@pSeed@w=1532#Limited Tries:false*/
/*|Auto Revolver[A truly auto weapon.]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|Cool Flame[Make all those enemies liars!##What do you mean that's#not how that works?]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]|bouncermini[FILL THE SKIES WITH BULLETS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Auto Revolver,Damage Numbers,Cool Flame,Challenge Mode,bouncermini");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Revolver,Damage Numbers,Cool Flame,Challenge Mode,bouncermini" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Auto Revolver" + " Seed:1532";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-09 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Revolver,Damage Numbers,Cool Flame,Challenge Mode,bouncermini" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Auto Revolver" + " Seed:1532" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Revolver,Damage Numbers,Cool Flame,Challenge Mode,bouncermini" + " Character:Choose" + " Crown:crown of hatred" + " Weapon:Auto Revolver" + " Seed:1532",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
