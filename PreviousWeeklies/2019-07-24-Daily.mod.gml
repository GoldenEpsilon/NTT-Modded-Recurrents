...
/*@rMods@w=#  Blindness#  Blood#  Detailer#  Determination#  Bomber#@bCharacter@w=#  Choose#@yCrown@w=#  crown of curses#@gWeapon@w=#  revolver#@pSeed@w=5493#Limited Tries:false*/
/*|Blindness[MY GLASSES! I BROKE MY GLASSES!]|Blood[If Nuclear Throne had an M rating...]|Detailer[Adds pretty patches of things]|Determination[Seeing this filler text#fills you with determination]|Bomber[Bombs?]*/
#define init
...
trace("Current Daily:");
trace("Mods=Blindness,Blood,Detailer,Determination,Bomber");
trace("Character:Choose");
trace("Crown:crown of curses");
trace("Weapon:revolver");
trace("Seed:5493");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Blindness,Blood,Detailer,Determination,Bomber" + " Character:Choose" + " Crown:crown of curses" + " Weapon:revolver" + " Seed:5493";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-24 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-24 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-24 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-24 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-24 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Blindness,Blood,Detailer,Determination,Bomber" + " Character:Choose" + " Crown:crown of curses" + " Weapon:revolver" + " Seed:5493" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Blindness,Blood,Detailer,Determination,Bomber" + " Character:Choose" + " Crown:crown of curses" + " Weapon:revolver" + " Seed:5493",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
