...
/*@rMods@w=#  Belly#  Boltshard#  Death Pit#  Cool Trails#  Director#@bCharacter@w=#  Choose#@yCrown@w=#  deathpit#@gWeapon@w=#  Boltshard#@pSeed@w=4878#Limited Tries:false*/
/*|Belly[The only survivor of the#meme that is melee actives]|Boltshard[If looking cool was good at killing,#this would one-shot the throne.]|Death Pit[FIGHT TO THE DEATH#IN THIS SMALL SQUARE#WHEN YOU HAVE THIS CROWN#DO IT NOW]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Director[He will direct your enemies#to the nearest exit of life]*/
#define init
...
downloadmod("directorchar/inmaterace.race.gml");
downloadmod("directorchar/captgrunt.ogg");
downloadmod("directorchar/magic.png");
while (!file_loaded(string_trim(data+"belly/belly.race.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyChst.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyCnfm.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyCrwn.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyDead.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyHurt.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyIDPD.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyLowA.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyLowH.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellySlct.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellySpch.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyValt.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"belly/sndBellyWrld.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"Boltshard.wep.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"deathpit.crown.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"cooltrails.mod.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"directorchar/inmaterace.race.gml"))) {wait 1;}
while (!file_loaded(string_trim(data+"directorchar/captgrunt.ogg"))) {wait 1;}
while (!file_loaded(string_trim(data+"directorchar/magic.png"))) {wait 1;}
...
wait mod_load(string_trim("data/Daily.mod/directorchar/inmaterace.race.gml"));
...
trace("Current Daily:");
trace("Mods=Belly,Boltshard,Death Pit,Cool Trails,Director");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Belly,Boltshard,Death Pit,Cool Trails,Director" + " Character:Choose" + " Crown:deathpit" + " Weapon:Boltshard" + " Seed:4878";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-19 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-19 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-19 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-19 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-19 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Belly,Boltshard,Death Pit,Cool Trails,Director" + " Character:Choose" + " Crown:deathpit" + " Weapon:Boltshard" + " Seed:4878" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Belly,Boltshard,Death Pit,Cool Trails,Director" + " Character:Choose" + " Crown:deathpit" + " Weapon:Boltshard" + " Seed:4878",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
