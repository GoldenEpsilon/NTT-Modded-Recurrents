...
/*@rMods@w=#  Ascension 5#  Chicken Head#  boys#  Team Attack#  blaac48#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  laser minigun#@pSeed@w=8362#Limited Tries:false*/
/*|Ascension 5[ASCENSION LEVEL#5/15]|Chicken Head[Chicken can now carry her head.#Gross.]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Team Attack[Team Age Mutant#Ninja Mutants Go!]|blaac48[The jungle is deeper]*/
#define init
...
trace("Current Daily:");
trace("Mods=Ascension 5,Chicken Head,boys,Team Attack,blaac48");
trace("Character:Choose");
trace("Crown:crown of protection");
trace("Weapon:laser minigun");
trace("Seed:8362");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("laser minigun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Ascension 5,Chicken Head,boys,Team Attack,blaac48" + " Character:Choose" + " Crown:crown of protection" + " Weapon:laser minigun" + " Seed:8362";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-30 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-30 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-30 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-30 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-30 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Ascension 5,Chicken Head,boys,Team Attack,blaac48" + " Character:Choose" + " Crown:crown of protection" + " Weapon:laser minigun" + " Seed:8362" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Ascension 5,Chicken Head,boys,Team Attack,blaac48" + " Character:Choose" + " Crown:crown of protection" + " Weapon:laser minigun" + " Seed:8362",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
