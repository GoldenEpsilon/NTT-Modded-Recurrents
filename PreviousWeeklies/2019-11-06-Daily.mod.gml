...
/*@rMods@w=#  6 Weapons#  Baller#  Determination#  CoDeath#  Detonator#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  toxic bow#@pSeed@w=6573#Limited Tries:false*/
/*|6 Weapons[Gives you one weapon slot#for each ammo type like DOOM]|Baller[Fire burns...#me like fire...]|Determination[Seeing this filler text#fills you with determination]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Detonator[Active has a radius#where when you release#it blows up your bullets]*/
#define init
...
trace("Current Daily:");
trace("Mods=6 Weapons,Baller,Determination,CoDeath,Detonator");
trace("Character:Choose");
trace("Crown:CoDeath");
trace("Weapon:toxic bow");
trace("Seed:6573");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("toxic bow"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:6 Weapons,Baller,Determination,CoDeath,Detonator" + " Character:Choose" + " Crown:CoDeath" + " Weapon:toxic bow" + " Seed:6573";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-06 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-06 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-06 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-06 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-06 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:6 Weapons,Baller,Determination,CoDeath,Detonator" + " Character:Choose" + " Crown:CoDeath" + " Weapon:toxic bow" + " Seed:6573" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":6 Weapons,Baller,Determination,CoDeath,Detonator" + " Character:Choose" + " Crown:CoDeath" + " Weapon:toxic bow" + " Seed:6573",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
