...
/*@rMods@w=#  bossbar#  Belly#  Auto Bullet Shotgun#  Doctor#  Croc Man#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  Auto Bullet Shotgun#@pSeed@w=15430#Limited Tries:false*/
/*|bossbar[For when you want to know#just how close you were to looping]|Belly[The only survivor of the#meme that is melee actives]|Auto Bullet Shotgun[BLAM BLAM BLAMMO]|Doctor[The feathery, creepy,#totally-not-about-to-#stab-you-with-a-knife#doctor is in!]|Croc Man[From what I can tell you either#don't take damage from anything or#you die instantly because of that passive.]*/
#define init
...
trace("Current Daily:");
trace("Mods=bossbar,Belly,Auto Bullet Shotgun,Doctor,Croc Man");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bossbar,Belly,Auto Bullet Shotgun,Doctor,Croc Man" + " Character:Choose" + " Crown:crown of death" + " Weapon:Auto Bullet Shotgun" + " Seed:15430";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-15 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-15 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-15 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-15 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-15 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bossbar,Belly,Auto Bullet Shotgun,Doctor,Croc Man" + " Character:Choose" + " Crown:crown of death" + " Weapon:Auto Bullet Shotgun" + " Seed:15430" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bossbar,Belly,Auto Bullet Shotgun,Doctor,Croc Man" + " Character:Choose" + " Crown:crown of death" + " Weapon:Auto Bullet Shotgun" + " Seed:15430",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
