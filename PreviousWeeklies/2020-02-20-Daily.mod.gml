...
/*@rMods@w=#  Car#  Chaos Guns#  banditstack#  Ally QOL#  Ascension 15#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  Hyper Chaos Minigun#@pSeed@w=3744#Limited Tries:false*/
/*|Car[Ever wanted to play#as a flying car?#No? TOO BAD!]|Chaos Guns[For when you need a bit#more chaos in your life.]|banditstack[Can stack bandits,#leaving the player in awe#of their balancing skills]|Ally QOL[Makes allies look nicer.#Because this only#really affects Rebel,#rebel is more likely#for this recurrent.]|Ascension 15[@qASCENSION LEVEL#@q15/15]*/
#define init
...
trace("Current Daily:");
trace("Mods=Car,Chaos Guns,banditstack,Ally QOL,Ascension 15");
trace("Character:Choose");
trace("Crown:crown of guns");
trace("Weapon:Hyper Chaos Minigun");
trace("Seed:3744");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Hyper Chaos Minigun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Car,Chaos Guns,banditstack,Ally QOL,Ascension 15" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Hyper Chaos Minigun" + " Seed:3744";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-20 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-20 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-20 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-20 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-20 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Car,Chaos Guns,banditstack,Ally QOL,Ascension 15" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Hyper Chaos Minigun" + " Seed:3744" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Car,Chaos Guns,banditstack,Ally QOL,Ascension 15" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Hyper Chaos Minigun" + " Seed:3744",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
