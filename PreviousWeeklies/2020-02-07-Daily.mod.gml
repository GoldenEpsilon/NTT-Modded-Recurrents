...
/*@rMods@w=#  bouncermini#  Big Dog Snipers#  Adjustable Ammo#  Cursed Hyper Crystal#  BL2#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  super flak cannon#@pSeed@w=20120#Limited Tries:false*/
/*|bouncermini[FILL THE SKIES WITH BULLETS]|Big Dog Snipers[The actual boss]|Adjustable Ammo[Push the limits#of ammo storage!]|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]|BL2[Borderlands 2 x Nuclear Throne]*/
#define init
...
trace("Current Daily:");
trace("Mods=bouncermini,Big Dog Snipers,Adjustable Ammo,Cursed Hyper Crystal,BL2");
trace("Character:Choose");
trace("Crown:crown of luck");
trace("Weapon:super flak cannon");
trace("Seed:20120");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("super flak cannon"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:bouncermini,Big Dog Snipers,Adjustable Ammo,Cursed Hyper Crystal,BL2" + " Character:Choose" + " Crown:crown of luck" + " Weapon:super flak cannon" + " Seed:20120";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-07 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-07 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-07 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-07 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-07 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:bouncermini,Big Dog Snipers,Adjustable Ammo,Cursed Hyper Crystal,BL2" + " Character:Choose" + " Crown:crown of luck" + " Weapon:super flak cannon" + " Seed:20120" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":bouncermini,Big Dog Snipers,Adjustable Ammo,Cursed Hyper Crystal,BL2" + " Character:Choose" + " Crown:crown of luck" + " Weapon:super flak cannon" + " Seed:20120",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
