...
/*@rMods@w=#  Cool Dark#  Disk Marrow#  Dasher x10#  boys#  Challenge Mode#@bCharacter@w=#  Choose#@yCrown@w=#  crown of luck#@gWeapon@w=#  hyper launcher#@pSeed@w=5418#Limited Tries:false*/
/*|Cool Dark[spoopy time]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]|Dasher x10[@qEXTREME DASHING]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Dark,Disk Marrow,Dasher x10,boys,Challenge Mode");
trace("Character:Choose");
trace("Crown:crown of luck");
trace("Weapon:hyper launcher");
trace("Seed:5418");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("hyper launcher"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Dark,Disk Marrow,Dasher x10,boys,Challenge Mode" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyper launcher" + " Seed:5418";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-13 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-13 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-13 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-13 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-13 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Dark,Disk Marrow,Dasher x10,boys,Challenge Mode" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyper launcher" + " Seed:5418" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Dark,Disk Marrow,Dasher x10,boys,Challenge Mode" + " Character:Choose" + " Crown:crown of luck" + " Weapon:hyper launcher" + " Seed:5418",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
