...
/*@rMods@w=#  Derpweps#  Fire Riders#  Clasweps#  Elite Dasher#  Cool Shadows#@bCharacter@w=#  Choose#@yCrown@w=#  Slaughter#@gWeapon@w=#  ultrarifle#@pSeed@w=25655#Limited Tries:false*/
/*|Derpweps[Derrrrrrrrrrrp]|Fire Riders[NOTE: when the gators on your back die,#you start taking damage.]|Clasweps[You got ultra weapons,#you got zappy weapons,#call now and you'll even get#some crowns thrown in for free!]|Elite Dasher[@qPICK UP ALL THE THINGS]|Cool Shadows[WHAT IS IT?#IT'S FOLLOWING MY EVERY MOVE!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Derpweps,Fire Riders,Clasweps,Elite Dasher,Cool Shadows");
trace("Character:Choose");
trace("Crown:Slaughter");
trace("Weapon:ultrarifle");
trace("Seed:25655");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("ultrarifle"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Derpweps,Fire Riders,Clasweps,Elite Dasher,Cool Shadows" + " Character:Choose" + " Crown:Slaughter" + " Weapon:ultrarifle" + " Seed:25655";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-21 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-21 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-21 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-21 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-21 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Derpweps,Fire Riders,Clasweps,Elite Dasher,Cool Shadows" + " Character:Choose" + " Crown:Slaughter" + " Weapon:ultrarifle" + " Seed:25655" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Derpweps,Fire Riders,Clasweps,Elite Dasher,Cool Shadows" + " Character:Choose" + " Crown:Slaughter" + " Weapon:ultrarifle" + " Seed:25655",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
