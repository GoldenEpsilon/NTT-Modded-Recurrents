...
/*@rMods@w=#  Double Plasma Minigun#  boys#  Ally QOL#  Bitpack#  blstorm#@bCharacter@w=#  Choose#@yCrown@w=#  crown of death#@gWeapon@w=#  blstorm#@pSeed@w=2977#Limited Tries:false*/
/*|Double Plasma Minigun[For when you want to#go through even more ammo]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Ally QOL[Makes allies look nicer.#Because this only#really affects Rebel,#rebel is more likely#for this recurrent.]|Bitpack[idk, bitflux made this]|blstorm[BLEED YOURSELF DRY]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/Double Plasma/doubleplasma.wep.gml"));
...
trace("Current Daily:");
trace("Mods=Double Plasma Minigun,boys,Ally QOL,Bitpack,blstorm");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Plasma Minigun,boys,Ally QOL,Bitpack,blstorm" + " Character:Choose" + " Crown:crown of death" + " Weapon:blstorm" + " Seed:2977";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-02-19 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-02-19 Daily.txt");
		while(!file_loaded(global.alias + " 2020-02-19 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-02-19 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-02-19 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Plasma Minigun,boys,Ally QOL,Bitpack,blstorm" + " Character:Choose" + " Crown:crown of death" + " Weapon:blstorm" + " Seed:2977" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Plasma Minigun,boys,Ally QOL,Bitpack,blstorm" + " Character:Choose" + " Crown:crown of death" + " Weapon:blstorm" + " Seed:2977",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
