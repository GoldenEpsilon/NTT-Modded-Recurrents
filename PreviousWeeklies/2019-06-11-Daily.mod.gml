...
/*@rMods@w=#  Double Pop Gun#  bow#  Bigger Bandit#  Dasher#  Challenge Mode#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  double_pop_gun#@pSeed@w=12711#Limited Tries:false*/
/*|Double Pop Gun[Now you can tickle enemies#twice as fast!]|bow[It's a bow.]|Bigger Bandit[Ever gotten bored of Big Dog?]|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Pop Gun,bow,Bigger Bandit,Dasher,Challenge Mode");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Pop Gun,bow,Bigger Bandit,Dasher,Challenge Mode" + " Character:Choose" + " Crown:crown of guns" + " Weapon:double_pop_gun" + " Seed:12711";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Pop Gun,bow,Bigger Bandit,Dasher,Challenge Mode" + " Character:Choose" + " Crown:crown of guns" + " Weapon:double_pop_gun" + " Seed:12711" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Pop Gun,bow,Bigger Bandit,Dasher,Challenge Mode" + " Character:Choose" + " Crown:crown of guns" + " Weapon:double_pop_gun" + " Seed:12711",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
