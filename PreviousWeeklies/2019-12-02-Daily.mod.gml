...
/*@rMods@w=#  Carpocalypse#  Double Laser Minigun#  Chips#  Bandit#  C Ultras#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  doublelaser#@pSeed@w=23815#Limited Tries:false*/
/*|Carpocalypse[Beep Beep, motherf***er]|Double Laser Minigun[Great for personal rave parties!]|Chips[Gives you a mutation every time you#beat the Throne or a Hyper Crystal]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]|C Ultras[Adds a C ultra to all#the vanilla characters]*/
#define init
...
trace("Current Daily:");
trace("Mods=Carpocalypse,Double Laser Minigun,Chips,Bandit,C Ultras");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Carpocalypse,Double Laser Minigun,Chips,Bandit,C Ultras" + " Character:Choose" + " Crown:crown of risk" + " Weapon:doublelaser" + " Seed:23815";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-02 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-02 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-02 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-02 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-02 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Carpocalypse,Double Laser Minigun,Chips,Bandit,C Ultras" + " Character:Choose" + " Crown:crown of risk" + " Weapon:doublelaser" + " Seed:23815" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Carpocalypse,Double Laser Minigun,Chips,Bandit,C Ultras" + " Character:Choose" + " Crown:crown of risk" + " Weapon:doublelaser" + " Seed:23815",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
