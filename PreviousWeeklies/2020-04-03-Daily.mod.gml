...
/*@rMods@w=#  Auto Flame Crossbow#  Big Toothbrush#  blstorm#  Bullet Time#  Cosmi Gun Enemies#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  blstorm#@pSeed@w=15922#Limited Tries:false*/
/*|Auto Flame Crossbow[If flames were good this would be better than just looking cool...]|Big Toothbrush[a big toothbrush for big mouths]|blstorm[BLEED YOURSELF DRY]|Bullet Time[The closer you are to bullets#the slower time moves]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Auto Flame Crossbow,Big Toothbrush,blstorm,Bullet Time,Cosmi Gun Enemies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Auto Flame Crossbow,Big Toothbrush,blstorm,Bullet Time,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of blood" + " Weapon:blstorm" + " Seed:15922";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-03 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Auto Flame Crossbow,Big Toothbrush,blstorm,Bullet Time,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of blood" + " Weapon:blstorm" + " Seed:15922" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Auto Flame Crossbow,Big Toothbrush,blstorm,Bullet Time,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of blood" + " Weapon:blstorm" + " Seed:15922",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
