...
/*@rMods@w=#  Crazy Allies#  Director#  Carbine#  Charge Template 2#  Bandit#@bCharacter@w=#  Choose#@yCrown@w=#  none#@gWeapon@w=#  charge_template#@pSeed@w=21367#Limited Tries:false*/
/*|Crazy Allies[Changes rebel's active to#spawn allies with weapons,#but you need to be next to a weapon#to be able to use it]|Director[He will direct your enemies#to the nearest exit of life]|Carbine[Pew Pew.]|Charge Template 2[Why aren't people more descriptive, like#"a shotgun that uses a pistol sprite that shoots#more bullets the more you charge that#happens to be a charge template"?]|Bandit[Armed only with your trusty Bandit Blaster,#it's you against the entire wasteland, baby!]*/
#define init
...
trace("Current Daily:");
trace("Mods=Crazy Allies,Director,Carbine,Charge Template 2,Bandit");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Crazy Allies,Director,Carbine,Charge Template 2,Bandit" + " Character:Choose" + " Crown:none" + " Weapon:charge_template" + " Seed:21367";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-03-28 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-03-28 Daily.txt");
		while(!file_loaded(global.alias + " 2019-03-28 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-03-28 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-03-28 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Crazy Allies,Director,Carbine,Charge Template 2,Bandit" + " Character:Choose" + " Crown:none" + " Weapon:charge_template" + " Seed:21367" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Crazy Allies,Director,Carbine,Charge Template 2,Bandit" + " Character:Choose" + " Crown:none" + " Weapon:charge_template" + " Seed:21367",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
