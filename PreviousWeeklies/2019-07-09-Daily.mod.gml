...
/*@rMods@w=#  Cursed Hyper Crystal#  CoDeath#  Chicken Head#  Brimthrone#  BadEnd#@bCharacter@w=#  Choose#@yCrown@w=#  CoDeath#@gWeapon@w=#  brimthrone#@pSeed@w=28598#Limited Tries:false*/
/*|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]|CoDeath[Adds a REAL crown of death#in addition to the existing CoD]|Chicken Head[Chicken can now carry her head.#Gross.]|Brimthrone[A portable throne, to#know how it feels to#zap people with that laser.]|BadEnd[For when you don't want a good end.#(sit after loop 3)]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cursed Hyper Crystal,CoDeath,Chicken Head,Brimthrone,BadEnd");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cursed Hyper Crystal,CoDeath,Chicken Head,Brimthrone,BadEnd" + " Character:Choose" + " Crown:CoDeath" + " Weapon:brimthrone" + " Seed:28598";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-07-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-07-09 Daily.txt");
		while(!file_loaded(global.alias + " 2019-07-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-07-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-07-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cursed Hyper Crystal,CoDeath,Chicken Head,Brimthrone,BadEnd" + " Character:Choose" + " Crown:CoDeath" + " Weapon:brimthrone" + " Seed:28598" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cursed Hyper Crystal,CoDeath,Chicken Head,Brimthrone,BadEnd" + " Character:Choose" + " Crown:CoDeath" + " Weapon:brimthrone" + " Seed:28598",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
