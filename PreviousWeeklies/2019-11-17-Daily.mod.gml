...
/*@rMods@w=#  Cursed Hyper Crystal#  Cannoner#  Detailer#  Cool Trails#  Bear#@bCharacter@w=#  Choose#@yCrown@w=#  crown of haste#@gWeapon@w=#  cannoner#@pSeed@w=19955#Limited Tries:false*/
/*|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]|Cannoner[EIGHT RADICAL SHOTS]|Detailer[Adds pretty patches of things]|Cool Trails[Bullet trails,#for when you want to feel#like you're in the matrix]|Bear[4 da meeeeeeeeee#eeeeemez]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cursed Hyper Crystal,Cannoner,Detailer,Cool Trails,Bear");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cursed Hyper Crystal,Cannoner,Detailer,Cool Trails,Bear" + " Character:Choose" + " Crown:crown of haste" + " Weapon:cannoner" + " Seed:19955";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-11-17 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-11-17 Daily.txt");
		while(!file_loaded(global.alias + " 2019-11-17 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-11-17 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-11-17 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cursed Hyper Crystal,Cannoner,Detailer,Cool Trails,Bear" + " Character:Choose" + " Crown:crown of haste" + " Weapon:cannoner" + " Seed:19955" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cursed Hyper Crystal,Cannoner,Detailer,Cool Trails,Bear" + " Character:Choose" + " Crown:crown of haste" + " Weapon:cannoner" + " Seed:19955",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
