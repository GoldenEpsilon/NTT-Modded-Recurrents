...
/*@rMods@w=#  blackholes#  Cool Hud#  Dummy#  basicgunallies#  Vanthro#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  auto grenade shotgun#@pSeed@w=11805#Limited Tries:false*/
/*|blackholes[All projectiles attract enemies of the user#that means you get pulled into enemy bullets, too.#this is going to be a hard one...]|Cool Hud[*C418 starts playing*]|Dummy[No strings on me!]|basicgunallies[Ever wanted your allies to have normal guns?#Wait - @q HE'S GOT A GRENADE LAUNCHER!#@q@qRUN!!!]|Vanthro[...blame 4chan.]*/
#define init
...
trace("Current Daily:");
trace("Mods=blackholes,Cool Hud,Dummy,basicgunallies,Vanthro");
trace("Character:Choose");
trace("Crown:crown of risk");
trace("Weapon:auto grenade shotgun");
trace("Seed:11805");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("auto grenade shotgun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:blackholes,Cool Hud,Dummy,basicgunallies,Vanthro" + " Character:Choose" + " Crown:crown of risk" + " Weapon:auto grenade shotgun" + " Seed:11805";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-01 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-01 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-01 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-01 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-01 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:blackholes,Cool Hud,Dummy,basicgunallies,Vanthro" + " Character:Choose" + " Crown:crown of risk" + " Weapon:auto grenade shotgun" + " Seed:11805" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":blackholes,Cool Hud,Dummy,basicgunallies,Vanthro" + " Character:Choose" + " Crown:crown of risk" + " Weapon:auto grenade shotgun" + " Seed:11805",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
