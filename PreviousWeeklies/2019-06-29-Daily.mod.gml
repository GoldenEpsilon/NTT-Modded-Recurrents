...
/*@rMods@w=#  Dasher#  Some Crowns#  BadEnd#  bow#  Cosmi#@bCharacter@w=#  Choose#@yCrown@w=#  dogs#@gWeapon@w=#  bow#@pSeed@w=21425#Limited Tries:false*/
/*|Dasher[Dash at thing#-> pick up thing#-> throw thing#-> repeat.]|Some Crowns[Adds... Some Crowns.#dunno what you expected]|BadEnd[For when you don't want a good end.#(sit after loop 3)]|bow[It's a bow.]|Cosmi[*insert *insert *insert self insert joke here* joke here* joke here*]*/
#define init
...
trace("Current Daily:");
trace("Mods=Dasher,Some Crowns,BadEnd,bow,Cosmi");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Dasher,Some Crowns,BadEnd,bow,Cosmi" + " Character:Choose" + " Crown:dogs" + " Weapon:bow" + " Seed:21425";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-06-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-06-29 Daily.txt");
		while(!file_loaded(global.alias + " 2019-06-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-06-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-06-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Dasher,Some Crowns,BadEnd,bow,Cosmi" + " Character:Choose" + " Crown:dogs" + " Weapon:bow" + " Seed:21425" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Dasher,Some Crowns,BadEnd,bow,Cosmi" + " Character:Choose" + " Crown:dogs" + " Weapon:bow" + " Seed:21425",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
