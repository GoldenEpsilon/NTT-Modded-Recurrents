...
/*@rMods@w=#  Belly#  Baldi#  Discs#  Ancient Hatred#  Carpocalypse#@bCharacter@w=#  Choose#@yCrown@w=#  crown of ancient hatred#@gWeapon@w=#  Splinter Gun Disc Gun#@pSeed@w=13031#Limited Tries:false*/
/*|Belly[The only survivor of the#meme that is melee actives]|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]|Discs[Because, you know, we need#MORE discs killing us.]|Ancient Hatred[Crown that trades health for rads over time]|Carpocalypse[Beep Beep, motherf***er]*/
#define init
...
trace("Current Daily:");
trace("Mods=Belly,Baldi,Discs,Ancient Hatred,Carpocalypse");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Belly,Baldi,Discs,Ancient Hatred,Carpocalypse" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Splinter Gun Disc Gun" + " Seed:13031";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-08-26 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-08-26 Daily.txt");
		while(!file_loaded(global.alias + " 2019-08-26 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-08-26 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-08-26 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Belly,Baldi,Discs,Ancient Hatred,Carpocalypse" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Splinter Gun Disc Gun" + " Seed:13031" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Belly,Baldi,Discs,Ancient Hatred,Carpocalypse" + " Character:Choose" + " Crown:crown of ancient hatred" + " Weapon:Splinter Gun Disc Gun" + " Seed:13031",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
