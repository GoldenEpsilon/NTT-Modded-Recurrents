...
/*@rMods@w=#  Cricket#  Blaac's Weapon Mutations#  brad#  Ascension 5#  Minecraft Area#@bCharacter@w=#  brad#@yCrown@w=#  crown of blood#@gWeapon@w=#  Cricket#@pSeed@w=29717#Limited Tries:false*/
/*|Cricket[Batter up!#...or whatever they say for cricket.]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|brad[Adds Brad from LISA: the Painful RPG]|Ascension 5[ASCENSION LEVEL#5/15]|Minecraft Area[Who Love Minecraf]*/
#define init
...
downloadmod("ascension5.mod.gml");
...
wait mod_load(string_trim("data/Weekly.mod/brad/brad.mod.gml"));
wait mod_load(string_trim("data/Weekly.mod/brad/brad.race.gml"));
wait mod_load(string_trim("data/Weekly.mod/brad/nowep.wep.gml"));
wait mod_load(string_trim("data/Weekly.mod/ascension5.mod.gml"));
...
trace("Mods=Cricket,Blaac's Weapon Mutations,brad,Ascension 5,Minecraft Area");
trace("Character:brad");
...
if(!global.canStart){
	...
	with(CharSelect){
		...
		if(instance_exists(self) && !ChooseCharacter){
			if(string_lower(string_trim(race))!=string_lower(string_trim("brad")) && fork()){
				wait(0);
				if(string_lower(string_trim(race))!=string_lower(string_trim("brad"))){
					instance_destroy();
				}
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cricket,Blaac's Weapon Mutations,brad,Ascension 5,Minecraft Area" + " Character:brad" + " Crown:crown of blood" + " Weapon:Cricket" + " Seed:29717";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-08 Weekly.txt")){
		prevScores = string_load(global.alias + " 2019-12-08 Weekly.txt");
		while(!file_loaded(global.alias + " 2019-12-08 Weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-08 Weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-12-08 Weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cricket,Blaac's Weapon Mutations,brad,Ascension 5,Minecraft Area" + " Character:brad" + " Crown:crown of blood" + " Weapon:Cricket" + " Seed:29717" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cricket,Blaac's Weapon Mutations,brad,Ascension 5,Minecraft Area" + " Character:brad" + " Crown:crown of blood" + " Weapon:Cricket" + " Seed:29717",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
...
global.start = true;
with(Player){if(!ChooseCharacter){race=string_trim("brad");}else{global.Race = race;}}
