...
/*@rMods@w=#  Big Dog Snipers#  Discs#  Minecraft Area#  boys#  Bioluminary Oscillator#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Splinter Gun Disc Gun#@pSeed@w=32650#Limited Tries:false*/
/*|Big Dog Snipers[The actual boss]|Discs[Because, you know, we need#MORE discs killing us.]|Minecraft Area[Who Love Minecraf]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|Bioluminary Oscillator[TARGET AQUIRED]*/
#define init
...
trace("Current Daily:");
trace("Mods=Big Dog Snipers,Discs,Minecraft Area,boys,Bioluminary Oscillator");
trace("Character:Choose");
trace("Crown:crown of destiny");
trace("Weapon:Splinter Gun Disc Gun");
trace("Seed:32650");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Splinter Gun Disc Gun"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Big Dog Snipers,Discs,Minecraft Area,boys,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Splinter Gun Disc Gun" + " Seed:32650";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-12 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-12 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-12 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-12 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-12 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Big Dog Snipers,Discs,Minecraft Area,boys,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Splinter Gun Disc Gun" + " Seed:32650" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Big Dog Snipers,Discs,Minecraft Area,boys,Bioluminary Oscillator" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Splinter Gun Disc Gun" + " Seed:32650",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
