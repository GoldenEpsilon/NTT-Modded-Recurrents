...
/*@rMods@w=#  Cool Dust#  Bullet Time#  Babylon Shotgun#  Minecraft Area#  Big Bandit#@bCharacter@w=#  Choose#@yCrown@w=#  crown of love#@gWeapon@w=#  Babylon Shotgun#@pSeed@w=28465#Limited Tries:false*/
/*|Cool Dust[It's not just dust, it's#COOL DUST]|Bullet Time[The closer you are to bullets#the slower time moves]|Babylon Shotgun[So much destruction your enemies will babble-on]|Minecraft Area[Who Love Minecraf]|Big Bandit['Nuff said.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Cool Dust,Bullet Time,Babylon Shotgun,Minecraft Area,Big Bandit");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Cool Dust,Bullet Time,Babylon Shotgun,Minecraft Area,Big Bandit" + " Character:Choose" + " Crown:crown of love" + " Weapon:Babylon Shotgun" + " Seed:28465";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-03 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-03 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-03 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-03 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-03 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Cool Dust,Bullet Time,Babylon Shotgun,Minecraft Area,Big Bandit" + " Character:Choose" + " Crown:crown of love" + " Weapon:Babylon Shotgun" + " Seed:28465" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Cool Dust,Bullet Time,Babylon Shotgun,Minecraft Area,Big Bandit" + " Character:Choose" + " Crown:crown of love" + " Weapon:Babylon Shotgun" + " Seed:28465",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
