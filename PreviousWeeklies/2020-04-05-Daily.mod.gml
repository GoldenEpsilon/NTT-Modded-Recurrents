...
/*@rMods@w=#  Area Health#  Belly#  boys#  BL2#  Disk Marrow#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  black sword#@pSeed@w=29381#Limited Tries:false*/
/*|Area Health[A nice mod that shows how many enemies are left.]|Belly[The only survivor of the#meme that is melee actives]|boys[Instead of hit points, you have a 1hp clone per hit point, and max health basically doesn't exist.]|BL2[Borderlands 2 x Nuclear Throne]|Disk Marrow[Now discs are counted as a bolt weapon#for bolt marrow to help!#It doesn't save you from stupidity, though.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Area Health,Belly,boys,BL2,Disk Marrow");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Area Health,Belly,boys,BL2,Disk Marrow" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:black sword" + " Seed:29381";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-04-05 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-04-05 Daily.txt");
		while(!file_loaded(global.alias + " 2020-04-05 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-04-05 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-04-05 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Area Health,Belly,boys,BL2,Disk Marrow" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:black sword" + " Seed:29381" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Area Health,Belly,boys,BL2,Disk Marrow" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:black sword" + " Seed:29381",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
