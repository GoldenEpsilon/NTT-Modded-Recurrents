...
/*@rMods@w=#  AllSkills#  Dodge Roll#  Colored Projectiles#  Cool Dark#  Damage Numbers#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  golden screwdriver#@pSeed@w=3114#Limited Tries:false*/
/*|AllSkills[Congratulations!#You just got the perfect#mutation set.]|Dodge Roll[Fish visited the Gungeon#on holiday and#came back with new tricks!]|Colored Projectiles[Colors projectiles based on team color]|Cool Dark[spoopy time]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]*/
#define init
...
trace("Current Daily:");
trace("Mods=AllSkills,Dodge Roll,Colored Projectiles,Cool Dark,Damage Numbers");
trace("Character:Choose");
trace("Crown:crown of blood");
trace("Weapon:golden screwdriver");
trace("Seed:3114");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("golden screwdriver"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:AllSkills,Dodge Roll,Colored Projectiles,Cool Dark,Damage Numbers" + " Character:Choose" + " Crown:crown of blood" + " Weapon:golden screwdriver" + " Seed:3114";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-23 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-23 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-23 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-23 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-23 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:AllSkills,Dodge Roll,Colored Projectiles,Cool Dark,Damage Numbers" + " Character:Choose" + " Crown:crown of blood" + " Weapon:golden screwdriver" + " Seed:3114" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":AllSkills,Dodge Roll,Colored Projectiles,Cool Dark,Damage Numbers" + " Character:Choose" + " Crown:crown of blood" + " Weapon:golden screwdriver" + " Seed:3114",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
