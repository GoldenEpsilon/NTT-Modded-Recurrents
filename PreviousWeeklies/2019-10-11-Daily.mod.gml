...
/*@rMods@w=#  Crypt Of The Necrodancer#  Ascension 5#  Chaos Guns#  Cannondemnation#  Cursed Hyper Crystal#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Blunderbuss#@pSeed@w=20791#Limited Tries:false*/
/*|Crypt Of The Necrodancer[Move to the beat!]|Ascension 5[ASCENSION LEVEL#5/15]|Chaos Guns[For when you need a bit#more chaos in your life.]|Cannondemnation[DOOM DEATH DESTRUCTION##AND LINKIN PARK]|Cursed Hyper Crystal[Adds a Cursed variant of the Hyper Crystal.#If you fight it you'll probably die,#but man does it look cool.]*/
#define init
...
wait mod_load(string_trim("data/Daily.mod/ascension5.mod.gml"));
...
trace("Current Daily:");
trace("Mods=Crypt Of The Necrodancer,Ascension 5,Chaos Guns,Cannondemnation,Cursed Hyper Crystal");
trace("Character:Choose");
trace("Crown:crown of destiny");
trace("Weapon:Blunderbuss");
trace("Seed:20791");
...
if(global.start){
	global.alias = "";
	with(Player){
		give_wep(string_trim("Blunderbuss"));
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Crypt Of The Necrodancer,Ascension 5,Chaos Guns,Cannondemnation,Cursed Hyper Crystal" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Blunderbuss" + " Seed:20791";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-10-11 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-10-11 Daily.txt");
		while(!file_loaded(global.alias + " 2019-10-11 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-10-11 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-10-11 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Crypt Of The Necrodancer,Ascension 5,Chaos Guns,Cannondemnation,Cursed Hyper Crystal" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Blunderbuss" + " Seed:20791" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Crypt Of The Necrodancer,Ascension 5,Chaos Guns,Cannondemnation,Cursed Hyper Crystal" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Blunderbuss" + " Seed:20791",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
