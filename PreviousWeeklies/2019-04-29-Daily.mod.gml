...
/*@rMods@w=#  Chaos Guns#  Grappler#  bossbar#  Crypt Of The Necrodancer#  Dasher x10#@bCharacter@w=#  Choose#@yCrown@w=#  crown of guns#@gWeapon@w=#  Super Blunderbuss#@pSeed@w=1732#Limited Tries:false*/
/*|Chaos Guns[For when you need a bit#more chaos in your life.]|Grappler[For when you really want to throw someone at someone else.]|bossbar[For when you want to know#just how close you were to looping]|Crypt Of The Necrodancer[Move to the beat!]|Dasher x10[@qEXTREME DASHING]*/
#define init
...
trace("Current Daily:");
trace("Mods=Chaos Guns,Grappler,bossbar,Crypt Of The Necrodancer,Dasher x10");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Chaos Guns,Grappler,bossbar,Crypt Of The Necrodancer,Dasher x10" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Super Blunderbuss" + " Seed:1732";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-29 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-29 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-29 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-29 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-29 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Chaos Guns,Grappler,bossbar,Crypt Of The Necrodancer,Dasher x10" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Super Blunderbuss" + " Seed:1732" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Chaos Guns,Grappler,bossbar,Crypt Of The Necrodancer,Dasher x10" + " Character:Choose" + " Crown:crown of guns" + " Weapon:Super Blunderbuss" + " Seed:1732",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
