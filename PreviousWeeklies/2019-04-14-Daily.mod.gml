...
/*@rMods@w=#  Big Dog Snipers#  Baldi#  Chrononaut#  Director#  Carbine#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  Baldi/ruler#@pSeed@w=29715#Limited Tries:false*/
/*|Big Dog Snipers[The actual boss]|Baldi[Baldi's Basics in Education and Learning#is a state-of-the-art, fully 3D interactive,#fun-time educational game that teaches a slew of subjects!]|Chrononaut[Wibbley Wobbley#Timey Wimey]|Director[He will direct your enemies#to the nearest exit of life]|Carbine[Pew Pew.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Big Dog Snipers,Baldi,Chrononaut,Director,Carbine");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Big Dog Snipers,Baldi,Chrononaut,Director,Carbine" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Baldi/ruler" + " Seed:29715";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-04-14 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-04-14 Daily.txt");
		while(!file_loaded(global.alias + " 2019-04-14 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-04-14 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-04-14 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Big Dog Snipers,Baldi,Chrononaut,Director,Carbine" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Baldi/ruler" + " Seed:29715" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Big Dog Snipers,Baldi,Chrononaut,Director,Carbine" + " Character:Choose" + " Crown:crown of risk" + " Weapon:Baldi/ruler" + " Seed:29715",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
