...
//@rMods@w=#  Nuke Launcher#  Lots O' Muts#  Chaos Guns#  Level Balls#  Bandit Cannon#@bCharacter@w=#  char_steroids#@yCrown@w=#  crwn_destiny#@gWeapon@w=#  wep_golden_slugger#@pSeed@w=6417
#define init
...
while (!file_loaded(string_trim("nukelauncher.race.gml"))) {wait 1;}
...
while (!file_loaded(string_trim("bcannon.wep.gml"))) {wait 1;}
...
trace("Mods=Nuke Launcher, Lots O' Muts, Chaos Guns, Level Balls, Bandit Cannon");
trace("Character=char_steroids");
trace("Crown=crwn_destiny");
trace("Weapon=wep_golden_slugger");
trace("Seed=6417");
...
else{with(instances_matching(CustomObject, "name", mod_current)){instance_change(CharSelect, 0);}with(CharSelect){race=char_steroids;visible=true;}}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Nuke Launcher, Lots O' Muts, Chaos Guns, Level Balls, Bandit Cannon" + " Character=char_steroids" + " Crown=crwn_destiny" + " Weapon=wep_golden_slugger" + " Seed=6417";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-02-21 weekly.txt")){
		prevScores = string_load(global.alias + " 2019-02-21 weekly.txt");
		while(!file_loaded(global.alias + " 2019-02-21 weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-02-21 weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-02-21 weekly.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Nuke Launcher, Lots O' Muts, Chaos Guns, Level Balls, Bandit Cannon" + " Character=char_steroids" + " Crown=crwn_destiny" + " Weapon=wep_golden_slugger" + " Seed=6417" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(leaderboard, "Mods:Nuke Launcher, Lots O' Muts, Chaos Guns, Level Balls, Bandit Cannon" + " Character=char_steroids" + " Crown=crwn_destiny")) > 1){
			file_delete("sha.txt");
