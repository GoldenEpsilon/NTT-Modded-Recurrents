...
/*@rMods@w=#  Double Time#  Team Attack#  Damage Numbers#  bow#  Baller#@bCharacter@w=#  Choose#@yCrown@w=#  crown of blood#@gWeapon@w=#  bow#@pSeed@w=12766#Limited Tries:false*/
/*|Double Time[GOTTAGOFAST]|Team Attack[Team Age Mutant#Ninja Mutants Go!]|Damage Numbers[So you can see how good/bad#your setup is on the fly!]|bow[It's a bow.]|Baller[Fire burns...#me like fire...]*/
#define init
...
trace("Current Daily:");
trace("Mods=Double Time,Team Attack,Damage Numbers,bow,Baller");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Double Time,Team Attack,Damage Numbers,bow,Baller" + " Character:Choose" + " Crown:crown of blood" + " Weapon:bow" + " Seed:12766";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-03-19 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-03-19 Daily.txt");
		while(!file_loaded(global.alias + " 2020-03-19 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-03-19 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-03-19 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Double Time,Team Attack,Damage Numbers,bow,Baller" + " Character:Choose" + " Crown:crown of blood" + " Weapon:bow" + " Seed:12766" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Double Time,Team Attack,Damage Numbers,bow,Baller" + " Character:Choose" + " Crown:crown of blood" + " Weapon:bow" + " Seed:12766",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
