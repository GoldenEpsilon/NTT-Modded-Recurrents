...
/*@rMods@w=#  Big Toothbrush#  blaac48#  Druid#  Bullet Time#  Cosmi Gun Enemies#@bCharacter@w=#  Choose#@yCrown@w=#  crown of destiny#@gWeapon@w=#  Big Toothbrush#@pSeed@w=3314#Limited Tries:false*/
/*|Big Toothbrush[a big toothbrush for big mouths]|blaac48[The jungle is deeper]|Druid[Not Getafix,#but he'll do.]|Bullet Time[The closer you are to bullets#the slower time moves]|Cosmi Gun Enemies[RUN THEY HAVE REAL GUNS]*/
#define init
...
trace("Current Daily:");
trace("Mods=Big Toothbrush,blaac48,Druid,Bullet Time,Cosmi Gun Enemies");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Big Toothbrush,blaac48,Druid,Bullet Time,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Big Toothbrush" + " Seed:3314";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-12-24 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-12-24 Daily.txt");
		while(!file_loaded(global.alias + " 2019-12-24 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-12-24 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-12-24 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Big Toothbrush,blaac48,Druid,Bullet Time,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Big Toothbrush" + " Seed:3314" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Big Toothbrush,blaac48,Druid,Bullet Time,Cosmi Gun Enemies" + " Character:Choose" + " Crown:crown of destiny" + " Weapon:Big Toothbrush" + " Seed:3314",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
