...
/*@rMods@w=#  Champions#  Baller#  Blaac's Weapon Mutations#  Egg#  Chaos Guns#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Super Blunderbuss#@pSeed@w=1387#Limited Tries:false*/
/*|Champions[weeeee are the buffed minionssss,#my frieeeeeendssss]|Baller[Fire burns...#me like fire...]|Blaac's Weapon Mutations[YOU GET A WEAPON MUTATION!#YOU GET A WEAPON MUTATION!#EVERYONE GETS A WEAPON MUTATION!!!]|Egg[You will GIVE#me an @qEGG]|Chaos Guns[For when you need a bit#more chaos in your life.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Champions,Baller,Blaac's Weapon Mutations,Egg,Chaos Guns");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Champions,Baller,Blaac's Weapon Mutations,Egg,Chaos Guns" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Super Blunderbuss" + " Seed:1387";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2020-01-24 Daily.txt")){
		prevScores = string_load(global.alias + " 2020-01-24 Daily.txt");
		while(!file_loaded(global.alias + " 2020-01-24 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2020-01-24 Daily.txt");
	}else{
		string_save(score, global.alias + " 2020-01-24 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Champions,Baller,Blaac's Weapon Mutations,Egg,Chaos Guns" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Super Blunderbuss" + " Seed:1387" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Champions,Baller,Blaac's Weapon Mutations,Egg,Chaos Guns" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Super Blunderbuss" + " Seed:1387",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
