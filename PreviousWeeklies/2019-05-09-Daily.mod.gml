...
/*@rMods@w=#  Vanthro#  Crypt Of The Necrodancer#  Bitpack#  Detonator#  Charge Template#@bCharacter@w=#  Choose#@yCrown@w=#  crown of risk#@gWeapon@w=#  ct#@pSeed@w=13951#Limited Tries:false*/
/*|Vanthro[...blame 4chan.]|Crypt Of The Necrodancer[Move to the beat!]|Bitpack[idk, bitflux made this]|Detonator[Active has a radius#where when you release#it blows up your bullets]|Charge Template[Hey, yes this is a#template, but it's#also a cool weapon.]*/
#define init
...
trace("Current Daily:");
trace("Mods=Vanthro,Crypt Of The Necrodancer,Bitpack,Detonator,Charge Template");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Vanthro,Crypt Of The Necrodancer,Bitpack,Detonator,Charge Template" + " Character:Choose" + " Crown:crown of risk" + " Weapon:ct" + " Seed:13951";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-05-09 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-05-09 Daily.txt");
		while(!file_loaded(global.alias + " 2019-05-09 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-05-09 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-05-09 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Vanthro,Crypt Of The Necrodancer,Bitpack,Detonator,Charge Template" + " Character:Choose" + " Crown:crown of risk" + " Weapon:ct" + " Seed:13951" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Vanthro,Crypt Of The Necrodancer,Bitpack,Detonator,Charge Template" + " Character:Choose" + " Crown:crown of risk" + " Weapon:ct" + " Seed:13951",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
