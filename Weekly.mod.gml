...
//@rMods@w=#  Fuzzy Shader#  Nuke Launcher#  MS Paint Shader#  Bandit Cannon#  Level Balls#@bCharacter@w=#  char_chicken#@yCrown@w=#  crwn_life#@gWeapon@w=#  wep_energy_hammer#@pSeed@w=10754
#define init
...
trace("Mods=Fuzzy Shader, Nuke Launcher, MS Paint Shader, Bandit Cannon, Level Balls");
trace("Character=char_chicken");
trace("Crown=crwn_life");
trace("Weapon=wep_energy_hammer");
trace("Seed=10754");
...
with(CharSelect){race=char_chicken;}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Fuzzy Shader, Nuke Launcher, MS Paint Shader, Bandit Cannon, Level Balls" + " Character=char_chicken" + " Crown=crwn_life" + " Weapon=wep_energy_hammer" + " Seed=10754";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Fuzzy Shader, Nuke Launcher, MS Paint Shader, Bandit Cannon, Level Balls" + " Character=char_chicken" + " Crown=crwn_life" + " Weapon=wep_energy_hammer" + " Seed=10754" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else{
			file_delete("sha.txt");
