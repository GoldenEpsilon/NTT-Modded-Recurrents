...
trace("Mods=Bandit Cannon, Bloom Shader, NTT X10 Ver 1.0.1, Level Balls, Fuzzy Shader");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Bandit Cannon, Bloom Shader, NTT X10 Ver 1.0.1, Level Balls, Fuzzy Shader" + " Character=char_venuz" + " Crown=crwn_destiny" + " Weapon=wep_lightning_hammer" + " Seed=29650";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-01-13 weekly.txt")){
		prevScores = string_load(global.alias + " 2019-01-13 weekly.txt");
		while(!file_loaded(global.alias + " 2019-01-13 weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-01-13 weekly.txt");
	}else{
		string_save(score, global.alias + " 2019-01-13 weekly.txt");
	}
}
#define draw_pause
//Anti-Cheat
