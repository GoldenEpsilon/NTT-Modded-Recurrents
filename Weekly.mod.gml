...
/*@rMods@w=#  Assault Flak Cannon#  Belly#  Bear#  Big Bandit#  Ash#@bCharacter@w=#  Bear#@yCrown@w=#  crwn_hatred#@gWeapon@w=#  wep_triple_machinegun#@pSeed@w=17024*/
#define init
...
trace("Mods=Assault Flak Cannon, Belly, Bear, Big Bandit, Ash");
trace("Character=Bear");
...
else{with(instances_matching(CustomObject, "name", mod_current)){instance_change(CharSelect, 0);}with(CharSelect){race="Bear";visible=true;}}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Assault Flak Cannon, Belly, Bear, Big Bandit, Ash" + " Character=Bear" + " Crown=crwn_hatred" + " Weapon=wep_triple_machinegun" + " Seed=17024";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Assault Flak Cannon, Belly, Bear, Big Bandit, Ash" + " Character=Bear" + " Crown=crwn_hatred" + " Weapon=wep_triple_machinegun" + " Seed=17024" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(leaderboard, "Mods:Assault Flak Cannon, Belly, Bear, Big Bandit, Ash" + " Character=Bear" + " Crown=crwn_hatred")) > 1){
			file_delete("sha.txt");
