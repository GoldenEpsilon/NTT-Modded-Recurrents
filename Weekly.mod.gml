...
trace("Mods=Fuzzy Shader, NTT X10 Ver 1.0.1, Bloom Shader, MS Paint Shader, Lots O' Muts");
trace("Character=char_steroids");
trace("Crown=crwn_life");
trace("Weapon=wep_blood_launcher");
trace("Seed=16014");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Fuzzy Shader, NTT X10 Ver 1.0.1, Bloom Shader, MS Paint Shader, Lots O' Muts" + " Character=char_steroids" + " Crown=crwn_life" + " Weapon=wep_blood_launcher" + " Seed=16014";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2018-11-11 weekly.txt")){
		prevScores = string_load(global.alias + " 2018-11-11 weekly.txt");
		while(!file_loaded(global.alias + " 2018-11-11 weekly.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2018-11-11 weekly.txt");
	}else{
		string_save(score, global.alias + " 2018-11-11 weekly.txt");
	}
}
#define draw_pause
//Anti-Cheat
