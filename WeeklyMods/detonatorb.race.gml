...
global.sprUltIcon[1] = sprite_add_base64("",1,9,9)
global.sprUltIcon[2] = sprite_add_base64("",1,9,9)
 // Don't Detonate:
...
spr_dead = global.spr_dead;
 ...
 // Sounds:
	...
	snd_lowh = sndSteroidsUpg;      
	snd_chst = sndSalamanderCharge; 
	snd_valt = sndMutant7Valt;      
	snd_crwn = sndMutant7Crwn;      
	snd_spch = sndMutant7Spch;      
	snd_idpd = sndMutant7IDPD;      
	snd_cptn = sndMutant7Cptn;      
...
with instances_matching(projectile,"creator",self){
    var projtype = 1;
    var explotype = Explosion;
    var amount = 1;
	 // Set Projectile Type:
	projtype = weapon_get_type(other.wep);
	for(i = 0; i < array_length_1d(global.NoDetonate); i++)
		if object_index = global.NoDetonate[i] projtype = 0;
		
	if(object_index = PlasmaBall || object_index = PlasmaBig || object_index = PlasmaHuge) projtype = 5;
	
	 // Detonate Projectiles:
	if button_pressed(other.index,"spec") && point_in_rectangle(x, y, view_xview, view_yview, view_xview + game_width, view_yview + game_height){
		if team = 2 || object_index = Disc || global.ultra[1] = 1 && damage < 3{ // Ultra A : Gamma Burst - Explode Enemy Projectiles With <3 Damage

			 // Melee : Can't Detonate
			if(projtype = 0 && team = 2) sound_play(sndFlamerStop);

			 // Bullets, Shells, and Bolts : Can Detonate
			if projtype = 1 || projtype = 2 || projtype = 3{ 	
				if damage < 7{
					if ultra_get(mod_current,2){ // Ultra B : Blood Burst - Detonate Blood Explosions Instead Of Normal Ones
						explotype = MeatExplosion
					}
					else 
					    explotype = SmallExplosion
				}
				else if damage < 25{
					if ultra_get(mod_current,2){ // Ultra B : Blood Burst - Detonate Blood Explosions Instead Of Normal Ones
						amount = 3
					...
					if ultra_get(mod_current,2){ // Ultra B : Blood Burst - Detonate Blood Explosions Instead Of Normal Ones
						...
						explotype = GreenExplosion
					}
				}
			if ultra_get(mod_current,2) sound_play(sndBloodLauncherExplo);
			...
			if projtype = 4{			
			 ...
			 // Energy : Plasma Can Detonate Into Plasma Explosions
				...
				explotype = PlasmaImpact
			
	if projtype > 0{
    	with instances_matching(object_index,"creator",creator) if distance_to_object(other)<4
		    {
		    x = other.x+10000;
		    y = other.y+10000;
		    }
    	with instance_create(x,y,explotype){
            ...
            hitid = [sprite_index,object_get_name(explotype)]
            if explotype = MeatExplosion{
            ...
            else if explotype = Explosion
                sound_play(sndExplosion)
            else if explotype = SmallExplosion
                sound_play(sndExplosionS)
            else if explotype = GreenExplosion
...
with(BoltStick) // Bolts Stuck in Enemies Can Explode Into Small Explosions:
	if button_pressed(other.index,"spec") && point_in_rectangle(x, y, view_xview, view_yview, view_xview + 320, view_yview + 240){
		var explotype = Explosion;
		if ultra_get(mod_current,2)
			explotype = MeatExplosion;
		else 
		...
		with instance_create(x,y,explotype){
	        direction = other.direction;
            speed = min(other.speed/0.75, 20);
            friction = 2.5;
            creator = other;
            hitid = [explotype,sprite_index]
            if explotype = MeatExplosion{
                with instance_create(x,y,MeltSplat){
	        		sprite_index = sprMeltSplatBig;
	        		depth = 2;
...
with(TrapFire){
	if(distance_to_object(Player) < 15) instance_destroy();
}

if ultra_get(mod_current,1) and random(10)<speed
