...
global.sh = shader_create(
	...
	float4 main(PixelShaderInput INPUT) : SV_TARGET
	{
		...
		for(float dist = 0; dist < Radius; dist += Precision)
		{
			...
			if(nCol.r - ((float)(floor(dist)) / Radius) > nR){
				nR = nCol.r - ((float)(floor(dist)) / Radius);
			}
			if(nCol.g - ((float)(floor(dist)) / Radius) > nG){
				nG = nCol.g - ((float)(floor(dist)) / Radius);
			}
			if(nCol.b - ((float)(floor(dist)) / Radius) > nB){
				nB = nCol.b - ((float)(floor(dist)) / Radius);
			}
		}
		
		return float4(nR, nG, nB, bloomStrength);
	}
");
