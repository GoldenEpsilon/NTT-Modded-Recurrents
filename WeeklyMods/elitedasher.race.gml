#define init
global.ultra[1] = 0;
global.ultra[2] = 0;
global.enemiesProcessed = 0;

//load the sprites in! im pretty sure YAL made a base64 plugin for paint.net to get this
global.spr_idle = sprite_add_base64("", 13, 12, 12);
global.spr_walk = sprite_add_base64("", 6, 12, 12);
global.spr_hurt = sprite_add_base64("", 3 , 12, 12)
global.spr_dead = sprite_add_base64("", 1, 0, 0);
global.spr_chrg = sprite_add_base64("", 3, 12, 12);
global.spr_slct = sprite_add_base64("", 1, 0, 0);
global.spr_car = sprite_add_base64("", 1, 18, 24);
global.spr_port = sprite_add_base64("", 1, 20, 243);
global.spr_icon = sprite_add_base64("", 1, 10, 10);
global.spr_ult[1] = sprite_add_base64("", 1, 12, 16);
global.spr_ult[2] = sprite_add_base64("", 1, 12, 16);

global.sprPizzaTop = sprite_add_base64("", 1, 16, 8);


#define game_start
//ultras cant be tracked atm, gotta make my own thing to track them
global.ultra[1] = 0;
global.ultra[2] = 0;

#define race_soundbank
//givin it robot sounds
return 8;

#define scrPlayerPickupEnemy
	if other.carrying = 0 && other.charge > 0{
		sound_play(sndSnowBotPickup);
		other.carrying = 1;
		other.charge = 0;
		other.timer = 0;
		other.angle = 0;
		other.radcarry = raddrop;
		//These two checks exist for the sole purpose of determining if
		//the player is picking up Rad Chests. It's shitty to lose those rads.
		if spr_hurt = 1435 other.radcarry = 25;
		if spr_hurt = 1430 other.radcarry = 45;
		other.lastEntSpr_idle = spr_hurt;
		other.lastEntSpr_hurt = spr_hurt;
		other.lastEntSpr_dead = spr_dead;
		
		trace("GET!!");
		instance_change(Wind,false);
	}

#define create
//charge variable to stop active
charge = 0;
//since im having trouble setting this up with wait, i made my own timer
timer = 0; 
//sprite of last entity carried
lastEntSpr_idle = sprNOWALLS;
lastEntSpr_hurt = sprNOWALLS;
lastEntSpr_dead = sprNOWALLS;

//for ultra B, so the game knows were carrying the car
carrying = 0;

//set the sprites. spr_chrg is the only one that isnt a default character sprite
spr_idle = global.spr_idle;
spr_walk = global.spr_walk;
spr_hurt = global.spr_hurt;
spr_dead = global.spr_dead;
spr_chrg = global.spr_chrg;

radcarry = 0;

#define step

if my_health <= 0{ //death explosion, because im too lazy to actually make a sprite for death
	instance_create(x,y,Explosion);
	sound_play(sndExplosion);
}

with(PizzaEntrance){
	if point_distance(x+16,y+16,other.x,other.y) < 12 && other.carrying = 0 && other.charge > 0 && image_index = 0{
		sound_play(sndSnowBotPickup);
		other.carrying = 1;
		other.charge = 0;
		other.timer = 0;
		other.lastEntSpr_idle = global.sprPizzaTop;
		other.lastEntSpr_hurt = global.sprPizzaTop;
		other.lastEntSpr_dead = global.sprPizzaTop;
		image_index = 1;
	}
}
with(prop){
	if point_distance(x,y,other.x,other.y) < 12 && other.carrying = 0 && other.charge > 0{
		scrPlayerPickupEnemy();
	}
}
if global.ultra[2] = 1{
	with Car{ //collect cars!!
		if point_distance(x,y,other.x,other.y) < 12 && other.carrying = 0
		{
			sound_play(sndSnowBotPickup);
			other.charge = 0;
			other.carrying = 1;
			other.timer = 0;
			instance_change(Wind,false);
		}
	}
	with enemy{ //cool car passive thing
		if my_health <= 0 && random(5) < 1 && !(object_index = Car and object_index = RadChest and object_index = VenuzCar and object_index = VenuzCarFixed)
		{
			instance_create(x,y,Car);
		}
	}
}
with(CarThrow){
	if ((direction + 270) mod 360 > 180) image_angle -= 1 * speed
		else image_angle += 1 * speed;
	if sprite_index = spr_idle image_index = 1;
}
if button_pressed(index,"spec") and charge = 0{ //active stuff
	if carrying = 0//if global.ultra[2] = 0 or carrying = 0 //make sure you cant charge while throwing cars
	{
		charge = 1;
		timer = 30;
		spr_walk = spr_chrg;
		
		if global.ultra[1] = 1{ //CHARGE SPAWNS BULLETS
			sound_play(sndWolfRoll);
			for (i = -15; i <= 15; i += 15) if (fork()){
				with instance_create(x,y,EnemyBullet1){
					motion_add(point_direction(other.x,other.y,mouse_x[other.index],mouse_y[other.index])+other.i,8);
					team = 2;
					image_angle = direction;
				}
				exit;
			}
		} else{ //sound for active
			sound_play(sndSnowBotSlideStart);
		}
	}
	else if carrying = 1 //THROW A CAR HELL YEAH
	{
		sound_play(sndSnowBotThrow)
		with instance_create(x,y,CarThrow)
		{
			sprite_index = other.lastEntSpr_idle;
			team = 2;
			raddrop = other.radcarry;
			motion_add(point_direction(other.x,other.y,mouse_x[other.index],mouse_y[other.index]),12)
			visible = false;
			spr_idle = other.lastEntSpr_idle;
			spr_hurt = other.lastEntSpr_hurt;
			spr_dead = other.lastEntSpr_dead;
			visible = true;
		}
		carrying = 0;
		radcarry = 0;
		motion_add(point_direction(mouse_x[index],mouse_y[index],x,y),8);
	}
}

with enemy{
	//make sure contact damage is decreased, by creating a variable if one does not exist so it only decreases once
	if ("meleedamage" not in self){meleedamage = 0;}
	if ("realMeleeDmg" not in self){realMeleeDmg = meleedamage;}
	if (other.charge > 0) meleedamage = 0
		else meleedamage = realMeleeDmg;
	if ("pass" not in self){
		pass = 0;
		if meleedamage > 0
		meleedamage -= 1;
		cdmg = meleedamage;
	} else { //for snowbots since their contact damage changes
		if meleedamage != cdmg{
			if meleedamage > 0
			meleedamage -= 1;
			cdmg = meleedamage;
		}
	}

	//damage enemies close enough. jw did a bad with damaging stuff
	if other.charge = 1 && point_distance(x,y,other.x,other.y) < 16{
		sprite_index = spr_hurt;
		image_index = 0;
		sound_play(snd_hurt);
		var temphp;
		temphp = GameCont.level * (1*max(1,skill_get(5)*2));
		//trace(string(my_health) + ":" + string(my_health - temphp));
		if (my_health - temphp <= round(maxhealth/4) || my_health <= 6) {//makes sure you dont have 0 damage
			scrPlayerPickupEnemy();
		}else my_health -= temphp;
		motion_add(direction,4);
	}
	
}

//active code
if charge = 1{
	motion_add(point_direction(x,y,mouse_x[index],mouse_y[index]),3); //move forward
	timer -= 1;
	
	//cute lil trail!
	if skill_get(5) = 0
	instance_create(x,y,Dust);
	else
	instance_create(x,y,Smoke);
	
	//stop the charge with the ultra
	if global.ultra[1] = 1{
		if button_released(index,"spec"){
			charge = 0;
			timer = 0;
		}else{
			angle -= 32 * right;
			instance_create(x+2-random(4),y+2-random(4),Dust);
		}
	}
	//charge timer stuff
	if timer = 0{
		charge = 0;
		angle = 0;
	}
} else { //visual stuff
	spr_walk = global.spr_walk;
}

//make sure youre not charging when u leave
if instance_exists(GenCont){
	charge = 0;
	timer = 0;
}

#define draw
//car code
if (carrying){
	var yscale, angle;
	if sprite_get_height(lastEntSpr_hurt) > 24 || lastEntSpr_hurt = global.sprPizzaTop{
		yscale = -1;
		angle = 0;
	}else{
		yscale = 1;
		angle = -90;
	}
	draw_sprite_ext(lastEntSpr_hurt,2,x,y-(sprite_get_height(lastEntSpr_hurt)/3.5),-1,yscale,angle,c_white,1);
	
}
with(PizzaEntrance){
	if (image_index = 1 && point_distance(x+16,y+16,other.x,other.y) < 12){
		draw_sprite(sprEPickup, 0, x, y-4);
		draw_text_nt(x+8, y-16, "ENTER");
		if button_pressed(other.index,"pick"){
			with(instance_create(x+16,y+16,Portal)) endgame = 2;
			with(GameCont){
				area = 102
				subarea = 0
			}
		}
	}
}
//draw_sprite_ext(global.spr_car,0,x,y,-1,1,0,c_white,1);
//menu sprites and stuff
#define race_portrait
return global.spr_port;

#define race_mapicon
return global.spr_icon;

#define race_menu_button
sprite_index = global.spr_slct;

//name, abilities, starting weapon, and throne butt
#define race_name
return "*E*L*I*T*E* Dasher";

#define race_text
return "No Contact Damage while Charging#Throws everything";

#define swep
return 1;

#define race_tb_text
return "double charge damage";

#define race_ultra_button
sprite_index = global.spr_ult[argument0];

#define race_ultra_name
switch(argument0){ //ultra names
	case 1: return "Sonic Speed";
	case 2: return "Car Mechanic";
}

#define race_ultra_text 
switch (argument0){ //ultra words
	case 1: return "CHARGE STOPS WHEN NOT RIGHT-CLICKING#CHARGE SHOOTS BULLETS"; //ITS A MEME OK
	case 2: return "SOME KILLS SPAWN MORE CARS"; //probably bad
}

#define race_ultra_take
//make sure you get the ultras
global.ultra[argument0] = 1;

#define race_ttip
return["prancer", "for glory!", "warm", "bundled up", "christmas jingles", "payback", "charge damage scales with#your level"];//i really love tips ok