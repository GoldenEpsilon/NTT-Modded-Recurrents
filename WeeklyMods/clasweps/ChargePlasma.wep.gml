#define init
global.sprite = sprite_add_weapon("ChargePlasmaGun.png",1,3);

#define weapon_name
return "Charge Plasma Gun";
...
with creator {
    //Standard bullet creation
        with instance_create(x+lengthdir_x(6,gunangle),y+lengthdir_y(6,gunangle),PlasmaBall){
  	speed = 3 + (num * 0.2)
  	direction = other.gunangle + (random_range(-3, 3) * other.accuracy);
      image_angle = direction;
      image_xscale = 0.7 + (num * 0.2) + (skill_get(17) * 0.4);
      image_yscale = 0.7 + (num * 0.2) + (skill_get(17) * 0.4);
      damage= 5 + (num * 1)
      team = other.team;
      creator = other;
        }
    //Visual effects
    weapon_post(8,-5,5);
    motion_add(gunangle,(num * -2))
    //Pitch increases with charge
    sound_play_pitch(sndPlasma,  .7 + .1*num)
}
