#define init
global.sprite = sprite_add_weapon("BloodGenocide.png",5,6);

#define weapon_name
return "ASSYRIAN ASSAULT";	//If this is offensive in any way, take it on Jsburg, He suggested it
...
sound_play_pitchvol(sndBloodCannon,0.8,2);
...
sound_play_pitchvol(sndBloodHurt,0.8,2);
lasthit = [global.sprite,"ASSYRIAN ASSAULT"]	
sound_play(snd_hurt);
}else
{
ammo[4] -= 13}
...
sound_play_pitchvol(sndBloodCannonEnd,0.8,2);
for(i = 65; i < 300; i += 45)
{
	with instance_create(x+lengthdir_x(i,gunangle+sdira+random_range(40,-40)),y+lengthdir_y(i,gunangle+sdira+random_range(30,-30)),MeatExplosion)
	{
		team = other.team;
		creator = other;
	}
	...
	with instance_create(x+lengthdir_x(i,gunangle+sdira+random_range(40,-40)),y+lengthdir_y(i,gunangle+sdira+random_range(0,-30)),MeatExplosion)
	{
		team = other.team;
		creator = other;
	}
	with instance_create(x+lengthdir_x(i,gunangle+sdira+random_range(40,0)),y+lengthdir_y(i,gunangle+sdira+random_range(30,0)),MeatExplosion)
	{
		team = other.team;
		creator = other;
	}
...
if weapon_get_name(wep) = "ASSYRIAN ASSAULT"{
reload = 0.1
}
if weapon_get_name(bwep) = "ASSYRIAN ASSAULT"{
breload = 0.1
}
        }
}
