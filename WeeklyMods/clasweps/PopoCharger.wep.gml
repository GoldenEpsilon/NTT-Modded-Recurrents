#define init
global.sprite = sprite_add_weapon("PopoCharger.png",2,3);

#define weapon_name
return "IDPD Charge Gun";
...
with creator {
    //Standard bullet creation
        with instance_create(x+lengthdir_x(6,gunangle),y+lengthdir_y(6,gunangle), IDPDBullet){
  	speed = 14 + (num * 0.5)
    ...
    sound_play_pitch(sndRogueRifle,  .7 + .1*num)
}
...
if charge >= 5 && charge < 7{
draw_text_nt(x, y - 18, '@r.@y.@g.@d..')}
if charge >= 7 && charge < 9{
draw_text_nt(x, y - 18, '@r.@y.@g..@d.')}
if charge >= 9 && charge < 10{
