#define init
global.sprite = sprite_add_weapon("ChargeFlameThrower.png",2,5);

#define weapon_name
return "Charge Flamethrower";
...
#define weapon_text
return "Holyshitwhydoesthisexist";
...
if charged = 0 and charge = maxcharge{
    ...
    sound_play_pitch(sndSwapFlame, 1.6)
...
with creator {
    ...
    sound_play_pitch(sndFlamerStart,  .7 + .1*num)
repeat(6+num){
  if instance_exists(self){
  {
    ...
    sound_play_pitch(sndFlamerLoop,  .7 + .1*num)
repeat(num+3){
    with instance_create(x,y,Flame)
    {
move_contact_solid(other.gunangle, 6);
      direction = other.gunangle + (random_range(-2*num,2*num) * other.accuracy);
      speed=8 + (num * 0.5);
      damage=1 + (num * 0.25);
...
audio_stop_sound(sndFlamerLoop);
    sound_play_pitch(sndFlamerStop,  .7 + .1*num)
}
