#define init
global.sprite = sprite_add_weapon("PopoGuider.png",4,3);

#define weapon_name
return "IDPD GUIDER BLASTER";
...
#define weapon_text
return "TRACK THEM DOWN";

#define weapon_fire
weapon_post(4,-4,4);
if skill_get(17){
sound_play_pitchvol(sndEliteShielderFire,1.1,1);
}else{
sound_play_pitchvol(sndEliteShielderFire,1.6,1);
}
...
with instance_create(x+lengthdir_x(8,gunangle),y+lengthdir_y(8,gunangle),CustomProjectile)
{
	...
	image_yscale = 0.6 + (skill_get(17) * 0.1)
	image_xscale = 0.9 + (skill_get(17) * 0.1)
	typ = 2;
	creator = other;
