#define init
global.sprite = sprite_add_weapon("SuperIceFlakCannon.png",3,4);
global.sprsflak = sprite_add("SuperIceFlakBullet.png",2,12,12)
...
#define weapon_fire
sound_play_pitchvol(sndSuperFlakCannon,1.3,1.0);
sound_play_pitchvol(sndDoubleFireShotgun,1.1,1.0);
weapon_post(10, -14, 24);
with instance_create(x + lengthdir_x(2,gunangle),y + lengthdir_y(2,gunangle),CustomProjectile)
{
  sprite_index = global.sprsflak;
  ...
  on_destroy = script_ref_create(sflakdestroy);
  on_step = script_ref_create(sflakstep);
  on_draw = script_ref_create(sflakdraw);
}
...
#define sflakdestroy
sound_play_pitchvol(sndSuperFlakExplode,1.3,0.9);
sound_play_pitchvol(sndDoubleFireShotgun,1,1);
for (i = 0; i < 360; i += 72)
{
with instance_create(x + lengthdir_x(2,i),y + lengthdir_y(10,i),CustomProjectile)
{
