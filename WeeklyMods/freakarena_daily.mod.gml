#define init 
	global.gce = 0;
	global.loops = 0;
	with instances_matching(CustomDraw,"name","float_msg") instance_destroy();
...
#define array_merge(a,b)
	var c = array_clone(a);
	array_copy(c, array_length(c), b, 0, array_length(b));
	return c;
...
#define mutator_create
	with(instance_create(argument0,argument1,CustomObject)){
		var d = 0;
		muts = [];
		mutlist = array_merge(global.mutlist,mod_get_names("skill"));
		for(var i = 0;i<array_length(mutlist);i++){
			if !skill_get(mutlist[i]){
				muts[d] = mutlist[i];
				d++;
			}
		}
...
#define mutator_step
	if abs(image_xscale) < 0.05{
		...
		if is_real(mut){
			...
			if mod_script_exists("skill",mut,"skill_button"){
				mod_script_call("skill",mut,"skill_button");
			}else{
				sprite_index = sprSkillIcon;
				image_index = mut_last_wish;
			}
		}
	}
	...
	if z + zspd > 0{
		zspd -= 0.5 * current_time_scale;
	}else{
		z = 0;
		zspd *= -0.33;
	}
	z += zspd * current_time_scale;
	y = ystart - z;
	with(Player){
...
#define campfire_step
	...
	// Player simulated death. Removed for dailies/weeklies
	with(projectile){
	...
	if(active){
		...
		if (timer > 0){
			// Deincrement when boss is not present, do so at half speed if boss is present
			if array_length_1d(instances_matching_ne(enemy,"intro",undefined)) = 0{
				timer -= current_time_scale;
			}else{
				timer -= current_time_scale / 4;
			}
			if timer <= 0{
				timer = 30 * 15;
				if mode = mode_freak{
	...
	if sprite_index = sprCampfire{
		...
		with(Player){
			...
			if distance_to_object(other) < 48{
				draw_text_nt(other.x,other.y-32,"MODE:#" + nl[? other.mode] + "#####SHOOT TO BEGIN");
			}
		}
...
#define step 
	...
	with(Last){
		if sprite_index != sprLastDash meleedamage = 0 
			else meleedamage = 10;
	}
	with(Player){
