...
for i in ${MODS[@]}
do
	DWL+="wait mod_load(string_trim("'"'"data/Weekly.mod/"$i'"'"));"$'\n'
done
...
WEP=$[$RANDOM % ${#WEAPONS[@]}]
echo -e "
/*@rMods@w=$(printf "#  %s" "${MOD[@]}")#@bCharacter@w=#  ${CHARACTERS[$CHR]}#@yCrown@w=#  ${CROWNS[$CRW]}#@gWeapon@w=#  ${WEAPONS[$WEP]}#@pSeed@w=$SEED#Limited Tries:${LIMITEDTRIES}*/
/*$(printf "|%s" "${COMMENTS[@]}")*/
#define init
...
FILE="leaderboards/Weekly.txt"
SCORE_DATE=$(echo $(cat "$FILE")| cut -d';' -f 2)
cp leaderboards/Weekly.txt leaderboards/${SCORE_DATE}-Weekly.txt
git add Weekly.mod.gml
git add leaderboards/${SCORE_DATE}-Weekly.txt
git rm leaderboards/Weekly.txt
echo -e "Mods:${MOD[0]}$(printf ",%s" "${MOD[@]:1}") Character:${CHARACTERS[$CHR]} Crown:${CROWNS[$CRW]} Weapon:${WEAPONS[$WEP]} Seed:$SEED Date:;$DATE;" | tee leaderboards/Weekly.txt
git add leaderboards/Weekly.txt
