...
/*@rMods@w=#  Challenge Mode#  Double Time#  Bioluminary Oscillator#  Crypt Of The Necrodancer#  Director#@bCharacter@w=#  Choose#@yCrown@w=#  crown of protection#@gWeapon@w=#  Bioluminary Oscillator#@pSeed@w=6340#Limited Tries:false*/
/*|Challenge Mode[This run's weapons are like a box of chocolates -#you finally find one you like, then you don't have it anymore.]|Double Time[GOTTAGOFAST]|Bioluminary Oscillator[TARGET AQUIRED]|Crypt Of The Necrodancer[Move to the beat!]|Director[He will direct your enemies#to the nearest exit of life]*/
#define init
...
trace("Current Daily:");
trace("Mods=Challenge Mode,Double Time,Bioluminary Oscillator,Crypt Of The Necrodancer,Director");
...
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Character: " + (ChooseCharacter ? global.Race : "") + " Mods:Challenge Mode,Double Time,Bioluminary Oscillator,Crypt Of The Necrodancer,Director" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Bioluminary Oscillator" + " Seed:6340";
	}
	trace(score);
	global.finished = true;
	if(file_exists(global.alias + " 2019-09-22 Daily.txt")){
		prevScores = string_load(global.alias + " 2019-09-22 Daily.txt");
		while(!file_loaded(global.alias + " 2019-09-22 Daily.txt")){wait 1;}
		string_save(prevScores, global.alias + " 2019-09-22 Daily.txt");
	}else{
		string_save(score, global.alias + " 2019-09-22 Daily.txt");
	}
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Challenge Mode,Double Time,Bioluminary Oscillator,Crypt Of The Necrodancer,Director" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Bioluminary Oscillator" + " Seed:6340" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(string_replace_all(string_replace_all(leaderboard,chr(10),""),chr(13),""), string_replace_all(string_replace_all(":Challenge Mode,Double Time,Bioluminary Oscillator,Crypt Of The Necrodancer,Director" + " Character:Choose" + " Crown:crown of protection" + " Weapon:Bioluminary Oscillator" + " Seed:6340",chr(10),""),chr(13),""))) > 1){
			var fail = 0;
