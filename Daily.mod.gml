...
/*@rMods@w=#  Big Dog#  basicgunallies#  Assault Flak Cannon#  Bandit Brute#  Ash#@bCharacter@w=#  Ash#@yCrown@w=#  crown of destiny#@gWeapon@w=#  golden revolver#@pSeed@w=1543*/
/*#*/
#define init
...
mod_load(string_trim("data/Daily.mod/Assault_Flak_Cannon.wep.gml"));
mod_load(string_trim("data/Daily.mod/BanditBrute.mod.gml"));
mod_load(string_trim("data/Daily.mod/ash/ash.race.gml"));


trace("Current Daily:");
trace("Mods=Big Dog,basicgunallies,Assault Flak Cannon,Bandit Brute,Ash");
trace("Character=Ash");
...
if(global.start){
	global.alias = "";
	with(Player){
		wep = string_trim("golden revolver");
		ammo[0] = 0;
		ammo[1] = 0;
		ammo[2] = 0;
		ammo[3] = 0;
...
if(!global.canStart){with(CharSelect){instance_change(CustomObject, 0);name=mod_current;race=string_lower(string_trim("Ash"));}}
else{with(instances_matching(CustomObject, "name", mod_current)){instance_change(CharSelect, 0);}with(CharSelect){race=string_lower(string_trim("Ash"));visible=true;}}
if(!global.finished && !instance_exists(Player) && !instance_exists(Menu)){
	var score = "";
	with(GameCont){
		score = global.alias + ": Area " + string(area) + "-" + string(subarea) + " L" + string(loops) + " Kills: " + string(kills) + " Mods:Big Dog,basicgunallies,Assault Flak Cannon,Bandit Brute,Ash" + " Character=Ash" + " Crown=crown of destiny" + " Weapon=golden revolver" + " Seed=1543";
	}
	trace(score);
	...
	if(global.qualified){
		...
		if(string_copy(leaderboard, 0, 3) == "404"){
			...
			"content":"'+base64("Mods:Big Dog,basicgunallies,Assault Flak Cannon,Bandit Brute,Ash" + " Character=Ash" + " Crown=crown of destiny" + " Weapon=golden revolver" + " Seed=1543" + "|" + string_split(score, "Mods:")[0])+'"
			}'
			, "out.txt");
		}else if(array_length(string_split(leaderboard, "Mods:Big Dog,basicgunallies,Assault Flak Cannon,Bandit Brute,Ash" + " Character=Ash" + " Crown=crown of destiny")) > 1){
			file_delete("sha.txt");
...
global.start = true;
with(Player){race=string_lower(string_trim("Ash"));}
give_crown();
